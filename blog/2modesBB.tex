...
\item[2014-01-31 Burak]                                     \toCB
\paragraph{Multiple shooting method for search for the \rpo s:}
...
is obtained by solving
    \PC{2014-02-02 note that I have changed your \refeq{PCnetwonStep} by
    $\Lg \flow{\zeit_i}{\ssp_i}
        \to
    \Lg \LieEl (\gSpace_i)\flow{\zeit_i}{\ssp_i}$.
    If I am right, the rest of Burak's draft is wrong.}
...
\etc.
Under Newton iteration, the improved values should converge to
the exact values
$(\Delta\ssp_i\,,\,\Delta\zeit_i\,,\,\Delta\gSpace_i)$
...
%\eeq
In order to deal with the marginal multipliers
along the time and group orbit directions,
we apply two constraints, a local Poincar\'e section orthogonal to
the flow, and a local slice orthogonal to the group orbit at each
...
\eeq
Imposing these local conditions is natural; in the limit of
infinitely many infinitesimal time steps, the \rpo\ that they yield
is the so-called `comoving \rpo'. A globally imposed fixed
Poincar\'e section and \slice\ should be equally good, and once
the \rpo is computed,
its shift $\gSpace_p$ in the `lab frame' can be computed by imposing
a fixed (not moving in time) slice.
...
\beq \label{eq:AforNewton}
	...
	\begin{pmatrix}
	  ...
	  \LieEl (\gSpace_1) \vel (\flow{\zeit_1}{\ssp_1}) &
	  \Lg \flow{\zeit_1}{\ssp_1}  &
	  - \matId & 0 & 0 & 0 & \cdots & 0 \\
	  \vel (x_1) & 0 & 0 & 0 & 0 & 0 & 0 & \cdots & 0 \\
	  \sliceTan{} & 0 & 0 & 0 & 0 & 0 & 0 & \cdots & 0 \\
	  0 & 0 & 0 &
	  \LieEl (\gSpace_2) \jMps^{\zeit_2}(\ssp_2) &
	  \LieEl (\gSpace_2) \vel (\flow{\zeit_2}{\ssp_2}) &
	  \Lg \flow{\zeit_2}{\ssp_2}   &
	  - \matId & \cdots & 0 \\
	  0 & 0 & 0 & \vel (x_2) & 0 & 0 & 0 & \cdots & 0 \\
	  0 & 0 & 0 & \sliceTan{} & 0 & 0 & 0 & \cdots & 0 \\
	  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \ddots & \vdots \\
	  - \matId & 0 & \cdots & 0 & 0 & 0 &
	  \LieEl (\gSpace_n) \jMps^{\zeit_n}(\ssp_n) &
	  \LieEl (\gSpace_n) \vel (\flow{\zeit_n}{\ssp_n}) &
	   \Lg \flow{\zeit_1}{\ssp_1}   \\
...
\beq \label{eq:DeltaandE}
	\Delta =
	 ...
	 E =
	 \begin{pmatrix}
	  \ssp_{2} - \LieEl (\gSpace_1) \flow{\zeit_1}{\ssp_1} \\
	   0 	\\
	   0 	\\
	  \ssp_{3} - \LieEl (\gSpace_2) \flow{\zeit_2}{\ssp_2} \\
	  0 	\\
	  0 \\
	  \vdots \\
	  \ssp_{1} - \LieEl (\gSpace_n) \flow{\zeit_n}{\ssp_n} \\
	  0 	\\
	  0 	\\
	 \end{pmatrix}
\eeq
...
I stopped, however, until Burak and I agree on my correction to his
initial formula.
