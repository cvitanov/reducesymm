...
decomposition\rf{DinCvi14} code\rf{DinCvi14} to compute the Floquet
...
\bea
    - \sum_{r=1}^{\infty} \frac{1}{r} \frac{e^{r(\beta A_p - s T_p)}
                                               z^{n_p r}}{\oneMinJ{r}}
                                               &\approx &
    - \sum_{r=1}^{r n_p \leq N} \frac{1}{r} \frac{e^{r(\beta A_p - s T_p)}
                                                    z^{n_p r}}{\oneMinJ{r}} \,
                                    \continue
                                 &=&
                          c_1(\beta , s) z^{n_p}
...
for the leading zero of \refeq{e-NthOrderSpectDet} with $\beta = 0$, namely, solved
...
\ee{e-meanT}
with, $F_N$ given by \refeq{e-NthOrderSpectDet}.
...
the spectral determinant \refeq{e-SpectralDeterminant}, observables measured
...
\reftab{t-DynamicalAveragesNoGrammar}. Convergence-wise, it goes slightly
...
\reftab{t-DynamicalAveragesNoGrammar} is ordered in $z^n$, not in the
pseudocycle stabilities, so the convergence of the sequences in
\reftab{t-DynamicalAveragesNoGrammar} can potentially be improved.
...
at length 8 in \reftab{t-DynamicalAveragesNoGrammar}? If finite grammar approximation
...
\item[2014-06-24 Burak] I updated \reftab{t-DynamicalAveragesNoGrammar} and
...
why you get different results for escape rate for $N=1$ in \reftab{t-DynamicalAveragesNoGrammar}
...
\item[2014-07-14 Burak] You are right, $N=1$ results in \reftab{t-DynamicalAveragesNoGrammar}
...
\item[2014-07-19 Evangelos] Great! Now \reftab{t-DynamicalAveragesNoGrammar} and
...
to find it? This would also make the comparison between \reftab{t-DynamicalAveragesNoGrammar}
...
I'm back into at least editing mode. Here's a couple of preliminary comments to
go with any edits that I make to the manuscript: 1.) Don't love the title
