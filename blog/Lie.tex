...
Mathematicians can even make sense of the $d= $odd-dimensional case. If
...
progress\rf{Dragt11}. Fulton and Harris\rf{Fulton91} seem to use it.
...
Killing vector fields can also be defined on any (possibly nonmetric) manifold $\pS$ if we take any Lie group $\Group$ acting on it instead of the group of isometries. In this broader sense, a Killing vector field is the pushforward of a left invariant vector field on $\Group$ by the group action. 
If the group action is effective, then the space of the Killing vector fields is isomorphic to the Lie algebra $\mathfrak{g}$ of $\Group$.
