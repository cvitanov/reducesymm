...
\item[2012-04-28 Keith to Evangelos]  Yes I actually did that this morning, it turns out that it is a limit cycle.  So sadly this is not a good situation to use.
...
don't want to symmetry reduce within invariant subspaces? [2012-04-28 Keith] I never intended to use this as the invariant subspace, I just needed something to cut through the symmetries to see what was happening instead of the full space.  I agree, using this space is not optimal.
...
much easier: how to solve for \eqva\ of \refeq{PKinvEqs}? Can rewrite it
as
\bea
  0  &=& 2\,u\,(\mu_1+a_1\,u+b_1\,v) +c_1\,w
\continue
  0  &=& 2\,v\,(\mu_2+a_2\,u+b_2\,v) +c_2\,w
\continue
  0  &=& [2\,\mu_1+\mu_2+(2a_1+a_2)\,u+(2b_1+b_2)\,v]\,w
\ceq
...
can be easily eliminated, by rewriting \refeq{PKinvEqs3}
\bea
  w  &=& - \frac{2\,u}{c_1}(\mu_1+a_1\,u+b_1\,v)
\continue
  w  &=& - \frac{2\,v}{c_2}(\mu_2+a_2\,u+b_2\,v)
\continue
   e_2\,q  &=& A\,w   +c_1\,u\,v +c_2\,u^2
\label{PKinvEqs4}\\
  w  &=& -\frac{4}{e_2} A\,q
\continue
  \mbox{where   }  2\,A &=& 2\,\mu_1+\mu_2+(2a_1+a_2)\,u+(2b_1+b_2)\,v
\,,
\nnu
\eea
and we obtain two (bivariate)  polynomials in two variables $\{u,v\}$ with
constant coefficients
\bea
  \frac{u}{c_1}(\mu_1+a_1\,u+b_1\,v)
     &=&
  \frac{v}{c_2}(\mu_2+a_2\,u+b_2\,v)
\continue
  \frac{2}{e_2^2} A \, (A\,w   +2\,c_1\,u\,v +2\,c_2\,u^2)
     &=& \frac{u}{c_1}(\mu_1+a_1\,u+b_1\,v)
       + \frac{v}{c_2}(\mu_2+a_2\,u+b_2\,v)
\,,
\label{PKinvEqs5}
\eea
where $w$ still has to be inserted from either of the first two equations
in \refeq{PKinvEqs4}. This equation seems nowhere as simple as
what as of {[2012-04-26]} Keith, Lei, and Predrag had:
    \bea
 \frac{u^2}{c_1^2} (\mu_1 +a_1 u+b_1 v)^2
+\frac{u^2 v^2 e_2^2}{(c_2 u+2 c_1 v)^2}  &=& u^2 \, v
    \continue
 \frac{v^2}{c_2^2} (\mu_2 +a_2 u +b_2 v)^2
+\frac{u^2 v^2 e_2^2}{(c_2 u+2 c_1 v)^2}  &=& u^2 \, v
\,.
\label{2modefixdenom}
\eea
The denominator on the $e^2 v$ terms in both equations should be the
same since this comes from the $\cos \psi$ term.
...
\eeq
This is the same as the second equation of \refeq{PKinvEqs5}?
 {\color{red}Please prove my algebra wrong!}
    \PC{Also check your
...
4$. So we are down to maximum 8 roots; as we know two already, we are
...
\item[2012-04-28 Bryce] The simplest root I was able to find (aside from the obvious ones) was the following:
\beq
\left\{u\to \frac{\text{b2} \text{$\mu $1}-\text{b1} \text{$\mu $2}}{\text{a2}
   \text{b1}-\text{a1} \text{b2}},v\to \frac{\text{a2} \text{$\mu $1}-\text{a1}
   \text{$\mu $2}}{\text{a1} \text{b2}-\text{a2} \text{b1}},w\to 0,q\to
   \frac{(\text{b1} \text{$\mu $2}-\text{b2} \text{$\mu $1}) (-\text{a1} \text{c1}
   \text{$\mu $2}+\text{a2} \text{c1} \text{$\mu $1}+\text{b1} \text{c2} \text{$\mu
   $2}-\text{b2} \text{c2} \text{$\mu $1})}{\text{e2} (\text{a2} \text{b1}-\text{a1}
   \text{b2})^2}\right\}
\label{invroot2}
\eeq
