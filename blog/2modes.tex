...
%guess are relative equilibria/\po s, since the symmetry is
...
%Up to a factor of 2
Equations \refeq{PKinvEqs5a} are what Lei had as of {[2012-04-26]}
...
solution was fixed point of the \twoMode\ system using Matlab. Uploaded
relevant code as
\texttt{/siminos/cgang/Daniel/Matlab/W0EquilibriumCheck.m}.
[2012-05-08 Predrag] the error was introduced by me, in process of
...
to find a system with two unstable relative equilibria and
...
not played with it nor rederived relative equilibria conditions yet. The
...
\item[2012-08-10 Evangelos] As an example calculation I've chosen 
$\mu_1 = -0.234, \mu_2 = 0.28, c_1 = 1; c_2 = 1, a_1 = -1.4, b_1 = 1.526; 
a_2 = -2.18, b_2 = -0.1, e_2 = 1.217, e_1 = 0$. 
My Mathematica code uses \refeq{PKinvEqs5} to find relative equilibria. 
There are two relative equilibria with $(u,v,w,q)$ coordinates
 \begin{align}
...
we get eigenvalues:
\begin{align}
  \lambda^{(1)}_i:\qquad  & (1.83992, 0.872034, -0.10026 + 0.516938 \ii, -0.10026 - 0.516938 \ii)\\
  \lambda^{(2)}_i:\qquad  & (-0.191519, -1.1123 + 0.497766 \ii, -1.1123 - 0.497766 \ii, -2.12585) 
\end{align}
Using \refeq{2modeStabMatrixInvSyz} (invariant polynomial basis, with
syzygy incorporated) we get eigenvalues:
\begin{align}
  \lambda^{(1)}_i:\qquad  & (0.872034, -0.10026 + 0.516938 \ii, -0.10026 - 0.516938 \ii)\\
  \lambda^{(2)}_i:\qquad  & (-0.191519, -1.1123 + 0.497766 \ii, -1.1123 - 0.497766 \ii) 
\end{align}
I do not understand what the missing eigenvalue really means. 

Using \refeq{2modeStabMatrix} (original coordinates) we get
\begin{align}
  \lambda^{(1)}_i:\qquad  & (1.51099, -0.177221 + 1.42156 \ii, -0.177221 - 1.42156 \ii, -0.121078)\\
  \lambda^{(2)}_i:\qquad  & (0.608294, -0.853009, -1.0441 + 1.29555 \ii, -1.0441 - 1.29555 \ii) 
\end{align}
Note that in this case the eigenvalues are related in any way to the
ones we find using invariant polynomials. If one wants to compute
stability in original coordinates, she has to go to a comoving frame.
As discussed in Appendix B of Cvitanovi\'c \etal\rf{SCD07}, 
this is equivalent to using the matrix $A+c\, L$ where $c$ the 
``drift velocity'' and $L$ the generator of the group. Matrices $A$ and 
$A+c\, L$ will in general have different eigenvalues, so I disagree
with [2012-08-01 Predrag] above.  
...
see it. Also, how is $H^{1}$ defined in 5D real space? I guess the $k^2$
