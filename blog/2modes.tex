...
go to \texttt{siminos/atlas/}, blog in chapter~\ref{chap:atlas}{\em Atlas}.
  \item
...
%back-burner and focus on the \cLe, even if the {\reqva} are
...
Rich (I'm assuming Kerswell?) has been looking at \po s in 2D
...
the fact that they can find (exact, rather than relative) \po s
probably means that the system does not have continuous symmetries. If it
did it'd be very hard to find exact \po s.
...
%Everything was either attractive fixed points or \po s, which I
%guess are {\reqva}/\po s, since the symmetry is
...
invariant subspaces. The papers discussed above exhibit some stable \reqva\
and \rpo s for specific (out-of the hat) parameter choices. You want to
screw up some parameters higher so these solutions go unstable.

\item[2012-04-05 Lei to Predrag]
I can see the $e$ added really breaks the $\On{2}$ symmetry to $\SOn{2}$
...
points (in polar coordinates) have conjugate complex eigenvalues. So there is nice spirals
...
\po s shouldn't be a problem, since once we reduce they become
\eqv.
...
of {\reqva} in polar coordinates, even though it is easier to locate
{\reqva} in polar coordinates. I think one should do what we did
in \refref{SCD07}.
...
10th order polynomial equation for {\reqva}. Could you please
enter it here?
...
\bea
\dot{x}_1 &=& \mu_1 x_1 + a_1 x_1^3 + b_1 x_1 y_1^2 + c_1 x_1 y_1 + a_1 x_1 x_2^2 + b_1 x_1 y_2^2 + c_1 x_2 y_2
\continue
\dot{x}_2 &=& \mu_1 x_2 + a_1 x_1^2 x_2 + c_1 x_1 y_2 + b_1 y_1^2 x_2 - c_1 y_1 x_2 + a_1 x_2^3
              + b_1 x_2 y_2^2
\continue
\dot{y}_1 &=& \mu_2 y_1 + a_2 x_1^2 y_1 + c_2 x_1^2 + b_2 y_1^3 + a_2 y_1 x_2^2 + b_2 y_1 y_2^2  - c_2 x_2^2 + e_2 y_2
\continue
\dot{y}_2 &=& \mu_2 y_2 + a_2 x_1^2 y_2 + 2 c_2 x_1 x_2 + b_2 y_1^2 y_2 - e_2 y_1 + a_2 x_2^2 y_2 + b_2 y_2^3
\label{2mode4Da}
\eea
...
\footnotesize
%\begin{widetext}
\beq \Mvar  \, = \left( \begin{array}{cccc}
        \mu_1 + 3 a_1 x_1^2 + b_1 y_1^2 \\
         + c_1 y_1 + a_1 x_2^2 + b_1 y_2^2
             &  c_1 y_2 + 2 a_1 x_1 x_2 & c_1 x_1 + 2 b_1 x_1 y_1
                & c_1 x_2 + 2 b_1 x_1 y_2 \\
        c_1 y_2 + 2 a_1 x_1 x_2
             &  \mu_1 + a_1 x_1^2 + b_1 y_1^2 \\
             &  - c_1 y_1 + 3 a_1 x_2^2 + b_1 y_2^2
                & 2 b_1 y_1 x_2 - c_1 x_2
                   & c_1 x_1 + 2 b_1 x_2 y_2 \\
          2 c_2 x_1 + 2 a_2 x_1 y_1
              & 2 a_2 y_1 x_2 - 2 c_2 x_2
                 &  \mu_2 + a_2 x_1^2 + 3 b_2 y_1^2 \\
              &  & + a_2 x_2^2 + b_2 y_2^2
                 & e_2 + 2 b_2 y_1 y_2\\
          2 c_2 x_2 + 2 a_2 x_1 y_2
              & 2 c_2 x_1 + 2 a_2 x_2 y_2
                 & 2 b_2 y_1 y_2 - e_2
                    & \mu_2 + a_2 x_1^2 + b_2 y_1^2 \\
                    &&& + a_2 x_2^2 + 3 b_2 y_2^2
      \end{array} \right)
\,. \ee{2modeStabMatrix}
%\end{widetext}
\normalsize
...
in {\twomode} $\SOn{2}$-symmetric equations and found that they do not
...
cleanest way to compute {\reqva}, as equations are given in
a clear polynomial form.
To find {\reqva} in $u,v,w,q$ basis
...
for meager \reqva\ and the easy \reqva.
	\begin{itemize}
		\item By using Matlab to symbolically solve  \refeq{2mode4Da},
             I found that the only \eqv\ is the origin, independent
...
\emph{Stability}).
             The eigenvalues are $\Lyap_1 = \mu_1$ with multiplicity 2 and
             $\Lyap_2 = \mu_2 \pm i e_2$. The eigenvectors for
             $\Lyap_1$ are $(1,0,0,0)$ and $(0,1,0,0)$ in the
             $(x_1,x_2,y_1,y_2)$ basis. The complex eigenvectors for
             $\Lyap_2$ are $(0,0,\mp i, 1)$. {\bf [2012-04-25 Predrag]}
             One always replaces complex eigenvector pair by  $\{\Re
             \jEigvec[j], \Im \jEigvec[j]\}$: that defines the real plane
             in which spiral-out takes place.
             The eigenvectors for
             $\Lyap_2$ thus are $(0,0,1,0)$ and $(0,0,0,1)$.
...
symmetry reduce \refeq{2mode4Da}. I am currently choosing a random slice
...
``attractor'' for (not a good choice) parameters \refeq{pars2012-04-25}.
...
variables \refeq{2mode4Da}- unfortunately for \cLe\ one writes $z_1 = x_1
...
of $z_1$ and $z_2$ in blog and \emph{my} Matlab codes to be consistent with the substitution used in DasBuch for
...
Starting from the following definitions:
    \PC{I agree this would be more natural, but that is not the $w$
    Dangelmayr used so we are stuck with unnatural factors of 2. Grin and
    bear it.}
\bea
u &=& {z}_1 \overline{z}_1
    \,,\quad
v = {z}_2 \overline{z}_2
    \continue
w &=& (z_1^2 \overline{z}_2 + \overline{z}_1^2 {z}_2)/2
    %\DBedit{/2} %Otherwise equality doesn't hold
...
Related by the syzygy $w^2+q^2 - u^2v =0$.
    \DB{2012-04-28}{Looks like Predrag changed these but I want to put
        these down for now.}
...
\label{PKinvEqsDB}
\eea
Again... these are using the \refeq{Dang86(1.2)DB} definitions of $w$ and
$q$,  not the ones that Predrag has specified in
\refsect{s:twoMode}. Use at your own risk.
...
%Up to a factor of 2
Eqs.~\refeq{PKinvEqs5a} are what Lei had as of {[2012-04-26]}
...
\,,
\ee{2modeRequa4}
but we know from \refeq{PKinvEqs5} that only `-' sign is correct.
    \PC{2012-04-28 The invariant polynomials get this right and probably
        might cure the wrong roots problems that [2012-04-27 Keith] has
        noted}
    \DB{2012-04-30}{This can also be shown by just solving the first two
        equations in \refeq{eq:AGpolarREQV} for $\cos{\psi}$ and setting
        them equal to each other. Gets the sign right too, no roots
        problem.}
Adding, and using \refeq{PKinvEqs4} I get
\beq
...
As we already know $[0,0,0,0]$ and $[0,v,0,0]$ roots,
we should divide them out, and that is what we have done. Finding roots
of bivariate polynomials is not easy. As fate would have it, you can
...
\left\{
   ...
   w\to 0,
   q\to \frac{2 u}{e_2 } \, (2 c_1 v   +  c_2 u) % Corrected by DB on 2012-05-08
\,.
\right\}
\label{invroot2}
\eeq
...
\item[2012-05-08 Daniel] Double checked eq. \refeq{invroot2} and found
that it was wrong (factors of 2 and $u$ and $v$ were switched in the term
in parentheses in the equation for $q$). Verified that my corrected
solution was an \eqv\ of the \twomode\ system using Matlab. Uploaded
relevant code as
\texttt{/siminos/cgang/Daniel/Matlab/W0EquilibriumCheck.m}.
{\bf [2012-05-08 Predrag]} the error was introduced by me, in process of
deMathematicizing Bryce's formula. Thanks Daniel for fix that pesky
factor of 2! Should be able to do verify it by hand now, using
\refeq{PKinvEqs6}.
...
The {\reqv} discovered by Bryce on 2012-04-28
\refeq{invroot2}
\bea
   u&=&   \frac{b_2 \mu_1 -b_1 \mu_2 }{a_2 b_1-a_1 b_2},
\continue
   v&=& - \frac{a_2 \mu_1 -a_1 \mu_2 }{a_2 b_1-a_1 b_2},
\continue
   w&=& 0,
\continue
   q&=& \frac{2 u}{e_2 } \, (2 c_1 v   +  c_2 u) % Corrected by DB on 2012-05-08
\,.
\label{invroot2a}
\eea
Does it satisfy syzygy $w^2+q^2-4u^2v=0 \to q^2=4u^2v$? It says:
\beq
\frac{4}{e_2^2 } \, (2 c_1 v   +  c_2 u)^2 = v
\,,
\ee{BryceNoSolu}
...
repelling directions to win. If $\{\Lyap_e\}$ is the set of expanding
eigenvalues, and $\{\Lyap_c\}$ the set of contracting ones, I believe
...
positive and one is negative. Please check my algebra,
confirm here when done. [DB 2012-05-01] Checked this last night and it was fine.
...
at my corrections in {\color{red}} in \refsect{s:twoMode} and see
...
\refeq{PKinvEqs1}.  Claims I am wrong. Corrected various terms.

\item[2012-04-30 Daniel to Evangelos] I don't see how you get enough
$v$'s to get a $v^2$ in $\dot{u}$ in \refeq{PKinvEqs1}. The highest power
...
Substitute $\ssp=\LieEl\sspRed$.
    \PC{This is OK for abelian groups, as along the way I assumed $\LieEl
    \Lg = \Lg \LieEl$, but we have to redo it more carefully for nonabelian
    groups (like $\SUn{n}$). I think the answer is the `Cartan derivative
    at unity', but we'll cross that ridge when we come to it. For
    example, we slice spiral chaos induced by the Euclidean
    $E(2)$ symmetry.}
...
\item[2012-05-01 Lei] Discussed with Bryce and trying to finish our term
paper by summarize all we have about PK system. I'm really confused about
the LaTeX templates used in the paper, so I wrote in my own templates and
put it in cgang/2-mode-Bryce-Lei. Other sections just copied from the
previous paper, rewrite a little using my own words. Just begins, keep
moving.

% \item[2012-05-01 Lei to Predrag] Still can't login using cnsuser cnsweb...

\item[2012-05-01 Daniel] Following up on Predrag's cry in the wilderness
of 2012-04-29, we need the trace of the {\stabmat},
\(
\tr \Mvar  = \sum \Lyap_e > - \sum \Lyap_c
\,,\)
to be greater than zero. For future reference, the trace of the
{\stabmat} for a generic point is given by
\begin{equation}
\tr \Mvar  = 2(\mu_1\,+\,\mu_2)\,+\,(4 a_1\,+\,2 a_2)\,r_1^2\,+\,(2 b_1\,+\,4 b_2)\,r_2^2
\label{2ModeStabTrace}
\end{equation}
This agrees with my earlier result of 2012-04-27 for the origin, but may
allow us to put further constraints on the parameter choices, especially
since $u,\,v \geq 0$.

\item[2012-04-29 Daniel] Here's a couple of other results that may be
useful for constraining parameter choices or looking for \reqva. Setting
the equation for $\dot{\psi}$ in the polar form of the \twomode\ system
to zero or starting from the equation for $q$ in \refeq{PKinvEqs4} and
the fact the fact that $q =  2\,u |v|^{1/2} \sin \psi$, one can solve for
...
condition is an additional constraint on what $r_1$ and $r_2$ (or $u$ and
$v$) can be. Might prove useful if we have to brute force search for
roots.

If one starts with the $A_1$ equation for $w$ in \refeq{PKinvEqs4} and $w
=  2\,u |v|^{1/2} \cos \psi$ and uses the constraint that $\cos{\psi}
\leq 1$, you can work out that
\beq
\dfrac{a_1}{b_1}r_1^2
 \,+\,\left(r_2\,+\,\dfrac{c_1}{2 b_1}\right)^2
 \,\geq\,\dfrac{c_1^2\,-\,4 \mu_1 b_1}{4 b_1^2}
\ee{2ModeCosCond1}
This result and the conditions from the range of $\sin$
\refeq{2modeSinCond} are nice but not great because they tell us that the
$r$'s must lie \emph{outside} of particular ellipses (i.e., where the
root are not, but not where the roots \emph{are}). However, it seems
(unless I messed up the algebra, so {\color{red} somebody please double
check this and report back here}) that the final condition $\cos{\psi}
\geq -1$ gives the more powerful constraint
\beq
\dfrac{a_1}{b_1}r_1^2\,+\,\left(r_2\,-\,\dfrac{c_1}{2 b_1}\right)^2
 \,\leq\,\dfrac{c_1^2\,-\,4 \mu_1 b_1}{4 b_1^2}
\ee{2ModeCosCond2}
This constraint tells us that valid $r$'s need to be \emph{inside} of a
certain ellipse, which really cuts down on the available \statesp!
Furthermore, the lefthand side of the inequality is non-negative, so the
righthand side must also be non-negative.
...
I have planes and conics waiting to be added.  I'd like to try and  setup
a model with buttons to turn vectors, sections, etc on and off.
Still have some bugs to work out. Let me know how it looks to you.

\item[2012-05-06  Predrag to Sarah] It's great! Let me know when you are
ready for public version, and I'll link it to
\HREF{http://ChaosBook.org/extras/\#maps}{ChaosBook.org}. Turning vectors,
sections, etc. on and off is probably necessary, as right now it is
pretty slow when I try to turn the R\"ossler attractor around.

\item[2012-05-16 Predrag to  Daniel]
Regarding [2012-04-27 Predrag], [2012-04-28 Daniel], and [2012-04-29
Daniel] above: Have you really changed to $z_1 = x_1 + \ii x_2$ and $z_2
= y_1 + \ii y_2$? Left-hand-side of \refeq{2mode4D} and \refeq{2mode4Da}
looks like the old version; $\mu_i$ factors do not match up. Please
recheck. [2012-06-11 Daniel] Somehow missed this request. Double-checked
them and it turns out that I forgot to update the left-hand sides of
\refeq{2mode4D} and \refeq{2mode4Da}. I have now corrected them. For
details of the derivation see
\texttt{/siminos/cgang/Daniel/Matlab/EOMComplexTo4.m}.

\item[2012-07-25 Evangelos] I have just finished writing a plasma physics
paper and I will have some time to work on the \twomode\ system. I have
no choice anyway, since a have to give a 15 min talk in Dynamics Days
Europe about slicing and I'd better have something new to say.

Are you still interested in / have time for this? I would suggest that we
split the tasks in some clear way so that we do not duplicate effort
(except for verification).

\item[2012-07-25 Daniel] I would definitely be interested in helping you
out. I don't have a TON of time (since I DO need to graduate at some
point), but I should be able to do some work in my spare time. I agree
that splitting up the tasks (and perhaps defining a goal and outlining
what the plan of attack will be) may help make things more efficient.

\item[2012-07-26 Predrag] The current draft \emph{\color{red}is} an
outline - improve it in the style that helps you see the goal clearly.

\item[2012-07-26 Keith]
I have waited a day to think about this before responding.  I don't want
to be irresponsible since I have been extremely busy this summer;
however, the temptation to see this to completion as well as seeing if we
can produce a return map is too great.  Having said that, as long as you
are ok with me not being as involved over the next week to two weeks
(things should calm down by the end of next week, but I say two to be
safe), I would love to be able to help out mostly out of curiosity to see
if I can generate my own return map as I was looking forward to do for
this 2-mode problem.  If this is problematic, then I can await the
published results.  If I am holding it down as well, then feel free to
drop me as well.

\item[2012-07-26 Predrag to Keith] OK, your job is generate return maps.
It will take a few weeks of experimentation with parameters before Chaos
Gang settles on a particular system to study. If you feel you cannot do
it, let us know.

I have dropped Bryce and Lei from the \twomode\ paper, as they have not
responded whether they are still in the gang or not.

\item[2012-07-30 Evangelos] I hope it won't take a few weeks to settle on
parameters as I only have two weeks of ``quality'' time before Dynamics Days
(Aug. 4-24 I am in Greece and usually it's very hard to work there).
However, Predrag might be right - I've spend the whole day Friday trying
to find a system with two unstable {\reqva} and
no attracting limit cycle and could not do it.

\item[2012-07-30 Evangelos]
2-modes system is driving nuts - I can't find interesting behavior.
Color-coded sign of $\tr \Mvar$ in \texttt{dangelmayr\_so2\_int.nb} in hope it
might help.

\item[2012-07-30 Evangelos] Starting from  \refeq{PKinvEqs} I compute the
following {\stabmat} for the \twomode\ system in invariant variables:
% \begin{widetext}
\footnotesize
\beq
\Mvar  \, =
\left( \begin{array}{llll}
	2 \mu_1 + 4 a_1 u + 2 b_1 v & 2 b_1 u & c_1 & 0 \\
	2 a_2 v & 2 \mu_2+2 a_2 u+4 b_2 v & c_2 & 0 \\
	4 c_2 u+4 c_1 v+(2 a_1+a_2) w & 4 c_1 u+(2 b_1+b_2) w & 2 \mu_1+\mu_2+(2 a_1+a_2) u+(2 b_1+b_2) v & 2e_1-e_2 \\
	(2 a_1+a_2) q & (2 b_1+b_2) q & -2e_1+e_2 & 2 \mu_1+\mu_2+(2 a_1+a_2) u+(2 b_1+b_2) v \\
      \end{array} \right)
\,.
\ee{2modeStabMatrixInv}
% \end{widetext}
\normalsize
The trace of this matrix is
\beq\label{2ModeStabTraceES}
  \tr \Mvar =2(3\mu_1+2\mu_2)+4(2a_1+a_2)u+6(b_1+b_2)v\,,
\eeq
not in agreement with \refeq{2ModeStabTrace},
\[
\tr \Mvar  = 2(\mu_1\,+\,\mu_2)\,+\,2(2 a_1\,+\, a_2) u\,+\,2(b_1\,+\,2 b_2) v
%\label{2ModeStabTrace}
\]
Daniel, are you using a different notation? Or am I doing something
wrong? Once cross-checked/corrected, I'll move \refeq{2modeStabMatrixInv}
and \refeq{2ModeStabTraceES} to the main text.

\item[2012-07-30 Daniel to Evangelos] Hmmm... To get
\refeq{2ModeStabTrace}, I started from \refeq{eq:DangSO2}, plugged in
$z_1 = x_1 + i x_2$ and $z_2 = y_1 + i y_2$, to get \refeq{2mode4D}, and
then calculated the jacobian \refeq{2modeStabMatrix} and its trace. This
was done using \texttt{/cgang/Daniel/Matlab/EOMComplexTo4.m}. I then,
replaced $x_1^2 + x_2^2$ by $u$ and $y_1^2 + y_2^2$ by $v$ to get
\refeq{2ModeStabTrace}.

If I start from \refeq{eq:DangSO2}, go to the $[u, v, w, q]$ basis to get
\refeq{PKinvEqs1}, then take the jacobian and trace, I get the same
result \refeq{2modeStabMatrixInv} and \refeq{2ModeStabTraceES} that you
do. This was calculated using
\texttt{/cgang/Daniel/Matlab/PKComplexToInvariant.m}

I guess the traces of the {\stabmats} for the \twomode\ system
written in the $[x_1, x_2, y_1, y_2]$ basis should be expected to be
different from the {\stabmat} for the system written in the $[u, v,
w, q]$ basis, right?

\item[2012-08-01 Predrag] Traces of {\stabmats} should be the same?
In the symmetry-\reducedsp\ there is one eigenvalue less, but its value
is zero.

\item[2012-07-31 Evangelos to Daniel] I think that $\Mvar$ (and its
trace) have to be computed in invariant coordinates of some form. For
instance, a \reqv\ is a \po\ in original space
and if we just pick a point on it and compute $\Mvar$, it tells as
nothing about stability, right?

\item[2012-08-01 Predrag] {\Stabmat} $\Mvar$ is explicitly coordinate
dependent. However, Floquet multipliers of invariant sets (\eqva,
\reqva,\po s, \rpo s, ...) are smooth-conjugacy invariants, so they,
their sums (traces) and their products (determinants) should not change
under (locally differentiable) coordinate changes. We should write this
down someplace more precisely, but going from equivariant coordinates to
invariant coordinates + syzygies keeps all multipliers the same.

In case at hand, while $\Mvar = \Mvar(\ssp(\zeit))$ depends on the point
$\ssp(\zeit)$ in the \reqv\ orbit, $\tr \Mvar(\ssp(\zeit)) = \tr\Mvar$
should not. If I am correct, for a \reqv\ the eigenvalues $\{\eigExp_j\}$
of $\Mvar(\ssp(\zeit))$ are constants independent of $\ssp(\zeit)$, and
the Floquet multipliers for one traversal of the \reqv\ orbit are
$\ExpaEig_j=\exp(\period{} \eigExp_j)$, where $\period{} = 2\pi
/\velRel$, and $\velRel$ is the \phaseVel.

\item[2012-08-01 Daniel] If what Predrag says is true and the traces
should be the equivalent independent of whether we use equivariant
coordinates or invariant coordinates, then I have either made a mistake
in calculating one or the other. It is kind of worrisome that these do
not agree for the stability of the origin. I've double checked my
calculation, but I don't see anything wrong with it. Is it possible that
they are not the same and that their difference (or something like that)
lets us extract a multiplier or something. Or maybe equating the two
gives some kind of information. I just don't see what I did wrong in my
computation.

\item[2012-07-31 Evangelos to Daniel]
One should go to a reduced space where the \reqv\ is an
equilibrium and compute its stability. Since the dimensions of the
original and reduced spaces are not the same (note there is a syzygy), we
don't have the nice property of invariance of stability eigenvalues under
smooth conjugacies.

What confuses me still, is whether one should use the syzygy explicitly.
Should we first eliminate one variable using the syzygy, then compute the
Jacobian as a $[3\!\times\!3]$ matrix? Maybe I'll try this here.

\item[2012-08-01 Predrag] That's an open question that Evangelos never
wrote up in his thesis, and would be nice to have a clean statement about
how it works. Infinitesimal variations are constrained to the
$d-N$-dimensional hypersurface defined by the syzygies - should not be
necessary to substitute them explicitly. I assume this is answered
somewhere in Gilmore and  Letellier\rf{GL-Gil07b} (read it
\HREF{http://chaosbook.org/library/GilLetSymm07.pdf} {here}) or if not,
elsewhere in the immense literature that uses invariant polynomials. Most
likely, the answer is the same as in constraining the Newton searches to
section an slice (for \rpo s); add every syzygy condition by a Lagrange
multiplier.

Or perhaps \HREF{http://ChaosBook.org/paper.shtml\#stability}{ChaosBook.org}
{\em 4.5.1 Stability of Poincar\'e return maps} - there the constraint
$U(\ssp + \delta\ssp)=0$ is your ticket.

\item[2012-08-01 Daniel] I seem to remember the syzygy simply coming out
as an equation when I was trying to solve for equilibria. I think after
massaging $g(u,v)$ in \refeq{PKinvEqs5}.

\item[2012-07-30 Daniel to Evangelos and Predrag] What would be the
features of a ``good'' set of parameters? Obviously we need the term that
breaks the reflection symmetry, but other than that? What's our metric
for chaos? Do we want to be calculating Lyapunov exponents or frequency
spectra of time series and looking for broadband noise?

\item[2012-07-31 Evangelos] Predrag could you answer this? I've added a
second $e_1$ term in hopes of getting more interesting behavior but have
not played with it nor rederived {\reqva} conditions yet. The
problem with any obvious way to detect chaos (such as Lyapunov exponents)
is that it does not provide a feeling on whether there are qualitatively
different regions in phase space, which we could pick as our templates.
Maybe the right thing to do is to scan the parameter space plotting the
largest Lyapunov exponent and at the end looking in details cases which
seem promising. The problem is the large number of parameters involved.

\item[2012-08-01 Predrag]                   \toCB
Until advent of period doubling experimentalist used their expensive
multi-channel analyzers to extract broad band spectra from chaotic
dynamics. I thought we had killed that monstrosity once for all - as
chaotic motions (in particular, the unstable \po s) have no fixed time
scale, Fourier spectra provide no information about chaotic dynamics,
they are good only for pulling out stable periodic motions. Chaotic
dynamics can only be untangled in the \statesp, and it's `Fourier'
transform are the trace formulas of the periodic orbit theory. That's
extra price: requires thinking, and the way the world is moving now
(there is a moronic Op-Ed in New York Times arguing that algebra should
be eliminated from American K12, replaced by `Citizen Statistics'), we
might never get there.

Most of the stuff physicists write about nonlinear problems is
thoughtless crap, the prime exhibit being `Lyapunov exponents'. They are
not well defined (unless we are talking about `covariant Lyapunov
vectors'), and they are uncomputable, as a generic dynamical system is
not nicely `ergodic', `axiom A' system. We never know whether a physical
system of interest is a `strange attractor' or a transient. With
probability 1 one it undecidable either one or the other. There is a very
good book on internet that politely tries to make the point, and explains
that the

``
Chaotic dynamics is
generated by the interplay of locally unstable motions, and the interweaving of
their global stable and unstable manifolds. These features are robust and accessible
in systems as noisy as slices of rat brains. Poincar´e, the first to understand
deterministic chaos, already said as much (modulo rat brains). Once this topology
is understood, a powerful theory yields the observable consequences of chaotic
dynamics, [...]''
...
powerful is Poincar\'e's vision of chaos as the interplay of local instability (unstable
periodic orbits) and global mixing (intertwining of their stable and unstable
...
spread over the extent of the entire asymptotically accessible state space. Once
this is grasped, the focus of theory shifts from attempting to predict individual
trajectories (which is impossible) to a description of the geometry of the space
of possible outcomes, and evaluation of averages over this space.
''

``
the {\nws} of
$f$ is the key to understanding the long-time behavior of a dynamical system; all
calculations undertaken here will be carried out on \nws s.
''

So, I feel that what one needs to do is to (1) identify the simplest
invariant solutions, (2) cut out the stable ones, (2) see whether stable
/ unstable manifolds of the unstable ones intersect and fold. That should
reveal Smale horseshoes embedded in the {\nws}, regardless of whether the
{\nws} is a `strange attractor' or a `strange repeller'.

Now, we nevertheless often just run finite time Lyapunov exponent
estimates as function of parameters, as that requires no thinking. As
Evangelos points out the problem is large parameter space - so I think we
first have to show that there are at least two unstable invariant
structures whose unstable manifolds communicate before we start fishing
for a `strange attractor'.

\item[2012-08-01 Daniel] I realize that perhaps FTLE's and power spectra
and not the smartest tools, but perhaps we can use them to have the computer
try a bunch of parameter combinations and come up with some interesting
candidates that we can then look at, no?

\item[2012-07-31 Evangelos] Added $e_1$ in \refeq{eq:DangSO2}. However in
invariant polynomials \refeq{PKinvEqs} only the difference $2e_1 - e_2$
appears, so it seems we don't get anything new by introducing it. I keep
it for the time being just to see how it affects dynamics in original
space and the projection from original to reduced space.

\item[2012-08-01 Predrag] We have done this somewhere in a more general
setting, I forget where. You can break the $\Un{1}$ symmetry only once,
so these imaginary parts of parameters always show up summed up, I
believe.

\item[2012-07-31 Evangelos]
Compute the {\stabmat} from \refeq{PKinvEqs1syz}:
\footnotesize
\beq
\Mvar  \, =
\left( \begin{array}{lll}
	2 \mu_1 + 4 a_1 u + 2 b_1 v & 2 b_1 u & c_1  \\
	2 a_2 v & 2 \mu_2+2 a_2 u+4 b_2 v & c_2  \\
	4 c_2 u+4 c_1 v+(2 a_1+a_2) w+\frac{4 (2 e_1-e_2) u v}{\sqrt{4 u^2 v-w^2}}
            & 4 c_1 u+(2 b_1+b_2) w+\frac{2 (2 e_1-e_2) u^2}{\sqrt{4 u^2 v-w^2}}
            & 2 \mu_1+\mu_2+(2 a_1+a_2) u+(2 b_1+b_2) v-\frac{(2 e_1-e_2) w}{\sqrt{4 u^2 v-w^2}}
      \end{array} \right)
\,.
\ee{2modeStabMatrixInvSyz}
% \end{widetext}
\normalsize
...
\beq
  \tr \Mvar =
  4 \mu_1+3 \mu_2+3 (2 a_1+a_2) u+(4 b_1+5 b_2) v+\frac{(-2 e_1+e_2) w}{\sqrt{4 u^2 v-w^2}}
\eeq
This is interesting: we now have a dependence on $w$ and on the symmetry
breaking parameters $e_1$ and $e_2$. I'll have to compare
\refeq{2modeStabMatrixInvSyz} with \refeq{2modeStabMatrixInv}
numerically, see which one correctly encapsulates dynamics
[\refeq{2modeStabMatrixInvSyz} should be the correct one since it takes
into account the functional dependence of all variables before taking
derivatives].

% For a \reqv\ we have $w = -2 u A_1(u, v) /c_1 = -2u (m_1 + a_1 u + b_1 v)/c_1$


\item[2012-08-02 Predrag]
In a way of motivation for a need for multiple charts, I have added
\reffig{f:antlong} from the
small-cell \KS\ system studied in \refref{lanCvit07}.

\item[2012-08-02 Predrag]
A wild idea that requires no thinking, just running simulations.
Ashley and Sebastian have now independently shown that replacing
Fourier space Euclidean norm $\delta_{jk}$ by the Sobolev $H^{-1}$ norm
$\delta_{jk}/k^2$ flattens out the group orbits and decreases the number of
linear charts needed to cover the ergodic attractor
(see \reffigs{fig:3610switching}{ChartBorder2}).

 \begin{figure}
 \begin{center}
(a) \includegraphics[width=0.90\textwidth]{3610switching}
 \\
(b) \includegraphics[width=0.90\textwidth]{3611switching}
 \end{center}
...
(a) [2012-07-19 Sebastian] Using the Euclidean norm.
(b) [2012-07-27 Sebastian] Using the Sobolev $H^{-1}$ norm. %  \refeq{braket}.
}\label{ChartBorder2}
\end{figure}


How about going the opposite way and using the Sobolev $H^{1}$ norm
$\delta_{jk}\,k^2$ for the \twomode\ flow, in order to make the
attractor more curved rather than less? Then you start with a chaotic looking simulation,
then pick a state $\slicep{}^{(1)} = \ssp(\zeit_1)$ of you liking to construct
the first slice hyperplane. Monitor the
$\cos$ of the angle between the two tangent vectors:
\beq
\cos \psi = \frac{
            \braket{\groupTan(\sspRed(\zeit))}{\sliceTan{}}
                    }{
            \Norm{\groupTan(\sspRed(\zeit))}\,\,{\Norm{\sliceTan{}}}
                    }
\,.
\label{chartBordAng}
\eeq
When that hits zero, you have hit the \chartBord. Pick that point as
$\slicep{}^{(2)} = \sspRed(\zeit_2)$, backtrack to the ridge between the
two \slice\ hyperplanes, restart there by moving in the second slice.
If we are lucky, we shall need (a) at least two charts, (b) no more than
two charts.

Take it for a spin!

\item[2012-08-02 Daniel (in ignorance)] So I guess the Fourier space
Euclidian norm is just the sum of the squares of the Fourier
coefficients, right? Then this $H^{1}$ norm weights the short-wavelength
components. I think I can visualize this for a 1D scalar function and for
a 2D scalar field. But what does this do for a 5D vector? I can't really
see it. Also, how is $H^{1}$ defined in 5D full \statesp? I guess the $k^2$
(by the way, the $k$ in $k^2$ is a different $k$ than the one in
$\delta_{jk}$, right?) corresponds to a second spatial derivative of some
sort. Once I understand this, I'll code it up. I believe that
\href{http://math.stackexchange.com/questions/4533/sobolev-space-norm-and-beppo-levi-space-norm}{this}
forum post has the relevant information, but I don't completely
understand what the summations and integrals are over.

\item[2012-08-02 Predrag]
Dunno what is 5D vector? If you are referring to \twomode\ \refeq{2mode4D},
it is 4D. $k$ above is the wave number, \ie, in the representation \refeq{LGTwoMode1}
the Sobolev $H^{1}$ norm
$\delta_{jk}\,k^2$ metric tensor is
    \beq
g_{ij}  \, =
\left( \begin{array}{cccc}
         1 & 0 & 0 & 0 \\
...
\beq
\braket{\ssp}{y}
   = x_1 x_1' + x_2 x_2' + 4(y_1 y_1' + y_2 y_2')
\,.
\ee{braket2mod}
The above \texttt{math.stackexchange.com} is not helpful.
\HREF{http://www.math.wisc.edu/~jeanluc/preprints.html} {Jean-Luc
Thiffeault}\rf{Thiffeault11} reviews Sobolev norms in Sect.~3 of
\arXiv{1105.1101}.

\item[2012-08-03 Daniel]
\HREF{http://www.ethansenglishcafe.com/wp-content/uploads/2011/09/homer_simpson_doh_02.gif}{Doh!}
Yes, by 5D I meant \twomode\, got my dimensionality confused with complex
Lorenz. Also forgot that $z_1$ and $z_2$ are actually $k=1,2$ spatial
modes, so the definition of the Sobolev norm is sort of obvious. My bad!
That being said, when we calculate the angle between tangent vector, do
we use Sobolev norm everywhere in our calculations of the dynamics in the
slice (for example, in calculating $\dot{\phi}$ in the equations of motion
constrained to a slice hyperplane) or just to check the cosine of the
angle between $\groupTan(\sspRed(\zeit))$ and $\sliceTan{}$?

\item[2012-08-03 Predrag]
Group tangent $\groupTan(\sspRed(\zeit))$ is always computed on the
\reducedsp\ trajectory $\sspRed(\zeit)$ in the slice hyperplane fixed by
$\sliceTan{}$ (not the full \statesp\ trajectory), and the angle
\refeq{chartBordAng} is computed using the Sobolev $H^{1}$ norm
\refeq{braket2mod}. You just run an ergodic trajectory, keep plotting
$\cos \psi$, stop whenever it gets small (let's say $\approx 0.1$) but
before it hits the \chartBord. If that happens, we are in the 2-chart
business. Might even happen for Euclidean norm, but I think Sobolev
$H^{1}$ that emphasizes the $k=2$ mode is more likely to kinkier group
orbits. This can be automatized, so you can start different runs for
parameter values that seem to give strange attractors.

\HREF{http://loridyan.com/wp-content/uploads/2011/01/einstein.jpg}{Do'h...}

\item[2012-08-06 Edgar Knobloch]
I do not like so much to work in the invariant coordinates, as in them
much information about symmetries is lost.

In trying to understand chaotic regions, I too would start with the
simplest things, \eqva, \reqva\ and their stabilities.

There is theorem due to Pascal Chossat\rf{Choss01,MePrKn02} that in
systems of hydrodynamic type ($2D$ Navier-Stokes) close to this
codimension 2 bifurcation $c_1c_2<0$ (download the article from
\HREF{http://online.watsci.org/volumes2001.html}{here}). This is also the
case for which one gets structurally stable heteroclinic cycles if
symmetry is \On{2}\ (\ie, when $e_1=e_2=0$).

Caroline Nore, Laurette Tuckerman \etal\rf{NoTaDauTu04} have a nice
derivation of \refeq{eq:DangSO2}, for the French washing machine.
Different $e_j$ if different angular velocities for the top and the
bottom. That only the difference $2e_1 - e_2$ appears is obvious from the
form of polar coordinates \refeq{Dang86(1.2)polar}.

\item[2012-08-06 Predrag to Evangelos] Can you again remove all $e_1$,
explain why only the difference $2e_1 - e_2$ appears in the text proper?
ES: It's in my todo list.

\item[2012-08-06 Edgar Knobloch]
There is a generic method for computing how many \reqva\ there are for
polynomial equations such as \refeq{PKinvEqs5a}. For several examples see
Callahan and Knobloch\rf{CaKno97,CaKno99}.

The general $\Mvar(\ssp)$ in \refeq{2modeStabMatrix} can only be
block-diagonalized into $[3\!\times\!3]\bigoplus[1\!\times\!1]$, the
1\dmn\ subspace eigenvector pointing in the marginal, rotation direction.

\item[2012-08-06 Predrag]
Presumably that is what the infinitesimal
equivariance condition \refeq{inftmInv} means - would be nice to make it
explicit. It would be useful for computing stability of \reqva, less so
for the \rpo s, I think.

\item[2012-08-07 Keith] playing catch up and reading articles on fitting curves
(if I find anything good or easy, Ill let you know) in space to try to tackle the
return map in an easier way.

\item[2012-08-08 Daniel to Keith] Do you have a map already? If so, could you post it?
I think it should be easy to fit with splines or cubic interpolants, both of which
are built-in Matlab (and probably Mathematica) functions.

\item[2012-08-08 Keith to Daniel] Not yet, I am more working on this new
method for a few days.  I have created return maps for other systems, but
these relied on using a summed up distance along the curvilinear manifold
and required a step that would order the points in space in a meaningful
way.  Worst comes to it, we could always use this, but I am trying to use
a method I found for finding curvilinear distances along the unstable
manifold with parametric equations for \emph{unordered points} in space.
