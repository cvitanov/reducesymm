...
\item[2012-04-29 Daniel] Here's a couple of other results that may be useful
for constraining paramter choices or looking for \reqva.
Setting the equation for $\dot{\psi}$ in the
polar form of the \twoMode\ system to zero or starting from the equation for $q$ in eqs. \refeq{PKinvEqs4} and the fact the fact that $q =  2\,u |v|^{1/2} \sin \psi$, one can solve for
...
condition is an additional constraint on what $r_1$ and $r_2$ (or $u$ and $v$) can be.
Might prove useful if we have to brute force search for roots.

If one starts with the $A_1$ equation for $w$ in \refeq{PKinvEqs4} and $w =  2\,u |v|^{1/2} \cos \psi$ and uses the constraint that $\cos{\psi} \leq 1$, you can work out that
\beq
\dfrac{a_1}{b_1}r_1^2\,+\,\left(r_2\,+\,\dfrac{c_1}{2 b_1}\right)^2\,\geq\,\dfrac{c_1^2\,-\,4 \mu_1 b_1}{4 b_1^2}
\ee{2ModeCosCond1}

This result and the conditions from the range of $\sin$ \refeq{2modeSinCond} are nice but not great because they tell us that the $r$'s must lie \emph{outside} of particular ellipses (i.e., where the root are not, but not where the roots \emph{are}). However, it
seems (unless I messed up the algebra, so {\color{red} somebody please double check this and report back here}) that the final condition $\cos{\psi} \geq -1$ gives the more powerful constraint
\beq
\dfrac{a_1}{b_1}r_1^2\,+\,\left(r_2\,-\,\dfrac{c_1}{2 b_1}\right)^2\,\leq\,\dfrac{c_1^2\,-\,4 \mu_1 b_1}{4 b_1^2}
\ee{2ModeCosCond2}
This constraint tells us that valid $r$'s need to be \emph{inside} of a certain ellipse, which really cuts down on the available \statesp! Furthermore, the lefthand side of the inequality is non-negative, so the righthand side must also be non-negative.
