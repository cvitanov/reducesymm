...
Substitute $\ssp=\LieEl\sspRed$.
    ...
    example, if our man Luis in matters spiral slices the Euclidean
    $E(2)$ symmetry}
...
\item[2012-05-01 Lei] Discussed with Bryce and trying to finish our term paper by summarize all we have about PK system. I'm really confused about the LaTeX templates used in the paper, so I wrote in my own templates and put it in cgang/2-mode-Bryce-Lei. Other sections just copied from the previous paper, rewrite a little using my own words. Just begins, keep moving.

% \item[2012-05-01 Lei to Predrag] Still can't login using cnsuser cnsweb...

\item[2012-05-01 Daniel] Following up on Predrag's cry in the wilderness of 2012-04-29, we need the trace of the stability matrix,
\(
tr(A) = \sum \lambda_e > - \sum \lambda_c
\,,\)
to be greater than zero. For future reference, the trace of the stability matrix for a generic point is given by
\begin{equation}
tr(A) = 2(\mu_1\,+\,\mu_2)\,+\,(4 a_1\,+\,2 a_2)\,r_1^2\,+\,(2 b_1\,+\,4 b_2)\,r_2^2
\label{2ModeStabTrace}
\end{equation}
This agrees with my earlier result of 2012-04-27 for the origin, but may allow us to put further constraints on the parameter choices, especially since $u,\,v \geq 0$.
...
following stability matrix for the \twoMode\ system in invariant variables:
% \begin{widetext}
\footnotesize
\beq
A  \, =
...
\beq\label{2ModeStabTraceES}
  \tr(A)=6\mu_1+4\mu_2+4(2a_1+a_2)u+6(b_1+b_2)v\,,
\eeq
not in agreement with \refeq{2ModeStabTrace}. Daniel, are you using a different notation? Or am I doing something wrong?
Once cross-checked/corrected I'll move \refeq{2modeStabMatrixInv} and \refeq{2ModeStabTraceES} to the main text.

\item[2012-07-30 Daniel to Evangelos] Hmmm... To get \refeq{2ModeStabTrace}, I started from \refeq{eq:DangSO2}, plugged in $z_1 = x_1 + i x_2$ and $z_2 = y_1 + i y_2$, to get \refeq{2mode4D}, and then calculated the jacobian \refeq{2modeStabMatrix} and its trace. This was done using \texttt{/cgang/Daniel/Matlab/EOMComplexTo4.m}. I then, replaced $x_1^2 + x_2^2$ by $u$ and $y_1^2 + y_2^2$ by $v$ to get \refeq{2ModeStabTrace}.

If I start from \refeq{eq:DangSO2}, go to the $[u, v, w, q]$ basis to get \refeq{PKinvEqs1}, then take the jacobian and trace, I get the same result \refeq{2modeStabMatrixInv} and \refeq{2ModeStabTraceES} that you do. This was calculated using \texttt{/cgang/Daniel/Matlab/PKComplexToInvariant.m}

I guess the traces of the stability matrices for the \twoMode system written in the $[x_1, x_2, y_1, y_2]$ basis should be expected to be different from the stability matrix for the system written in the $[u, v, w, q]$ basis, right? 

\item[2012-07-31 Evangelos to Daniel] I think that $A$ (and it's trace) have to be computed in invariant coordinates of some form. For instance, a relative equilibrium is a periodic orbit in original space and
if we just pick a point on it and compute $A$, it tells as nothing about stability, right? One should go to a reduced space where the relative equilibrium is an equilibrium and compute its stability. Since
the dimensions of the original and reduced spaces are not the same (note there is a syzygy), we don't have the nice property of invariance of stability eigenvalues under smooth conjugacies.

What confuses me still, is whether one should use the syzygy explicitly. Should we first eliminate one variable using the syzygy, then compute the Jacobian as a 3 by 3 matrix? Maybe I'll try this here.

\item[2012-07-30 Daniel to Evangelos and Predrag] What would be the features of a ``good'' set of parameters? Obviously we need the term that breaks the reflection symmetry, but other than that? 
What's our metric for chaos? Do we want to be calculating Lyapunov exponents or frequency spectra of time series and looking for broadband noise?

\item[2012-07-31 Evangelos] Predrag could you answer this? I've added a second $e_1$ term in hopes of getting more interesting behaviour but have not played with it nor rederived relative equilibria conditions yet.
The problem with any obvious way to detect chaos (such as Lyapunov exponents) is that it does not provide a feeling on whether there are qualitatively different regions in phase space, which we could pick as our 
templates. Maybe the right thing to do is to scan the parameter space plotting the largest Lyapunov exponent and at the end looking in details cases which seem promising. 
The problem is the large number of parameters involved.

\item[2012-07-31 Evangelos] Added $e_1$ in \refeq{eq:DangSO2}. However in invariant polynomials \refeq{PKinvEqs}
only the difference $2e_1 - e_2$ appears, so it seems we don't get anything new by introducing it. 
I keep it for the time being just to see how it affects dynamics in original space and the projection from
original to reduced space.

\item[2012-07-31 Evangelos] Using the syzygy $w^2+q^2-4u^2v=0$ we can eliminate q from \refeq{PKinvEqs} to
get 
...
\label{PKinvEqs1syz}
\eea
Now compute the stability matrix from \refeq{PKinvEqs1syz}:
\footnotesize
\beq
A  \, =
...
The trace reads
\beq
  \tr(A)=4 \mu_1+3 \mu_2+3 (2 a_1+a_2) u+(4 b_1+5 b_2) v+\frac{(-2 e_1+e_2) w}{\sqrt{4 u^2 v-w^2}}
\eeq

This is interesting: we now have a dependence on $w$ and on the symmetry breaking parameters $e_1$ and $e_2$.
I'll have to compare \refeq{2modeStabMatrixInvSyz} 
with \refeq{2modeStabMatrixInv} numerically, see which one correctly encapsulates dynamics
[\refeq{2modeStabMatrixInvSyz} should be the correct one since it takes into account the functional
dependence of all variables before taking derivatives].

% For a relative equilibrium we have $w = -2 u A_1(u, v) /c_1 = -2u (m_1 + a_1 u + b_1 v)/c_1$
