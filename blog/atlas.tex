...
\item[{[x]}] {\bf [2012-06-13 Predrag]} resolved. Resolve Evangelos'
...
\item[{[ ]}] \reffig{fig:BeThMovFr}\,(d) is really drawn backwards - one
moves the \template\ to agree with the text and  \refeq{PCsectQ0}.
...
\item[{[ ]}] Every author needs to read the paper and ok it.
    ...
    it carefully!}
		\begin{description}
			...
			\item[{[ ]}] Evangelos
			\item[{[ ]}] Daniel
			\item[{[x]}] 2012-06-14 Predrag: you gangsters have a first go at these
...
paragraph from poetry into prose. What say Yee, the poetry-sceptics?
  ...
  \item[{[x]}] all ``stratify' $\to$ `foliate'.
\end{itemize}

\item[2012-06-15 Evangelos] I've tried two-chart atlas for CLE using Daniel's
...
\item[2012-06-15 Daniel] I HAD noticed that any single one of my templates makes a decent single chart atlas. I'm pretty sure I pointed this out. I think using both just \emph{slightly} decreases the maximum value of \phaseVel , relative to when you just use one of them. 

As we have discussed before, it seems like a big problem with the \cLe\ is that the dynamics bring you near the z-axis, which is an invariant. You can't get onto it, so the group tangent never formally vanishes, nor does the dot product with the $t'$ switch signs; it just gets small and makes \phaseVel\ get large but not diverge. I think my templates may be successful because they happen to keep you as far away from the z-axis.

I agree that the two template tangents are close to parallel (although I guess in general they just have to have a positive dot-product) because the second template is rotated into the slice hyperplane of the first template. At least, this is going to get them as parallel as possible.

I guess I could try the same numerical experiment and just force it to use $\REQV{}{1}$ as a template and see what it comes up with for a second one. Or we could try to work with what Stephan constructed to force a border crossing. What do you think? 
