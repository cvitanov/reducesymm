...
% Look for \texttt{\textbackslash
% DBedit}'s and \texttt{\textbackslash DB}'s to see where I fiddled with
% the paper.
...
Somebody still has to sign off on {\bf [2012-06-15 Keith]} edits.
...
in (c) was a circle (pointing out that this is the case for SO(2)). Added some 
...
\item[2012-06-26 Daniel to Predrag or Evangelos] There was one little piece of my caption edit for Fig. 1 that I wasn't 100\% sure about and Predrag didn't okay. For \reffig{fig:A27wurst}\,(a), we have

``In the presence of an N-continuous parameter
symmetry, each state space point x owns (N+1) tangent vectors...
 tN(x) along infinitesimal symmetry shifts, tangent to the \DBedit{N-DIMENSIONAL} group orbit.''

I added ``N-dimensional'' to stress to readers that for symmetries other than SO(2), the group orbit is a more complicated manifold than the circles that we use everywhere. If you have N rotational symmetries, or a rotation with N parameters, I'm pretty sure that the group orbit HAS to be N dimensional. I'm guessing this is the case for other types of continuous symmetries, but I'm not 100\% sure. I just wanted to make sure that this addition isn't an incorrect statement in the more general case.
