...
in our {\twoMode} system (eqs. \refeq{eq:DangSO2}) are related to the paramters
...
For future reference, our {\twoMode} system \refeq{eq:AGpolar} is a special case ($\omega_1 = 0$) of the system of Porter and Knobloch\rf{PoKno05}. Our parameters are related to their parameters by (P\textit{\&}K $\leftrightarrow$ ChaosGang {\twoMode}):
...
\item[2012-04-27 Evangelos] If we let $\omega_1\neq0$ we might generate some interesting dynamics
(by adding an extra frequency we might gain an extra relative equilibrium).
 
\item[2012-04-26 Daniel to Chaos Gang] Sorry to keep raining on the party, but I believe eqs. \refeq{eq:AGpolarREQV} have another typo in them. If you take eqs. \refeq{eq:AGpolar} and set them to zero, you end up with a $b_1 r_2^2$ term in the equation for $\dot{r}_1$ rather than $b_1 r_1 r_2$ term. This is confirmed by my \texttt{EOMComplexTo4D.m} code using eqs. \refeq{eq:DangSO2} as a starting point. If you are working on the {\twoMode} equilibria, you probably want to stop and double check this before moving on but I'm pretty sure I'm right. {\bf to Lei} Did you derive eqs. \refeq{2modeRequa1} from eqs. \refeq{eq:AGpolar} or from eqs. \refeq{eq:AGpolarREQV}? If it was the latter, you probably want to double check it.
