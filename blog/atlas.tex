...
\item[2012-05-08 Daniel to Gang] Ok, now that the semester is over and everybody has finished their projects for Predrag's class, it is time to get the paper on \texttt{arxiv.org}. Here is a list of things that still need to be done. Check things off as you do them.
...
\item[{[ ]}] Include the template points (as shady as they are) for the 2-chart atlas of the \cLe\ in the text.

\item[{[ ]}] Mention that \cycle{01} is drawn in the sliced figures for \cLf .

\item[{[ ]}] Flesh out description of what is actually shown for the 2-chart of \cLf . Stress that colored blocks and planes are cartoon drawings of high dimensional object projected onto 3D.
...
\item[{[ ]}] Resolve Evangelos' comment about the statement in the paper about how sections and slices are not the same. I think that what we want to say is sort of outlined in \reffig{fig:slicePoincDiagr}. If this is a muddy point for us, imagine what the poor readers will think. See blog circa 2012-04-24 for details.

\item[{[ ]}] Perhaps come up with new names for the following:
	\begin{description}
	\item[wurst] You know the \HREF{http://www.youtube.com/watch?v=-VRBWLpYCPY}{3rd referee} is going to bitch about this.
...
\item[{[ ]}] Every author needs to read the paper and ok it. \DB{}{This should've been done before we submitted to Chaos, but at least we can change that before it gets published. Once it goes on arxiv, it is effectively published and has your name on it, so read it carefully!}
		\begin{description}
			\item[{[ ]}] Predrag
			\item[{[ ]}] Daniel
			\item[{[ ]}] Keith
