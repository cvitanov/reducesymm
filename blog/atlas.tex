...
Porter and Knobloch system, see {\bf [2012-04-21 Evangelos]}. Until some
volunteer in the gang establishes the connection to Porter and Knobloch
...
\item[2012-04-25 Predrag to 2-mode gang] continuing on the discussion
...
\item[2012-04-25 Evangelos] Yet another snapshot of 2-mode (Dangelmayr)
...
Just promise once you have e permanent job you'll snap out of it.
...
insights), and am now hopeful for this 2-mode, it would prove a nice
...
\item[2012-04-26 Lei to Keith] Yes. Zero should be a equilibrium. I just
...
\label{2modeRequa2}
\eea
This is good news, as only $u_1=r_1^2$, $u_2=r_2^2$ appear, so there is no
$\{\pm r_1, \pm r_2\}$ problem - one will just take '+' solutions:
\bea
 \frac{(\mu_1 +a_1 u_1+b_1 u_2)^2}{c_1^2 u_2}
+\frac{u_2 e^2}{(c_1 u_1+2 c_1 u_2)^2}  &=& 1
    \continue
 \frac{u_2 \, (\mu_2 +a_1 u_1 +b_2 u_2)^2}{c_1^2 u_1^2}
+\frac{u_2 e^2}{(c_2 u_1+2 c_1 u_2)^2}  &=& 1
\label{2modeRequa3}
\eea
At this point I stop: please confirm there is no type in the
second terms' denominators?

\item[2012-04-26 Keith to Lei and Predrag]  Yes, when I checked this out, I acutally have:
    \bea
 \frac{(\mu_1 +a_1 u_1+b_1 u_2)^2}{c_1^2 u_2}
+\frac{u_2 e^2}{(c_2 u_1+2 c_1 u_2)^2}  &=& 1
    \continue
 \frac{u_2 \, (\mu_2 +a_2 u_1 +b_2 u_2)^2}{c_2^2 u_1^2}
+\frac{u_2 e^2}{(c_2 u_1+2 c_1 u_2)^2}  &=& 1
\label{2modefixdenom}
\eea

in other words, the denominator on the $e^2 u_2$ terms in both equations should
be the same since this comes from the $cos(\psi)$ term.  Also check your
denominators on the other, I have $c_2$ instead of $c_1$ and a couple
of the other coefficients were different. I haven't checked if the original
reduces down this in polar coordinates, but maybe I should tonight.


\item[2012-04-26 Daniel] Probably old hat at this point but I'm writing the equations for the 2-mode system in case they ever come in handy. Here's Siminos's Eqs. \refeq{eq:DangSO2} in terms of 4 real variables, such that $z_1 = x_1 + i y_1$ and $z_2 = x_2 + i y_2$. I did the algebra using Matlab's symbolic math toolbox, so these should be correct.
\bea
...
in our 2-mode system (eqs. \refeq{eq:DangSO2}) are related to the paramters
in Porter and Knobloch's $SO(2)$-symmetric equations and found that they do not
...
\item[2012-04-26 Predrag to Evangelos] Zotero says ``The attached file could not be located ...''

\item[2012-04-27 Evangelos] Maybe we run out of space. I'll check tommorow, meanwhile I'll email it to you.

\item[2012-04-26 Daniel] Uploaded code to \texttt{/siminos/cgang/Daniel/Matlab/}. Just type \texttt{EOMComplexTo4D} at the Matlab prompt and it will calculate the system in 4D Cartesian, 4D polar, and 3D polar coordinates. The $\dot{z_j}$ equations can be modified within the code if need be.
For future reference, our 2-mode system \refeq{eq:AGpolar} is a special case ($\omega_1 = 0$) of the system of Porter and Knobloch\rf{PoKno05}. Our parameters are related to their parameters by (P\textit{\&}K $\leftrightarrow$ ChaosGang 2-mode):

 $\mu_1 \leftrightarrow \mu_1,\, \epsilon \omega_1 \leftrightarrow 0,\, \tilde{\alpha} \leftrightarrow c_1,\, \tilde{d}_{11} \leftrightarrow a_1,\, \tilde{d}_{12} \leftrightarrow b_1,\, \mu_2 \leftrightarrow \mu_2,\, \epsilon \omega_2 \leftrightarrow -e_2,\, \tilde{\beta} \leftrightarrow c_2,\, \tilde{d}_{21} \leftrightarrow a_2,$ and  $\tilde{d}_{22} \leftrightarrow b_2$.

\item[2012-04-27 Evangelos] Great, it wasn't so terrible after all. So now we can call this system Porter and Knobloch flow.
 
\item[2012-04-26 Daniel to Chaos Gang] Sorry to keep raining on the party, but I believe eqs. \refeq{eq:AGpolarREQV} have another typo in them. If you take eqs. \refeq{eq:AGpolar} and set them to zero, you end up with a $b_1 r_2^2$ term in the equation for $\dot{r}_1$ rather than $b_1 r_1 r_2$ term. This is confirmed by my \texttt{EOMComplexTo4D.m} code using eqs. \refeq{eq:DangSO2} as a starting point. If you are working on the 2-mode equilibria, you probably want to stop and double check this before moving on but I'm pretty sure I'm right. {\bf to Lei} Did you derive eqs. \refeq{2modeRequa1} from eqs. \refeq{eq:AGpolar} or from eqs. \refeq{eq:AGpolarREQV}? If it was the latter, you probably want to double check it.
