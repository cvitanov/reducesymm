...
Kerswell\rf{ChaKer12} cites both good and nonsensical references on `Periodic
Orbit Theory', so I have reread Zoldi and Kazantsev. It's like having to
...
% `Theory' is superior.
I had assumed that with time erroneous formulas would disappear, but
we have to address this in print, because they keep getting
...
is perhaps even harder. While the eigenfunctions of quantum evolution
...
eigenstates can be computed by computers, but the
deterministic eigenstates of high-dimensional dynamical systems can be
computed the same methods, at least without much further thought. Were ergodic
...
$p$, and the sum goes over all known unstable \po s, and sometimes also over
...
terms of the observable averaged over each and every prime \po\ $p$
...
unstable \po s, all one has to do is put $\Lyap_{p,j}$ and $\obser_p$
...
cycles, (6) The guess for the weight $\tau_p$ is clearly wrong, as any periodic
...
entirety. The modification does not help: the exact weight of the
unstable periodic orbit $p$
\beq
\frac{\period{p}}{\oneMinJ{r} } \,
e^{r\period{p}(\beta \cdot \obser_p -\eigenvL)}
\,,
\ee{weightPO-cont}
(as it shows in the {classical trace formula for flows}, for
% \refeq{tr-L-cont}
$r$th repeat of a prime periodic orbit $p$, the {\evOper} eigenvalue
$\eigenvL$ and auxiliary variable $\beta$ ) is determined by its
dimensionless Floquet \emph{multipliers} (eigenvalues of $\oneMinJ{}$)
which grow/shrink exponentially with cycle period, \emph{not} the
Oseledec Lyapunov exponents or \po\ Floquet exponents which measure
average expansion/contraction \emph{rate per unit time}.
...
\noindent
%Predrag 2009-12-01
We (in a collaboration with Aurell, Artuso and Gunaratne) derived `cycle expansions' in 1986-87
(earliest reference to `cycle expansion' in my notes is from 29 October 1987).
