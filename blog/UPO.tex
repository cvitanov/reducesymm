...
\item[2013-04-29 Gritsun]
Thank you for your comment on the "escape-time weighting" formula. Of
course you are right that there is no theory behind it. Probably I should
be more specific on that in my paper.

In fact I do not understand why "escape-time weighting" is comparable in
accuracy with the correct weighting formula in my case. Probably the
model in my study is far from hyperbolic system and both formulas are
wrong in this case. Or too many prime orbits were missed by my numerical
procedure.  Actually I plan to do more systematic study to resolve this
issue.

\item[2013-04-29  Predrag to Gritsun]
I agree on all points. But if you do not want other people to waste time
on a wrong formula (the most recent problem is Kerswell who insisted on
including it in his papers), either do not refer to it, or refer to it as
wrong.
...
\item[2013-05-07 Predrag]                           \toCB
This week's papal edict: Procaccia invented cycle expansions.
In \emph{The notion of
information in quantum physics and the dual ontology of nature}
\HREF{http://www.stoqatpul.org/lat/materials/basti_qft_dual_ontology.pdf}
{Gianfranco Basti} of Pontificia Universit\`a Lateranense writes
``Generally, the classical method of chaotic dynamics mathematical
characterization suggested by I. Procaccia et al. (Auerbach,
Cvitanovi\'c, Eckmann, Gunaratne, \&\ Procaccia, 1987) consists in
approximating it through cycles ever more complex (i.e., with ever
higher period). In this way, the characteristic quantities of the
dynamics are calculated by expanding them on the values estimated on
these long cycles. What is critical is thus the cycle extraction.
Procaccia and his collaborators originally suggested to search
combinatorially for the recurrences within a given string of the
dynamics values.''

\item[2013-05-08 Carl Dettmann] Makes as much sense as Alan Sokal,
though.
...
gives for some other examples. The treatment of these formulas  in
the attached paper is very brief, but Predrag's webbook
\wwwcb{} has all the
details. It is not difficult to adapt the formulas to compute the
average of $z(t)$.
...
shorter than the shortest recurrent time of the flow, it \emph{cannot} probe
enough of the \statesp\ to give a reliable expectation value for an
observable.
...
for a function of the state $J(x)$,
the corresponding statistical quantity $\langle J\rangle$ is defined
as an average of $J(x(t))$ over an infinitely long time interval:''
\begin{equation} \label{Wang13:stats}
\langle J\rangle = \lim_{T\rightarrow\infty}
\frac1T \int_0^T J(x(t))\,dt\;,
\end{equation}
\\
{\bf 2013-06-22 Predrag} cool, the same as ChaosBook 'time average of
the observable' eq.~(17.4). (All equation references are to
...
flows.''
\\
{\bf 2013-06-22 Predrag} I am no fan of Ruelle\rf{Ruelle96,Ruelle09}
Linear Response Theory; as chaotic sets are in generally structurally
unstable, the averages computed on them are in general not
differentiable with respect to system parameter variations.
...
vectors\rf{ginelli-2007-99,WoSa07}.''
\\
