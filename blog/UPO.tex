...
\renewcommand{\ssp}{x}             % finte-dimensional state space point
...
averages. We cite Zoldi and Greenside\rf{ZG96} for being the second paper
...
orbits. We love them, but not because of their \po\ averaging formulas.
...
it.
% Ergodic theory is no less of a theory than quantum mechanics, but it is
% perhaps even harder.
While the eigenfunctions of quantum evolution operators are smooth
Hilbert space states, the eigenfunctions of deterministic evolution
operators are highly  singular, nowhere differentiable functions with
support on fractal sets. The deterministic eigenstates of
high-dimensional ergodic flows thus cannot be computed using the methods
developed for quantum eigenstates, at least without much further thought.
The ergodic, singular `natural measure' is harder (and in high\dmn\
\statesp\ impossible) to construct numerically than its smooth quantum
cousin, the `ground state'. Were ergodic theory easy, ChaosBook.org and
Gaspard monograph\rf{PG97} would have been a much breezier reads.

In the vast and vastly uneven \po\ literature (should erroneous papers be
cited?) one sometimes encounters the `escape-time weighting',
% clips from dasbuch/book/notes/zoldi/zoldiLett.tex
\bea
\expct{\obser}  &=&  \frac{\sum_p \tau_p \obser_p}{\sum_p \tau_p}
               \,, \qquad \tau_p = \frac{1}{\sum^{+}_j \Lyap_{p,j}}
    \continue
\sum^{+}_j \Lyap_{p,j} &=&  \frac{1}{\period{p}}\ln \ExpaEig_p
                \,, \quad
\ExpaEig = |\ExpaEig_1 \ExpaEig_2 \cdots \ExpaEig_{e}|
    \continue
          &=& \Lyap_{p,1}+\Lyap_{p,2}+...+\Lyap_{p,e}
\,,
\label{ZoldiAv}
\eea
where the $p$ sum goes over all known unstable \po s, and sometimes also over
judiciously chosen subsets of unstable \eqva.
Here
$|\ExpaEig_{p,i}|>1\,,\quad i=1,\cdots,e$\, are the expanding Floquet multipliers
of the prime \po\
$p$,
$|\ExpaEig_i|$ is the magnitude of the  $i$th multiplier,
$\period{p}$ is the period of cycle $p$, and $\Lyap_{p,i}$'s are
strictly  positive Floquet exponents.

$\Lyap_p$ is the local escape rate  per unit time of a single repelling
cycle $p$.
    \PC{{\em cf.} chapter 8 (this is 1997 numbering) of ChaosBook.}
As $\sum \Lyap^+_{p}$ has
dimension of $1/$[time], one may interpret $\tau_p$ as ``an estimate of
the mean time spent by the system in vicinity of \po'' $p$.
$\tau_p=1/\Lyap_p$
is the Lyapunov time of cycle $p$, that is, the mean time
it takes for the density of neighboring trajectories in an arbitrarily small
ball centered around a point on the trajectory
to decrease by factor $1/e$.

The Lyapunov time has nothing to
do with the period of the particular cycle.
...
Formula \refeq{ZoldiAv} is then asserted to approximate the time average
$\expct{\obser}$ of observable $\obser(\ssp)$ over the chaotic attractor in
terms of the observable
% rewrite of ChaosBook \refeq{Phi_cyc}
\beq
         \obser_p   =  \frac{1}{\period{p}}
  \oint_0^{\period{p} }d\tau \, \obser[\flow{\tau}{\xInit}]
\,,
\qquad  \xInit \in p
\,,
\label{obsPhi_cyc}
\eeq
averaged over each and every prime \po\ $p$
found in a computer exploration of a dynamical system's \statesp.
...
If one has a dynamical problem, and if one has a computer one has
programmed to search for \po s, and if the computer brings back a set of
unstable \po s, all one has to do is to put $\Lyap_{p,j}$ and $\obser_p$
into the formula \refeq{ZoldiAv}, and it returns a number - let's say $D
= 9.0 \pm 0.1$, where the error one estimates somehow
% you had sitting in your fridge
- which one then publishes.
...
neighborhood of the \po\ $p$. To make this into escape time one would
...
under investigation or dwell isolated in the Moon orbit. (4) Were an ergodic orbit to
enter a neighborhood of a \po\ the time
it would linger there is presumably determined by the least
...
cycles. For that there is a fix in the literature, with
...
(6) The guess for the weight $\tau_p$ is clearly wrong, as any periodic
...
are comparable. The exact weight of the unstable periodic orbit $p$ was
essentially given by Kadanoff and Tang\rf{KT84} in 1984:
\beq
\period{p} \frac{e^{-r\period{p}\eigenvL}}{\oneMinJ{r} } \,
e^{r\period{p} \beta \obser_p}
\,,
\ee{weightPO-cont}
(as it shows in the {classical trace formula for flows}, for
% \refeq{tr-L-cont}
$r$th repeat of a prime periodic orbit $p$, the {\evOper} eigenvalue
$\eigenvL$ and an auxiliary variable $\beta$ ) is determined by its
dimensionless Floquet \emph{multipliers} (eigenvalues of the \po\
{\jacobianM} $\jMps$) which grow/shrink exponentially with cycle period,
\emph{not} the Oseledec Lyapunov exponents or \po\ Floquet exponents
which measure average expansion/contraction \emph{rate per unit time}.

For purely pedagogical reasons on sometimes looks at the leading
long-time behavior, and approximates the denominator of
\refeq{weightPO-cont} by the product of the expanding multipliers
$\ExpaEig_p = |\ExpaEig_{p,1} \ExpaEig_{p,2} \cdots \ExpaEig_{p,e}|$ of
the {\jacobianM} $\jMps$,
\beq
\frac{e^{-\period{p}\eigenvL}}{\oneMinJ{}}
    \to
t_p = \frac{e^{-\period{p}\eigenvL}}{\ExpaEig_p}
\,,
\ee{weightPO-approx}
as was first noted in Auerbach \etal\rf{pchaot}, who compute
(for discrete time dynamical system)
an $n$th order estimate $\eigenvL_{(n)}$ of the leading \FP\ eigenvalue
$\eigenvL$ by condition
\beq
1 = \sum_{i\inFix{n}} {  e^{\beta \Obser^n(x_i)} e^{-\eigenvL_{(n)} n} \over | \ExpaEig_i |}
% 1 = \sum_{i\inFix{n}} { g^n(x_i) e^{-\eigenvL_{(n)} n} \over | \ExpaEig_i |}
\,.
\ee{GOY:levl-sum}
More frequently cited is the
Grebogi, Ott and Yorke paper\rf{yorke2} about unstable periodic orbits
which focused on
the fractal dimensions of chaotic attractors, as was the fashion in the
late 1980's. They proved that for mixing hyperbolic attractors the
...
subset $\pS_S$ of the $d$\dmn\ \statesp\ $\pS$ is given by the sum over
...
%
Then there is in literature `Alternative Periodic Orbit Theory' so bold
that we call it The Heresy: if one looks carefully enough, there is a
\emph{single} \po\ that captures all dynamical averages of a turbulent
flow. This is so wrong that one is at loss what to say: there is NO such
single \po\ - instead, there is perfectly well developed theory that says
how you use \po s and how many do you need to capture the hyperbolic
parts of the {\nws} to a desired accuracy. It is as elegant and
systematic as Stat. Mech. and Quantum Field Theory. Read
\HREF{http://chaosbook.org/}{The Book}. But who reads books nowadays?

The strangest thing about `Alternative Periodic Orbit Theories' is that
since (at least) 1987 there is no need for them whatsoever. Why would on
need to \emph{guess} an approximate \po\ weight when the \emph{exact}
weight is already known? The exact spectral determinant formulas use the
same computational input as the wrong formulas: \po s hierarchically
ordered, their Floquet multipliers, periods, and observables
$\obser_p$ averaged over $p$. It costs exactly the same to
compute the exact formula as it costs to compute a wrong formula. Go
figure...
...
\item[Historical postscript 1:] As of 2012 Scott M. Zoldi was Vice President,
...
\item[Historical postscript 2:] Henry S. Greenside is a professor in
Physics Deapartment, Duke University. Web of Science shows no journal publication
...
Grebogi never cites me or Kiryat Anavim or cycle expansions. A number of
...
have been published for the first time. Ever since the Maryland school
