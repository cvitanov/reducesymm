...
not know what to make of them - like ZG, she gets all of them with a
large number unstable dimensions. Presumably none of them are close to
the asymptotic inertial manifold. More worrisome still, she uses ZG to
...
this for us, and Mainieri actually hired him at Los Alamos to learn how
this works. We are all convinced that it makes no sense whatsoever.
Regrettably Lopez used the nonsensical formulas of Zoldi\rf{Zoldi98,ZG96}
...
Laurent-Polz\rf{Laurent-Polz04} says: ``
We give a method to
...
\item[2012-08-28 Francesco]

\item[2012-08-28 Predrag]   %\toCB
...
the Schr\"odinger equation, so have their home-cooked delirious heuristic
`Theory' instead. So I have to explicitly refute them, without citing them,
in a remark for ChaosBook.org. Here are some preparatory notes (see
also {\bf [2004-09-15 Predrag]} above). Uuurgh.

Kazantsev\rf{Kazantsev98} `weighted averages' over \po s
\beq
...
Floquet exponents with positive real parts, and $\sum_p$ is the sum of
{\em all} unstable \po s, regardless whether they belong to the ergodic
component or are on the Moon) are a variant of Z \&\ G gibberish that
...
The logic seems to be this: $\sum_p
\Lyap^+_{p}$ has dimensions of $1/t$, so he interprets it as ``a priori
estimate of the mean time spent by the system in vicinity of PO.'' He
defines some arbitrary Euclidean \statesp\ distance $r=0.05$ as
`vicinity' and finds 90\% (!) correlation with episodes in which
`barotropic ocean model' ergodic trajectory approaches his \po, whose
periods range from 40 to 250 days. That cannot possibly be right - the
duration of a visit to a neighborhood surely also depends on the
contacting eigenvalues - if they are weakly negative, you longer in the
neighborhood. Would not see that for Lorenz, as contracting eigenvalues
are strongly contractive. This is not even a Kaplan-Yorke type formula,
and that is easy to understand... Go figure. Even more mysteriously he
defines a `dimension' of a \po, and his `weighted formula' is in the same
ballpark as what he gets from dimension $D=5.8$ computed over 500 years
of model integration.

\item[2012-08-28 Predrag]
