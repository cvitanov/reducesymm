...
Days of intense pain: because of Rich's paper\rf{ChaKer12} which
promiscuously cites both good and nonsensical references on `Periodic
Orbit Theory' I'll had to reread Zoldi and Kazantsev. It's like having to
read papers on alternative `Quantum Theories' by people who lack patience
to take intro QM and learn the Schr\"odinger equation. Instead, they
strenuously argue that their home-cooked delirious `Theory' is superior.
I had assumed that with time this gibberish would quietly disappear, but
we unfortunately we have to address head on, because it keeps getting
cited by reputable researchers. So we have to explicitly explain why they
are wrong, in a remark for ChaosBook.org (or in some
publication?). Here are some preparatory notes. Uuurgh.
...
ergodic theory, and periodic orbits are one aspect of it, just like there
is only one quantum mechanics, and WKB is one way to gain insight into
it. Ergodic theory is no less of a theory than quantum mechanics, but it
is arguably harder. While the eigenfunctions of quantum evolution
operators are nice smooth Hilbert space states, the eigenfunctions of
deterministic evolution operators are highly  singular, nowhere
differentiable functions with support on fractal sets. Quantum
eigenstates can be computed by computers, but there is no chance that the
deterministic eigenstates of high-dimensional dynamical systems can be
computed this way, at least without much further thought. Were ergodic
theory easy, ChaosBook.org would have been a much shorter book.
...
\,,
\ee{ZoldiAv}
where $\Lyap_{p,j}$ are the nonnegative Floquet exponents of prime \po\
$p$, and the sum goes over all known unstable \po s, and sometimes even
judiciously chosen subsets of unstable \eqva.  As $\sum \Lyap^+_{p}$ has
dimension of $1/$[time], one may interpret $\tau_p$ as ``an estimate of
the mean time spent by the system in vicinity of \po.''
...
weighting' \refeq{ZoldiAv}. That's it: the `derivation' in its entirety.
Formula \refeq{ZoldiAv} is then asserted to approximate the dynamical
average $\expct{\obser}$ of observable $\obser$ over the chaotic
attractor in terms of the same quantity~$\expct{\obser}_p$ averaged over
each and every prime \po\ $p$ found in a computer exploration of a
dynamical system's \statesp.

The enchantment with the escape-time weighting approach lies its charming
simplicity. Let's say you are a graduate student, without an adviser
versed in nonlinear dynamics. If you have a dynamical problem, and if you
have a computer you have trained to troll for \po s, and if the computer
brings back a set of unstable \po s, all you have to do is put
$\Lyap_{p,j}$ and $\obser_p$ into the formula \refeq{ZoldiAv}, and it
returns a number - let's say $D = 9.0 \pm 0.1$, where the error you had
sitting in your fridge - which you then publish.

The only drawback is that the `formula' is wrong. (1) It comes from
nowhere; (2) $\tau_p$ has dimension of $1/$[time], but a `weight' is
dimensionless number, likelihood that an ergodic trajectory enters the
neighborhood of the \po\ $p$. To make this into Lyapunov time one would
have to specify the size of a ball of initial conditions around an
initial point on the \po\ $p$; (3) $\sum_p$ is the sum of {\em all}
unstable \po s, regardless whether they belong to the ergodic component
under investigation or dwell isolated in the Moon orbit; (4) The time an
ergodic orbit lingers in a neighborhood is determined by the least
contracting stable multipliers (the time of approach) no less than by the
least expanding unstable multipliers (the time of escape); (5) Level sums
(see \refsect{s:MarylandSums}) count all periodic points, not prime
...
are comparable. For that there is a fix in the literature, with
...
\,,
\ee{KazantsevAv}
where \period{p} is the period of prime cycle $p$. It is derived thusly:
``it is reasonable also to suppose that orbits with longer periods must
be weighted more heavily as they are longer and should provide a greater
contribution to the total sum.'' That's it: the `derivation' in its
entirety. The modification does not help at all, as the weight of a
periodic orbit is determined by its dimensionless Floquet
\emph{multipliers} which grow/shrink exponentially with cycle period (not
Oseledec Lyapunov exponents which measure average expansion/contraction
\emph{rate per unit time}).
...
spectral determinant formulas use the same computational input as the wrong
...
exactly the same to compute the exact formula as it costs to compute a
wrong formula. Go figure...
...
[{\bf 2012-09-20 Predrag:} the sup norm.] Unfortunately
...
[{\bf 2012-09-20 Predrag:} what the ???] We impose the
...
Historical postscript: As of 2012 Scott M. Zoldi was Vice President,
...
\item[2004-09-15 Predrag]
Regrettably Vanessa L{\'o}pez\rf{lop05rel} used the escape-time weighting
...
published in Phys. Rev. Letters (but where were faculty advisers?), but
citing it will just keep confusing future readers.  I had hoped I would
...
Alamos to learn how this works. To best of my knowledge, none of us could
