...
A week of intense pain: because of Rich's paper\rf{ChaKer12} which
...
cited by reputable researchers. We have to explain clearly why they are
wrong, in a remark for ChaosBook.org (or in some publication?). Here are
...
nowhere. (2) $\tau_p$ has dimension of $1/$[time], but a `weight' should be a
dimensionless number, the likelihood that an ergodic trajectory enters the
...
ergodic orbit lingers in a neighborhood is presumably determined by the least
...
\PC{2004-09-20  Its subtle. Depending on the observable,
...
\PC{2004-09-20 Tough luck. Nobody promised you a rose
garden.}
...
accurately. \PC{2004-09-20 Really?}
...
weighted sum.
\PC{2004-09-15  One is averaging over local Lyapunov exponents
...
expansions yet), so only a `level sum' approximation
    \PC{read ChaosBook.org \refsect{rem_lev_sums}}
an $n$th order estimate $\eigenvL_{(n)}$ of the leading \FP\ eigenvalue $\eigenvL$ is given by
\beq
1 = \sum_{i\inFix{n}} {  e^{\beta \Obser^n(x_i)} e^{-\eigenvL_{(n)} n} \over | \ExpaEig_i |}
% 1 = \sum_{i\inFix{n}} { g^n(x_i) e^{-\eigenvL_{(n)} n} \over | \ExpaEig_i |}
\ee{GOY:levl-sum}
...
for the expectation value of the
observable $\obser$ is
\beq
\expct{\obser} =
{ \expct{\Obser}_\eigCond \over \expct{\period{}}_\eigCond }
    \label{GOY:16.16a}
\eeq
