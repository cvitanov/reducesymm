...
\expct{\obser}  =  \frac{\sum_p \tau_p \obser_p}{\sum_p \tau_p}
               \,, \qquad \tau_p = \frac{1}{\sum^{e}_j \Lyap_{p,j}}
\,,
\label{ZoldiAv}
\eeq
where the $p$ sum goes over all known unstable \po s, and sometimes also
over judiciously chosen subsets of unstable \eqva. Here
\beq
\sum^{e}_j \Lyap_{p,j} =  \frac{1}{\period{p}}\ln \ExpaEig_p
                \,, \quad
\ExpaEig_p  = |\ExpaEig_{p,1} \ExpaEig_{p,2} \cdots \ExpaEig_{p,e}|
%    \continue
%          &=& \Lyap_{p,1}+\Lyap_{p,2}+...+\Lyap_{p,e}
\,,
\ee{FloqMult}
$\period{p}$ is the period of of the prime \po\ $p$, $\ExpaEig_{p,j}$ is
the magnitude of the $j$th Floquet multiplier, $\ExpaEig_p$ is the
product of the expanding multipliers $|\ExpaEig_{p,j}|>1\,,\;
j=1,\dots,e$,  and $\Lyap_{p,j}$'s are the strictly  positive Floquet
exponents.

        \PC{{\em cf.} chapter 8 (this is 1997 numbering) of ChaosBook.}
$\sum^{e} \Lyap_{p,i}$ is the local escape rate from single repelling
cycle $p$, of dimension $1/$[time], so one may interpret its inverse
$\tau_p$ as ``an estimate of the mean time spent by the system in
vicinity of \po'' $p$. $\tau_p$ is the Lyapunov time of cycle $p$, that
is, the mean time it takes for the density of neighboring trajectories in
an arbitrarily small ball centered around a point on the trajectory to
decrease by factor $1/e$.

The  `escape-time weighting' was introduced in a rapid communication
thusly: `Less unstable orbits must be weighted more heavily, so the
attractor dimension is approximated by `escape-time weighting'
\refeq{ZoldiAv}. That's it: the `derivation' in its entirety. Formula
\refeq{ZoldiAv} is then asserted to approximate the time average
$\expct{\obser}$ of observable $\obser(\ssp)$ over the chaotic attractor
...
nowhere. (2) $\tau_p$ has dimension of $1/$[time], but a `weight' should
be a dimensionless number, the likelihood that an ergodic trajectory
enters the neighborhood of the \po\ $p$. (3) $\sum_p$ is the sum over
{\em all} unstable prime \po s, regardless whether they belong to the
ergodic component under investigation or dwell isolated in the Moon
orbit.
(4) The guess for the weight $\tau_p$ is clearly wrong, as any periodic
orbit, no matter how long and unstable, has the comparable weight
$\sum^{e} \Lyap_{p,i}$, as long as its Lyapunovs (instability rate per
unit time) are comparable; the Lyapunov time has nothing to do with the
period of the particular cycle. For that there is a fix in the
literature, with the Lyapunov time in
\refeq{ZoldiAv} replaced by
\beq
\tau_p = {\period{p}}/{\sum^{e} \Lyap_{p,i}}
\,.
\ee{KazantsevAv}
The fix is explained as follows:
...
The \emph{exact} weight of the unstable prime periodic orbit $p$ was
essentially given by Kadanoff and Tang\rf{KT84} in 1984. For the
{classical trace formula for flows} it is
% \refeq{tr-L-cont}
\beq
\frac{\period{p}  \, e^{- \period{p}\eigenvL}}{\oneMinJ{} } \,
e^{ \beta \Obser_p}
    \,,\qquad \Obser_p = \period{p} \obser_p
\,,
\ee{weightPO-cont}
where $\eigenvL$ is the {\evOper} eigenvalue, and  $\beta$  is an
auxiliary variable. It is determined by the dimensionless Floquet
\emph{multipliers} (eigenvalues of the \po's {\monodromyM}
$\monodromy_p$) which grow/shrink exponentially with cycle period,
...
The \emph{exact} \cycForm s %\rf{AACI}
for the expectation value of the observable $\obser$ have form
\beq
\expct{\obser} =
...
\,,
\eeq
where the form of the \po \ sum $\expct{\cdots}_\eigCond$ depends on
whether it is computed from the trace formula, the {\dzeta}, or the
{\Fd}. This sum is never of the form \refeq{ZoldiAv}.

For pedagogical reasons one sometimes looks at the leading long-time
behavior, and approximates the denominator of \refeq{weightPO-cont} by
the product of the expanding multipliers $\ExpaEig_p$ of the
{\monodromyM} $\monodromy_p$,
\beq
...
\,.
\ee{weightPO-approx}
This weight might have been first used in Auerbach \etal\rf{pchaot},
who computed (for discrete time dynamical systems given by iterates of
map $f$) an $\cl{}$th order estimate $\eigenvL_{(\cl{})}$ of the leading
{\evOper} eigenvalue $\eigenvL$ from the sum of all periodic points $j$ of
period $\cl{}$
\beq
1 = \sum_{j\inFix{\cl{}}} t_j \, e^{\beta \Obser^n(\ssp_j)}
    \,,\qquad
t_j = \frac{e^{-\cl{}\eigenvL_{(\cl{})}} }{\ExpaEig_j}
% 1 = \sum_{i\inFix{n}} { g^n(x_i) e^{-\eigenvL_{(n)} n} \over | \ExpaEig_i |}
\,.
\ee{GOY:levl-sum}
More frequently cited is the Grebogi, Ott and Yorke unstable periodic
orbits paper\rf{yorke2}. They proved
...
of chaos this straightforward statement of probability conservation even
...
limit. However, the exact {\Fd} is not a long-time limit of anything, it
...
bold that one can only call it The Heresy: if one looks carefully enough,
there exists a \emph{single} \po\ that captures all dynamical averages of
a turbulent flow. This is so wrong that one is at loss what to say: there
is NO such single \po. Instead, there is the well established theory that
says how \po s are to be used, and how many are needed to capture the
hyperbolic parts of the {\nws} to a desired accuracy. It is as elegant
and systematic as Stat. Mech. and Quantum Field Theory. Read
\HREF{http://chaosbook.org/}{ChaosBook.org}. But who reads books nowadays?

The strangest thing about `Alternative Periodic Orbit Theories' is that
since introduction of zeta functions of Smale (1967), Ruelle (1976),
Gutzwiller (1969) and their cycle expansions (1987) there is no need for
them whatsoever. Why would on need to \emph{guess} an approximate \po\
weight when the \emph{exact} weight is already known? It costs exactly
the same to compute the exact {\Fd} as it costs to compute a wrong
formula, both require the same \po s,
Floquet multipliers, periods, and observables $\obser_p$ averaged over
$p$.  Go figure...
