...
unstabledir = np.cross(vaux, unstabledir)
...
if plotretmap:
	...
	savefig('image/retmapdir-1.png', bbox_inches='tight', dpi=200)
...
imin = np.argmin(snplus1sorted)
tck1 = interpolate.splrep(snsorted[0:imin+1], snplus1sorted[0:imin+1])
xint1 = np.linspace(snsorted[0], snsorted[imin], 1000)
yint1 = interpolate.splev(xint1, tck1)

tck2 = interpolate.splrep(snsorted[imin:len(snsorted)], snplus1sorted[imin:len(snsorted)])
xint2 = np.linspace(snsorted[imin], snsorted[len(snsorted)-1], 1000)
...
iminint = np.argmin(snplus1int)
sboundary = snint[iminint]
...
for i in range(1,int(np.max(group)) +1):
	...
	#using fsolve:
	fsolveoutput = fsolve(FPO, xn, args=(tof), full_output=1)#, xtol=1e-11)
	#print fsolveoutput
	xpo = fsolveoutput[0]
	...
	#using root:
	#rootout = root(FPO, xn, args=(tof), method='lm', tol=1.49012e-08)
	#xpo = rootout.x
