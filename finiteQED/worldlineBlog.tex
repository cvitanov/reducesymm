% reducesymm/QFT/worldlineBlog.tex
% was dailyBlogGX.tex
% Predrag  created              Jan 31 2018

\chapter{Learning worldline QFT}
\section{Quantum Field Theory refresher} % Spring 2018 Report}

\noindent
%Andy Chen (dropped out)
%Guopeng Xu <guopengxu@gatech.edu> \\ (Spring 2018, dropped out)
...
(\HREF{../presentations/LesHouch18/finiteQED.pdf} {my talk}\\
...
%    Guopeng is from Jilin University,
Having gone through the first part of Peskin and Schroeder\rf{PesSch95}
would be a plus.
...
that of photons \refeq{Schubert12(1.43)} by setting all $\varepsilon = 0$.
    %}
...
Following eq.~(8.11) of \refref{Schubert01},
% \textit{Perturbative Quantum Field Theory in the String-Inspired
% Formalism.}( https://arxiv.org/abs/hep-th/0101036)
...
\continue
&=& \tr\log\!\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}+\frac{B_{ab}}{4\bar{T}}\right)
-\tr\log\!\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}\right)
\,,
\label{GXlnDet}
\eea
and
...
Using $\tr A=\int\!d\tau \bra\tau A\ket\tau$
    \GX{2018-08-25} {Not sure whether this is the correct definition of a trace.}
\[
...
\Delta^{(1)}(\tau_1,\tau_2|\tau_a,\tau_b)
 &:=& \bra{\tau_1}\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}+\frac{B_{ab}}{4\bar{T}}\right)^{-1}\ket{\tau_2}
\continue
...
\continue
&=& \bra{\tau_1}\left((-\frac{1}{4}\frac{\partial^2}{\partial\tau^2})^{-1}
      -(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2})^{-1}
      \frac{B_{ab}}{4\bar{T}}(-\frac{1}{4}
      \frac{\partial^2}{\partial\tau^2})^{-1}\right.
\ceq
      +\left.\cdots\cdots\right)\ket{\tau_2}
\,.
\label{GXlnDet2}
\eea
...
\Delta(\tau_1,\tau_2)
=
\bra{\tau_1}(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2})^{-1}\ket{\tau_2}
\,.
\ee{GXlnDet2a}
...
\Delta^{(1)}(\tau_1,\tau_2|\tau_a,\tau_b)
&:=& \bra{\tau_1}
  \left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}
        +\frac{B_{ab}}{4\bar{T}}
  \right)^{-1}
      \ket{\tau_2}
\continue
&=& \Delta_{12}+\frac{[\Delta_{a1}-\Delta_{b1}]\,
                      [\Delta_{a2}-\Delta_{b2}]}
                     {G_{Bab}+\bar{T}}
\,.
\label{GXlnDet4}
...
\section{New Trends in First Quantisation - Bad Honnef 2025}
\label{c-BadHonnef}
% ~~~ April 16, 2025
