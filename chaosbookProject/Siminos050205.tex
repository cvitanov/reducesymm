...
\section{Candidates for the role of Metric}
 ...
 \subsection{A Jacobian Matrix for a Loop}
  ...
  $\mathbf{J}_p$ describes the local deformation of the neighborhood of the periodic
   ...
   \bea
  	\Js^s(x_o) & \equiv & \mathbf{T} e^{\int ds  \mathbf{A}\left(x(s))\right)} 		\continue
			& \equiv &  \lim_{m\rightarrow \infty} \prod_{n=m}^1
			  	e^{ \Delta s \, \mathbf{A}\left(x(n\, \Delta s)\right) }\,, 
  ...
  \eea 
   ...
   \beq
   	\fd{\Js^s(x_o)}{s}=\mathbf{A}(x(s))\Js^s(x_o).
	\label{eq:difJ}
   \eeq 
 ...
 \subsection{Properties of $\mathbf{J}_L$}
  ...
  \beq
   	\frac{d\mathbf{K}^s}{ds}=\frac{\partial u}{\partial y}\mathbf{K}^s\,,
	\label{eq:difK}
  \eeq
  where 
  \bea
  ...
  \bea
 	\frac{\partial u_i}{\partial y_j} & = & \frac{\partial u_i}{\partial x_k}\frac{\partial x_k}{\partial y_j} \continue
  ...
  \beq
  	\mathbf{N}^s \equiv \mathbf{K}^s(y_o)-\mathbf{H}(x(s)) \mathbf{J}^s(x_o) \mathbf{H}^{-1}(x_o)\,,
	\label{eq:def:N}
  \eeq
  Differentiating with respect to $s$, 
  \beq
  	\fd{N_{ij}^s}{s}  =  \fd{K_{ij}^s}{s}-\fd{H_{im}}{s}J^s_{mk}H^{-1}_{kj}
						-H_{im}\fd{J^s_{mk}}{s}H^{-1}_{kj} \,,
  \eeq
  or, using \refeq{eq:difJ} and \refeq{eq:difK},
  \beq
	\fd{N_{ij}^s}{s}  = \fp{u_i}{y_m} K^s_{mj}-\fp{H_{im}}{x_n}\fd{x_n}{s}J_{mk}H^{-1}_{kj}
  ...
  \beq	
	\fd{N_{ij}^s}{s} =   \left(\fp{H_{in}}{x_k}v_n + H_{in} A_{nk} \right)
  ...
  we get
  \beq	
	\fd{N_{ij}^s}{s} =  \left(\fp{H_{ik}}{x_n}v_n + H_{in} A_{nk} \right)
				H^{-1}_{km}(x(s)) K^s_{mj} 
				-\left(\fp{H_{im}}{x_n}\fd{x_n}{s}
  ...
  Jacobian on a periodic orbit and its remaining part is given in \refAppe{appe:ProofInvJacPO}.  
     
%   The Jacobian is not in general a symmetric matrix and thus it is not
%   diagonalizable by a unitary similarity transformation, while its eigenvectors
%   do not form an orthonormal set. Therefore we use the metric 
%   \beq
% 	\mathbf{M}(x) = \mathbf{J}_L^T(x) \mathbf{J}_L(x)\,,
%   \eeq
%   where the superscript $T$ denotes the transpose of a matrix.
%   $\mathbf{M}$ is symmetric and thus diagonalizable by a unitary similarity
%   transformation and possesses a complete set of orthogonal eigenvectors.
   
%   \subsection{Redefinition of the Jacobian}
%   
%    Define
%    \beq
%    	\fd{\Js^s(x_o)}{s}=\fd{t}{s}\mathbf{A}(x(s))\Js^s(x_o).
% 	\label{eq:dJ}
%    \eeq 
%    
...
%   
%   With this choice of metric we consider minimizing the functional 
%  \beq
%     \overline{F}^2=\frac{1}{S}\oint \left(\mathbf{J}_L\, \mathbf{P} \tilde{v}\right)^T			
%     		\left(\mathbf{J}_L\, \mathbf{P}  \tilde{v}\right) \, ds \,,
%     \label{eq:functionalPJ}
%  \eeq
%  where
%  \beq
%  	P_{ij}=\delta_{ij}-\frac{v_i v_j}{v^2}
%  \eeq 
%  The operator $v_iv_j/v^2$ acting on a vector projects it to the direction parallel to the velocity and thus 
%  $\mathbf{P}$ projects a vector on the plane transverse to $v$. The motivation to act with $\mathbf{P}$ on 
%  $\tilde{v}$ is that we are interested in penalizing components of $v$ transverse to the direction of the 
%  velocity. We observe that \refeq{eq:functionalPJ} will have the minimum value of zero on a periodic orbit.
% 
%  $\mathbf{P}$ is obviously symmetric and thus, from \refeq{eq:functionalPJ},
%  \beq
%      \overline{F}^2=\frac{1}{S}\oint \tilde{v}^T  \mathbf{P}\, \mathbf{J}_L^T \mathbf{J}_L\, \mathbf{P}  
%      \tilde{v} \, ds \,.
%  \eeq

 We will try to motivate our variational method in terms of the path integral formulation of a random work, \cf\ \refref{Risken}.
 ...
 \bea
 	\tilde{v} & = & (\mathbf{1}-\frac{v \otimes v}{v^2}+\frac{v \otimes v}{v^2})\tilde{v} \continue
		& = & \mathbf{P}^{\perp}v + v\frac{v.\tilde{v}}{v^2}
	\label{eq:velperppar}
 \eea
...
\label{appe:ProofInvJacPO}
 ...
 From \refeq{eq:dNds} with $s=t$, \ie\ when calculating $\mathbf{N}^t(x_o)$ along a periodic orbit we get
 \bea	
	\fd{N_{ij}^t}{t}  	& = &  \left(\fp{H_{im}}{x_n}v_n + H_{in} A_{nm} \right) H^{-1}_{mq}(x(t)) K^t_{qj} 
					-\left(\fp{H_{im}}{x_n}v_n + H_{in}A_{nm}\right)H^{-1}_{mq}(x(t))
				...
				& = & \left(\fp{H_{im}}{x_n}v_n + H_{in} A_{nm} \right) H^{-1}_{mq}(x(t))
					N_{qj} \,,
 \eea
 from the definition \refeq{eq:def:N} of $\mathbf{N}^s$. Thus we have a differential equation of the form 
 $\fsd{\mathbf{N}^t}{t}=\mathbf{B}(t) \mathbf{N}^t$ where 
 $B_{ij}=\left(\fp{H_{im}}{x_n}v_n + H_{in} A_{nm} \right) H^{-1}_{mj}(x(t))$. 
 The initial condition is found from \refeq{eq:def:N} to be $\mathbf{N}^0=0$ and thus the solution will be 
 $\mathbf{N}^t \equiv 0$ for all times. Thus
 \beq
 	\mathbf{K}^t(y_o)=\mathbf{H}(x(t))\mathbf{J}^t\mathbf{H}^{-1}(x_o)\,.
 \eeq  
 On a periodic orbit for $t$ equal to the period $T_p$ we have $x(T_p)=x_o$ and thus
 \beq
 	\mathbf{K}_p(y_o)=\mathbf{H}(x_o)\mathbf{J}_p\mathbf{H}(x_o)\,,
 \eeq
...
% \begin{enumerate}
% %	   \item{\bf Tue Mar 8:}
% %Stated the problem and presented motivation.
% %	   \item{\bf  Tue Mar 15:}
% %Will check the invariant meaning of the eigenvalues of Jacobian-like matrix along a loop.
% %	   \item{\bf  Tue Mar 29:}
% %Will try to write down PDE using $g=J^T J$.
% %	   \item{\bf  Tue Apr  5:}
% %Will try to implement the PDE for the R\"ossler system.
% %	   \item{\bf  Fri Apr 8:}
% %Write down PDE for the variational principle.   
% %	   \item{\bf  Tue Apr 12:}
% %Implement variational principle for R\"ossler.   
% 	   \item{\bf  Tue Apr 19:}
% Implement variational principle for R\"ossler and/or KS.   
% 	   \item{\bf  Tue Apr 26:}
