...
ETDRK4 scheme from \rf{ks04com} which works well. So I'll turn to
...
\item[2011-10-13 Chao to Ruslan] I read the file ksfmetd2.m of KS Solver which uses ETDRK4 scheme from \rf{ks04com}. Therefore I understand the main idea but have some specific questions:
...
Copied and slightly modified the file ksfmetd2.m to an independent routine by changing the input arguments to variables or constants defined in the program and assigning proper initial values as in \rf{ks04com}. Thus I can test it independently
...
Is that what Evangelos plots in \refrefs{SiminosThesis,SCD07}?
...
coordinates, \refsect{s:visualStatSp}. Not sure I can say it more
clearly...
...
\item[2011-10-24 Chao to Ruslan, Predrag] Well, there're some misunderstanding. What I asked initially is that in order to plot RPO accurately(i.e, accurately close), I have to compute the final value, $u(x,T)$ more precisely. In Ruslan's code it is obtained by third-order interpolation, which has an accuracy of $O(h^{n+1})$, where h is the time step size. Here we have four values assinged at two points, h is 0.1 so the accuracy is just about $10^{-5}$. I have to write code to improve the evolution of the last time step to get better plot.(Well now it seems not that necessary if it's just for plotting figure). About the period I haven't noticed that there're only six digits show up though they have double precision and I just used it.

\item[2011-10-24 Chao] Requirements to form a functional space studied here: (1)$<u>$ =0; (2)Statisfy the same boundary condition. After the proper functional space is defined, unless the boundary condition is very irregular, we usually do the inner product in Fourier space and then transfer back to real space as needed. In rarely case we do it directly in real space latice wise. One worth-trying projection basis for a solution $u(x,t)$ of \KSe in such functional basis is 
  \beq u_s=\frac{u(x,t)+u(-x,t)}{\parallel u(x,t)+u(-x,t)\parallel}\eeq
  \beq u_a=\frac{u(x,t)-u(-x,t)}{\parallel u(x,t)-u(-x,t)\parallel}\eeq
  \beq u_m=max\{u_m\in\ the\ same\ rpo\ of\ u(x,t):\parallel u_m(x,t)-u(x,t)\parallel\}\eeq  
\end{description}
