...
here:
      \begin{enumerate}
        ...
        periodic boundary condition, $u(x+L,t)=u(x,t)$, see\refsect{VarKSe}
        and \refsect{chap:KSreview}
       \end{enumerate}
...
\refeq{innerproduct} in Fourier space and then transfer back to real
space as needed. In rare case we do it directly in real space
lattice-wise. One worth-trying projection basis \refeq{intrSspTraj} for a
solution $u(x,t)$ of \KSe\ in such functional basis is
  \beq \be_s=\frac{u(x,t)+u(-x,t)}{\Norm{u(x,t)+u(-x,t)}}\eeq
  \beq \be_a=\frac{u(x,t)-u(-x,t)}{\Norm{u(x,t)-u(-x,t)}}\eeq
  \beq \be_m=u_m(x,t): \max \Norm{u_m(x,t)-u(x,t)}
        \,,
  \eeq where $u_m(x,t)$ is in the same \rpo\ as $u(x,t)$. Usually to conserve orthogonality 
  we orthogonalize $\be_m$ by Gram-Schmidt process to make $\{\be_a, \be_s, \be_m\}$ a orthonormal 
  basis. 
