...
%   da - Jacobian
%   fa - f(a(tend)) flow vector, obtained from interpolation.
%
...
%  Adapted from: AK Kassam and LN Trefethen, SISC 2005
  ...
  aa = a0;  tt = 0;  g = 0.5i*k;  nstp = ceil(tend/h);
  da = eye(N-2);
  if nargout < 3,
    for n = 1:nstp,
    ...
    for n = 1:nstp,
      ...
      if np > 0 & mod(n,np)==0 & n < nstp,
        ...
        aa = [aa, y1(:)];  tt = [tt,t];
        % ES: also save Jacobian
        y0 = zeros(N-2,N-1);  y0(1:2:end,:) = real(v(2:Nh,:));  y0(2:2:end,:) = imag(v(2:Nh,:));
        da = [da, y0(:,2:end)];
        % end export of Jacobian 
      end, end, end
