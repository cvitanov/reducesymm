...
discretization variables \refeq{pipeDiscr}\CS{2011-10-24:reference missing here, ``??''shows up}. 
...
measured here as
    \PC{ please read 2011-10-14 blog entries, respond}\CS{I looked up the full log history and don't find 
    any 2011-10-14 blog entries except my questions to Ruslan about the code and simino's modification to ks22ppo-angl.m which denotes ``slight modification to Jacobian calculation'' }
\beq
...
of the flow. Furthermore, relations such as \refeq{Power=I-D}\CS{``??'',reference missing} depend on
...
components \refeq{pipeDiscr}\CS{reference missing} or other primitive basis elements as
...
\ssp_n)$, see \reffig{f:MeanVelocityFrame}\CS{reference missing}. The {\stateDsp} portraits are
...
particular \reqva\ to define low-dimensional projections (see
\refsect{s:eqbSols})\CS{``??'',reference missing} may appear arbitrary, such a choice turns out to be
