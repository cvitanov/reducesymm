...
\item[2011-08-24 PC to Chao] Can you read first the ChaosBook.org chapter \HREF{http://ChaosBook.org/paper.shtml\#PDEs}{Turbulence?}, very critically?
...
equation (4): 
\beq\displaystyle\dot{a}_{k} = (k/\tilde{L})^{2}(1-(k/\tilde{L})^{2})a_{k} - (k/\tilde{L})\sum\nolimits^{+\infty}_{m=-\infty}a_{m}a_{k-m}\eeq
...
\beq\dot{b}_k = \frac{\partial v_k(a)}{\partial a_j}b_j\eeq here duplicate indices imply summation.
\beq v_k(a) = \dot{a}_k = (q^2_k - q^4_k)a_k - \frac{iq_k}{2}\sum\nolimits_ma_ma_{k-m}\eeq 
\beq\frac{\partial v(a)_k}{\partial a_j} = (q^2_k - q^4_k) - \frac{iq_k}{2}\sum\nolimits_m(\delta_{mj}a_{k-m}+a_m\delta_{j,k-m}) = (q^2_k - q^4_k) - {iq_k}a_{k-j}\eeq

Combining the last two equation, we obtain: 
\beq\dot{b}_k = (q^2_k - q^4_k)b_k - iq_k\sum\nolimits_ja_{k-j}b_j\eeq
