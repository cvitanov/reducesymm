...
\eqv\ front $y(x,0)$ \ES{This definition does not only apply to equilibria. Greene and
Kim also study the transfer of energy between modes in non-stationary trajectories.}.
...
\PC{might also want to indicate Christiansen and Lan choices of $L$
    in \reffig{fig:GreeneKim} discussion. Requires thought - they are
...
since the terms involving $u_x$ and $u_{xxx}$ vanish due to our
assumption that $u$ can be represented by a Fourier series
\PC{Since $u=y_x$, the integration constant $c$ in \refeq{eq:stdks} is
a combination of this ``kinetic energy" $E$ and other terms
harder to interpret. 
please reexamine this - we need to know what $c$ stands for.}.
...
\PC{I do not see a ``dash-dotted curve", at least not on the screen}
