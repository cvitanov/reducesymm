...
\begin{figure}
	...
	coefficients  calculated using cycle		
		...
		transformation\rf{BenOrs78} (circles and diamonds) discussed in
		\refref{MR94}.
        Here $w = 0.3$.
		}
\end{figure}
\begin{figure}
	...
	formulas]{\label{fig-results}
		...
		transformation\rf{BenOrs78}
		(circles and diamonds) discussed in \refref{MR94}.
 		Here $w =
...
We also compute the diffusion coefficient for different inter-disk
...
\rf{MacZwa83}. The topological length for each trajectory is
...
We have numerically computed the diffusion coefficient using the map
...
are calculated using 35000 trajectories.}
\label{fig-disvscont}
\end{figure}
