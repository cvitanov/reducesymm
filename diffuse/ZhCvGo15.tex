...
\end{figure}
    \TZ{2015-10-19}{ I do not really use the three generators to compute the 
    fundamental domain cycles, instead I use the idea of topological distinct 
    flights (\reffig{fig-fdflights}). We have yet to discuss the equivalence 
...
long disk-disk separation generators of \Dn{12}. The entire space group $p6mm$
...
short disk-disk separation, \reffig{fig-7diskFundDflips}a. We find it 
...
$\ell f s$ that involves multiple flips, ~\reffig{fig-7diskFundDflips} b. 
...
\TZ{2015-10-22}{The idea is that the topological periodic orbit in the 
fundamental domain is not sensitive to the order of flips. When $w$ increases, 
one might notice that a single flight along the orbit changes from $sf$ to 
$fs$. However, the relative position between the start and end triangular cells 
are always the same.}
...
All longer equivalence relations in ~\reffig{fig-7diskFundDflips} b
