...
\]
where $g_{\tp,\tx_{0}}(\tx_i)=\prod_0^{j-1} g_\tp(\tx_{j+1},\tx_{j})$ is the accumulated orientation changes along the orbit when starting from $\tx_{0}$. The displacement now has its dependence on the starting point we choose. Though one may not appreciate immediately, we denote the total group action for the orbit \[h_{\tp}(\tx_i)\equiv g_\tp(\tx_{i},\tx_{i-1})\circ\ldots\circ g_\tp(\tx_{0},\tx_{N_\tp-1})\circ g_\tp(\tx_{N_\tp-1},\tx_{N_\tp-2})\circ \ldots\circ g_\tp(\tx_{i+1},\tx_{i}),\] and define the quantity:
\beq
\hat{L}_{\tp}^{r}(\tx_i)\equiv (e+\hp^{1}(\tx_i)\cdots+\hp^{r-1}(\tx_i))\cdot\hn_{\tp}(\tx_i),
\eeq
to be the displacement traveled along the orbit $r$ times, when starting at $\tx_i$. Although the group action $h_{\tp}(\tx_i)\in G$ depends on the initial points on the orbit, It directly reflects the symmetry of the orbit, and subsequently all $h_{\tp}(\tx_i), \tx_i\in G$ belongs to the \emph{same} subgroup. 
...
\eea
The first term in the formula corresponds to $ \langle\hx\rangle^2 $ and second to $ \langle\hx^2\rangle $. It is trivial to see that the terms in the sum \[\sum_{\sigma\in G}\frac{\sigma\cdot \hat{L}_{\tp}(r,\tx_i)t_{\tp}^r e^{\beta\cdot\sigma\cdot \hat{L}_{\tp}(r,\tx_i)}}{N_{\tp}r}\right)^{2}\]
cancels out each other exactly, and the above formula produce a zero mean drift.
