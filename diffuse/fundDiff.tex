...
quotiented state space, i.e. the fundamental domain, is more compact,
as understood by the one-to-many map of periodic orbits. The
...
noticing that the little group is closed , we have:
...
\refeq{eq-trace-ir-disc} is written as a sum over fundamental domain
cycles
\beq
...
orbit takes the form already computed in~\refeq{eq-fd-displacement},
...
\beq
    t_{\tp} =\frac {z^{n_\tp
    r}{\chi_{\alpha}(\hp^{r}(\tx))e^{\beta\cdot\sigma\cdot\hat{L}_{\tp}(r,\tx)}}}{r\vert\det\left({\bf
...
\bea
\langle\hx\rangle_{\cal M} &=& -\left.\frac{\partial Z_\alpha}{\partial 
\beta} \right\vert_{\beta = 0, s = 0} \\\nonumber
&=& \frac{Z_\alpha}{|G|}\sum_{\sigma\in 
G}\sum_{r=1}^{\infty}\sum_{\tp}\langle \sigma\cdot \hat{L}_{\tp}(r, 
\tx)t_\tp(r, \tx)\rangle_{\tp}\equiv 0\,,
\eea
...
\bea
\langle\hx^2\rangle_{\cal M} &=& \frac{1}{\zeta_\alpha}
\sum_{r=1}^{\infty}\sum_{\tp}\langle \vert\hat{L}_{\tp}(r, 
