...
\,,
\eeq
is the weight associated with the orbit. Eq.~\refeq{eq-fd-zeta} takes
...
\emph{different} results for any non-trivial symmetry. For
consistency, we define $\langle\rangle_{\tp}$ as the cycle average to
replace the sum/integral.
...
\bea
\langle\hx\rangle_{\cal M} &=& -\left.\frac{\partial 
Z_\alpha}{\partial
\beta} \right\vert_{\beta = 0, s = 0} \\\nonumber
&=& Z_\alpha (0, 0)\sum_{\sigma\in
G}\sum_{\tp}\sum_{r=1}^{\infty}\sum_{\sigma\in
G} \frac{\left\langle\sigma\cdot 
\hat{L}_{\tp}(r,
\tx)\right\rangle_{\tp}}{\vert G\ExpaEig_\tp^r\vert}\equiv 
0\,,
\eea
because the direction of $\sigma\cdot \hat{L}_{\tp}(r, \tx)$ cancel 
each other when we sum all the group elements $\sigma$. Observing 
that the length$\vert \sigma\cdot \hat{L}_{\tp}(r, \tx) \vert$ does 
not change under rotation or reflection, the mean squared 
displacement is, :
\bea
\langle\hx^2\rangle_{\cal M} &=& \prod_{\tp}\left(1 - 
\frac{1}{\vert\Lambda_\tp\vert}\right)
\sum_{\tp}\sum_{r=1}^{\infty} \frac{\left\langle\vert\hat{L}_{\tp}(r,
\tx)\vert^2\right\rangle_{\tp}}{\vert\ExpaEig_\tp\vert^r} 
\,.\nonumber\\
\label{eq-msd}
\eea
