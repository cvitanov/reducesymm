...
In In \refrefs{art91,LorentzDiff,CGS92,Artuso94,CBdiffusion}  an explicit
...
%$ z\,=\,e^{-s} $,
$ z^{\zeit} \rightarrow e^{-s \zeit} $,
where $\zeit$ is now the (not necessarily integer)
%{\sl time-}
period of the prime cycle $p$:
$$
Z(\beta,s)\,=\,\prod_{p\in\PP} \exp \left( - {
 \sum_{r=1}^\infty {1 \over r}
 { e^{(\beta \cdot \hn_p- s \zeit) r } % z^{n_p r}
