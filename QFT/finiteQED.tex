...
is a sum of 518 vertex diagrams (or 47 self-energy diagrams)\rf{KinLin90}.
...
Kitano's Monte-Carlo simulations yielded values of the 4 and 5-loop contributions 
listed in \reftab{anomalValues}
that - though not competitive with the Feynman diagrams evaluations - were
reasonable, in the right ball park.

Our notes on the Kitano, Takaura and Hashimos formulation of lattice QED are in
\refchap{c-stochastic}.
Their approach is an example of the numerical stochastic perturbation theory (NSPT)
of \refsect{sect:NSPT}.
It is a  body of work which presumably has many
lessons relevant to, and implications for the proposed \dft\ calculation.

We care about such nonperturbative QFT calculations, as we believes 
that at least quenched QED is finite, in sense explained in \refref{Cvit77b}.

In the proposal of \refsect{sect:PCproposal} {\em {\Dft} magnetic moment anomaly evaluation}
we outline the steps needed to redo the Kitano calculation
in the \spt\ field theory framework.
...
One is to replace Monte-Carlo simulations by
truncations of \spt\ zeta functions, each contribution given by a (numerically)
exact solutions of system's defining equations.
Enumeration of {\Blatt}s 
\BravCell{\speriod{1}\times\speriod{2}\times\speriod{3}}{\period{}}{\tilt{}}, 
and determination of {\lsts} for each might seem a large enterprise, but remember,
we are replacing combinatorialy exploding numbers of Feynman diagrams 
by polynomially many {\Blatt}s, each supporting a convergent sum over \lsts.
