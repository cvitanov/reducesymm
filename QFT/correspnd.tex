...
\item[2025-04-08 Sergey A. Volkov]
\HREF{https://arxiv.org/a/volkov_s_2.html}  {arXiv publications}. 
\HREF{https://scholar.google.com/citations?hl=en&user=ybkn8u0AAAAJ&view_op=list_works&sortby=pubdate}
{Google Scholar}.

\item[2017-05-21 Predrag] to
Sergey  A. Volkov 
% <volkoff\_sergey@mail.ru>
%\\
...
Looking at the 4- and 5-loop results in \reftab{tabGaugeSets}, Volkov surmises
...
\item[2024-04-03 Sergey] See Volkov\rf{Volkov24}
{\em Calculation of the total 10th order {QED} contribution to 
     the electron magnetic moment},
     \arXiv{2404.00649}:

The total 5-loop [...] calculated 
 [...] 
value $A_1^{(10)}=5.891(61)$  
 [...] 
verification of 
 [...] 
T. Aoyama \etal\ (AHKN). The discrepancy is $5 \sigma$. 
A comparison with the AHKN values in 32 gauge-invariant classes 
 [...] 
the results are divided into 95 small gauge-invariant classes 
 [...] 
information on numerical  cancellations is also given. 

I have now created \reffig{Volkov24fsetV} of 5-loop quenched 
gauge sets. Note that Aoyama \etal\rf{AHHN25} have since removed the
discrepancy, see {\bf 2025-04-08 Predrag} below.
...
\newpage %TEMPORARY
\item[2025-04-22 Predrag].
% Ryuichiro Kitano <ryuichiro.kitano@kek.jp>, Hiromasa Takaura <hiromasa.takaura@yukawa.kyoto-u.ac.jp>, Shoji Hashimoto <shoji.hashimoto@kek.jp> 

Dear Ryuichiro, Hiromasa and Shoji

I'm the person who, together with Kinoshita Toichiro, in our 
{\em {Feynman-Dyson} rules in parametric space},
and 
{\em New approach to the separation of ultraviolet and infrared 
divergences of {Feynman}-parametric integrals} (1974)\rf{CviKin74a,CviKin74b},
had set up the {$g-2$} calculation that has now been completed, in the 
heroic undertaking, by Nio, Aoyama, Hayakawa and Hirayama 
(2025)\rf{AHHN25}.
%  and Volkov\rf{Volkov24}. 
I say \emph{completed}, as there is no way forward to higher 
loops within the Feynman-diagrammatic formulation. 

So I have been reading your triptych of papers, 
{\em Stochastic computation of  {$g-2$}  in QED} (2021)\rf{KiTaHa21},
{\em Quantum electrodynamics on the lattice and numerical perturbative 
computation of  {$g-2$}} (2023)\rf{KitTak23}, 
and
{\em QED Five-Loop on the Lattice} (2025)\rf{Kitano24}
with great interest, as possibly the new way forward. 

Independently of you,
% (it's only a few weeks ago that I noticed  Ryuichiro's \refref{Kitano24}), 
we have developed a Euclidean lattice field formulation\rf{CL18}, 
\arXiv{2503.22972},  motivated by, and applied to what I believe is the 
same class of problems as QED. 

So far, we have tested our formulation on scalar field theories,
$\phi^3$ and $\phi^4$ in 1 and 1+1 spacetime dimensions. We have not 
attempted any gauge theory: I think you could easily port your 
{$g-2$} lattice formulation to our lattice field theory. 

In our formulation, the observable, let's say the coefficient of 
{$(g-2)^n$}, is evaluated in terms of weighted sums 
\toCLpdf{equation.11.143} {Eq.~(143)} over multi-periodic states, 
\toCLpdf{equation.11.135} {Eq.~(135)}, which are exact, deterministic 
solutions of systems equations, see 
\toCLpdf{section*.14} {Sect.~III.B}.

Here is the main new thing, from your point of view: in 
\toCLpdf{section*.9} {Sect.~II} {\em Semiclassical field theory} all 
calculations are numerically \emph{exact, neither} Monte Carlo, nor 
Langevin. The contributions are exponentially decreasing with spacetime 
Bravais lattice \emph{volume}, each computed on the \emph{infinite} 
spacetime lattice, so there are no finite volume effects. If all goes 
well, a finite number of periodic states suffices to determine the 
desired expectation value to accuracy exponential in the largest volume 
solutions included in the sums. 

For quantum \emph{mechanics}, the most impressive application of our 
approach was the quantization of helium by Wintgen \etal, see 
\toChaosBook{chapter.42} {Chap.~42 \em Helium atom}. 
For quantum \emph{field theory}, our formulation is new, and applying it 
to QED would be a first. Indeed, were it successful, it would be a 
complelty now kind of lattice gauge theories. 

I suggest first carrying out the calculations for quenched QED: no charge 
renormalization! all nonlinearities in the single electron line, or 
loop! If that works, incorporate fermion determinants later. 

Even if realistic truncations of solutions contributing to our 
spatiotemporal field theory predictions turn out to be not numerically 
competitive with the existing work, if you can establish \emph{any} bound 
on $(\alpha/\pi)^n$ coefficients, that would great! 

Here are some of my thoughts on why (all conjectures, no firm results, so 
you can safely skip or ignore all this:) 

\HREF{https://birdtracks.eu/courses/QCD19/finiteQED.pdf} 
    {birdtracks.eu/courses/QCD19/finiteQED.pdf -- slides}

\HREF{https://ChaosBook.org/~predrag/papers/finiteQED.pdf} 
    {ChaosBook.org/$\sim$predrag/papers/finiteQED.pdf -- a blog}

\HREF{https://ChaosBook.org/~predrag/papers/finitness.html} 
    {ChaosBook.org/$\sim$predrag/papers/finitness.html -- the  story}

\HREF{https://ChaosBook.org/~predrag/papers/DFS_pris.pdf} 
    {ChaosBook.org/$\sim$predrag/papers/DFS\_pris.pdf -- an oration}

\HREF{https://ChaosBook.org/~predrag/papers/g-2.html} 
    {ChaosBook.org/$\sim$predrag/papers/g-2.html -- the truth}
