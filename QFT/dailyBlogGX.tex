...
charged scalar field in external field \refeq{Nprop}
    ...
    \JPE{2018-08-21}{External scalar particles have no polarisation - the vertex operator would be $V_{\textrm{scal}}[k] = \int_{0}^{T}d\tau\, \e^{i k \cdot x(\tau)}$ in this case. Therefore, the amplitude for scalars is related to that of photons by setting all $\varepsilon = 0$.}
\bea
...
\JPE{2018-08-21}{Good point. The reason is that there are two ways to turn the line into a loop with a virtual photon: \begin{enumerate}
...
and expand the latter term,
    ...
    have not gone through {\em 1loopPhi3.pdf} myself.
        ...
        \JPE{2018-08-21}{Indeed all worldline Green functions are exact. As commented above, the final result represents an all orders summation of the matrix elements in the expansion of   $\bra{\tau_1}    [1+(-\frac{1}{4T}\frac{\partial^2~}{\partial\tau^2})^{-1}\frac{B_{ab}}{4\bar{T}}]^{-1} \ket{\tau_2}$. The error is in the claim $\bra{\tau_1}-\frac{B_{ab}}{4\bar{T}} \ket{\tau_2}
=\frac{(\Delta_{1a}-\Delta_{1b})(\Delta_{a2}-\Delta_{b2})}{\bar{T}+G_{Bab}}$ .}
I obtain
\bea
&&\Delta_{(1)}(\tau_1,\tau_2|\tau_a,\tau_b)
