...
together; this is the Ahmadiniaz, Bashir and Schubert\rf{AhBaSc16} approach to
...
for the open line in both cases\rf{AhBaSc16} are Dirichlet; the
difference lies in the modification to the kinetic operator.
    \end{enumerate}
Possibly Davydychev\rf{Davydychev06} is also relevant.
...
\JPE{2018-07-29} {
    It is probable that in \refref{CorSch15} the authors had made a rescaling
...
%\PC{2018-08-21}{$T$ replaced here by $\bar{T}$}
\bea
\Det\!\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}+\frac{B_{ab}}{4\bar{T}}\right)
&=& \Det\!\left[-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}
            \left(1-(\frac{\partial^2}{\partial\tau^2})^{-1}\frac{B_{ab}}{\bar{T}}\right)
        \right]
\label{GXgreen1}\\
&=& \Det\!\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}\right)
    \,
    \Det\!\left(1-(\frac{\partial^2}{\partial\tau^2})^{-1}\frac{B_{ab}}{\bar{T}}\right)
\,.
\nnu
\eea
where
$\Det\!\left(-\frac{1}{4}\frac{\partial^2}{\partial\tau^2}\right)
=(4\pi \bar{T})^{-\frac{D}{2}}$ is evaluated in \refeq{Schubert12(1.34)}.
...
term, it would appear that
    \JPE{2018-07-29, 2018-08-21} {
    This is not correct! See eq.~(8.9) in \refref{Schubert01}: you are missing
    some delta functions. Schubert 1999 literature review\rf{Schubert01} {\em
    Perturbative Quantum Field Theory in the String-Inspired Formalism},
    \arXiv{hep-th/0101036}, is a first base for just about any question on the
...
where $G_{Bab}$ is the loop Green's function.
    \JPE{2018-08-21}{
    The result you need is $\left(\,B_{ab} (\frac{d^{2}}{dt^{2}})^{-1}B_{ab}
    \,\right)(\tau, \tau^{\prime}) = -G_{Bab}B_{ab}(\tau, \tau^{\prime})$ which
    is eq.~(8.9) in \refref{Schubert01}. To prove this,
    insert two complete sets of states:
\[
    \bra{\tau} B_{ab} (\frac{d^{2}}{dt^{2}})^{-1}B_{ab} \ket{\tau^{\prime}}
    =
    \int_{0}^{T}\!\!ds \int_{0}^{T}\!\!ds^{\prime}
    \bra{\tau} B_{ab} \ket{s}
    \bra{s} (\frac{d^{2}}{dt^{2}})^{-1} \ket{s^{\prime}}
    \bra{s^{\prime}} B_{ab} \ket{\tau^{\prime}}
\,,
\]
    write the matrix elements as functions of $\tau$ and
    $s$, $\tau^{\prime}$ and $s^{\prime}$ and compute the $s$ and $s^{\prime}$
    integrals.
    }
As
\[
...
\label{GXgreen2}
\eea
% \GX{2018-07-29} {I don't know why this is equal to the loop Green's function.}
    \JPE{2018-08-21}{
    As discussed in \refref{Schubert01}, the
    incorporation of a photon propagator between two points on a one loop
    diagram (that turns it into a two loop diagram) can be incorporated into
    the worldline action at the expense of changing the kinetic term -- see
    eqs.~(8.2) - (8.4) of  \refref{Schubert01} :-). Finding the Green's function and
    the functional determinant of the new kinetic operator then ensures that
    the effect of the internal photon is automatically included into all further
    worldline calcultions. So the one-loop calculation turns into a two loop
    calculation upon application of the new Green's function. The same happens
    below for the open line: the incorporation of a virtual photon between two
    points on the line turns a tree level process into a one loop process. The
    effect of this photon can be absorbed into the worldline calculations by
    using $\Delta_{(1)}$ instead of the open line's $\Delta$.}
...
\ee{1loopPhi3GX1}
%    \JPE{2018-07-29} {
%    The Green's function is defined as the matrix element of the
...
%\]
%The first term is the Green's function with DBC. According to James'
...
%\JPE{2018-08-21}
For $\bra{\tau_1}\frac{B_{ab}}{4\bar{T}}\ket{\tau_2}$ ,
the operator $\hat{B}_{ab}$ has matrix elements
\[
B_{ab}(\tau_1,\tau_2)
    := \bra{\tau_1}\hat{B}_{ab}\ket{\tau_2}
    =[\delta(\tau_a-\tau_1)-\delta(\tau_b-\tau_1]\,
    [\delta(\tau_a-\tau_2)-\delta(\tau_b-\tau_2)]
\,.
\]
    \JPE{2018-08-21}{Be careful here about definitions of operators and their
    functional form (matrix elements) with respect to a given basis.
    Your calculation below doesn't really look right as it stands. }
%\item[2018-07-29 Guopeng]
\bea
&& B_{ab}(\tau_1,\tau_2) %<\tau_1|B_{ab}(\tau_1,\tau_2)|\tau_2>
\continue
...
\[
\Det\!\left(1-(\frac{\partial^2}{\partial\tau^2})^{-1}\frac{B_{ab}}{T}\right)
=(1+\frac{G_{Bab}}{T})^{-\frac{D}{2}}
\]
    \JPE{2018-08-21}{The result requires application of the $\ln\det = \tr\ln$
    formula, and a subsequent expansion of the logarithm. This is a main result of
    the new work that I must write up for you.
    }
...
\]
\JPE{2018-08-21}{
This is not what I'm saying, because you have in fact to calculate (see
your \refeq{XG2beExpanded}) the matrix elements to all orders in the expansion of the
$\left(1 + (\frac{d^{2}}{dt^{2}})^{-1}
\frac{B_{ab}}{\bar{T}}\right)^{-1}(\frac{d^{2}}{dt^{2}})^{-1}$. Again, I must
write this up and send it to you.
All worldline Green's functions are exact. As
