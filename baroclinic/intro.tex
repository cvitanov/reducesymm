...
the heavier fluid to the lighter one. This would create both subsurface
...
effects of the Coriolis force need to be considered. Thus, if we repeat
...
From these relations, one concludes that there is a equilibrium state,
...
properties of this equilibrium, and the motions that are generated afterwards? The first
question has been studied widely, and it uses linear approximations
...
nonlinear terms now become important. The hope is that nonlinear theory
...
one works with 50 layers, but not two). The two layers are coupled by the
...
The author would like to acknowledge the valuable discussions and advice
