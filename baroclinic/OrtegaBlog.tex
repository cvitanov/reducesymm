...
\item[2012-04-30 Sebastian]
I have made a Matlab code that shifts the image by a $\phi$. Using
$g(\phi)=diag(e^{-2 \pi i k \phi /M})$. Looks as expected when converted
...
\item[2012-04-30 Sebastian]
I think I now got the slicing condition. If it is not too late now, I would like to know what you think about it. I start with:

\beq
\frac{\partial}{\partial \phi} \|a-g a'\|^2=0
\ee{120429_5}

then

\beq
\frac{\partial}{\partial \phi} \left((a-g a')^+ (a-g a')\right)=0
\ee{120429_6}

expanding

\beq
\frac{\partial}{\partial \phi} \left(a^+ a - a^+ga'-(ga')^+a+(ga')^+ga'\right)=0
\ee{120429_7}

carrying out the derivative, nothing that $\partial g/\partial \phi = T g$ and grouping terms I arrive to:

\beq
(a-ga')^+T g a'+(T g a')^+(a-ga')=0
\ee{120429_8}

now, as both terms are a complex numbers and one is the complex conjugate of the other, this means that the imaginary term vanish and the condition can be written:

\beq
2 Re[(a-ga')^+T g a']=0
\ee{120429_9}

now, $a=g\widehat{a}$ so that

\beq
2 Re[(\widehat{a}-a')^+g^+T g a']=0
\ee{120429_10}

but $g^+=diag[\overline{e^{-im\phi}}]=diag[e^{\overline{-im\phi}}]=diag[e^{im\phi}]$ so that $g^+T g=T$ an the condition becomes:

\beq
2 Re[(\widehat{a}-a')^+T a']=0
\ee{120429_11}

In the turbulent pipe article, it is argued that as $T a'$ is perpendicular to $a'$ then $a'^+T a'=0$. Since up to here my results have been similar I guess this would also happen. However it is not clear for me that $a'^+T a'=0$. Anyway I guess I could use condition \refeq{120429_11} for Newton-Raphson now.

By the way, I tried to run the model changing the resolution, but results do not look well. Not much happened, energy did not traveled up scale. Small scales are important. I think the simulation starts looking good from 512X128 so I might change to that one. But there is always the chance that I did not change some parameter in the simulation needed when changing scales. I would have to check with Annalisa.
