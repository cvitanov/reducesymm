...
for $k=-N/2,....,N/2-1$. And the synthesis equation as:
    \PC{what is `synthesis equation'? Sebastian: It is another way to refer to the inverse of the DFT.}
\beq
...
$k=-N/2,\cdots,N/2-1$. This leads to the slice condition:
    \PC{$[-N/2,\cdots,N/2-1]$ is a strange range? Sebastian: It is the definition some authors give to the DFT (see for instance Chapter 1 of \refref{SpectralPDE}; one can get it electronically trough Gatech library webpage) probably it is done this way in order to avoid multiplying the coefficients related with $N/2$ and $-N/2$ by $1/2$. Other authors use the range $[0,\cdots,N-1]$.}
\beq
