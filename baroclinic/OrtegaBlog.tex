...
\item[2012-04-30 Predrag]
Unitary transformations preserved the magnitude of a complex vector
$\norm{\ssp} = \norm{\LieEl(\gSpace)\ssp}$, so $\LieEl^\dagger\LieEl=1$.
Make $\gSpace$ infinitesimal, and expand
$\braket{(1+\gSpace\Lg)}{(1+\gSpace\Lg)}$ to leading order in $\gSpace$.
You should get $\Lg^\dagger = - \Lg$, and from that it follows by complex conjugation
of the dot product $\slicep^\dagger \Lg \slicep$ that
$\slicep^\dagger \Lg \slicep =0$.

\item[2012-04-30 Sebastian, Predrag]                                            \toCB
Derivation of the slice condition for unitary groups:
\beq
\frac{\partial}{\partial \gSpace} \norm{a-\LieEl\slicep}^2 =0
\,,
\ee{120429_5}
Expand
\[
    (a-\LieEl\slicep)^\dagger (a-\LieEl\slicep)
= \norm{\ssp}^2
...
Substitute $\ssp=\LieEl\sspRed$.
    \PC{This is OK for abelain groups, as $\LieEl \Lg = \LieEl \Lg$, but
...
%\beq
%\frac{\partial}{\partial \gSpace}
%\left(a^\dagger a - a^\dagger g\slicep -(g\slicep )^\dagger a+(g\slicep)^\dagger g\slicep \right)
%\,,
%\ee{120429_7}
%carrying out the derivative, nothing that $\partial g/\partial \gSpace =
%\Lg g$ and grouping terms I arrive to:
%\beq
%(a-g\slicep)^\dagger \Lg \LieEl\slicep +(\Lg \LieEl\slicep)^\dagger (a-g\slicep )=0
%\,.
%\ee{120429_8}
...
%can be written:
%\beq
%2 \Re[(a-g\slicep)^\dagger \Lg \LieEl\slicep ]=0
%\,,
%\ee{120429_9}
%and $a=g\widehat{a}$, so that
%\beq
%2 \Re[(\widehat{a}-\slicep)^\dagger g^\dagger \Lg \LieEl\slicep]=0
%\ee{120429_10}
%but
...
%\beq
%2 \Re[(\widehat{a}-\slicep)^\dagger \Lg \slicep ]=0
%\ee{120429_11}
Now one use condition \refeq{120430_11} for Newton-Raphson now.
