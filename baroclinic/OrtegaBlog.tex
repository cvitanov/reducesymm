...
\item[2012-05-07 Sebastian]
By tansversality condition I meant chart border condition. I implemented something for this a few days ago, but I have to check it. What is implemented is different from \refeq{min5}; and I didn't spend enough time in the derivation so most likely it is not correct. I will plot the $\cos$ of the angle and $\Norm{\groupTan(\sspRed(\zeit))}$ once I have the implementation.
I am uploading the code I used. That is, the following three files:
\begin{itemize}
...
\end{itemize}
Newton-Raphson implementation is just a couple of lines in \verb"Slice.m". It is very similar to the continuous version, just that everything is evaluated in integer values of $\gSpace$ in \refeq{min4}. A centered finite difference equation is used to evaluate the derivative of \refeq{min4}.

Not sure where are Ashley's notes are in SVN. Let me know where 
 there are. It would be great to compare notes.
