...
I will search for an algorithm for this; although it should be easy to create one from scratch; we just have to evaluate \refeq{min4} form $1$ to $1024$, and see where the values goes from positive to negative. If we select a good initial value for \refeq{min4}, and if we are lucky enough, then it should work. Of course, we wont be able to find a value that makes \refeq{min4} zero, but this is a resolution issue.
