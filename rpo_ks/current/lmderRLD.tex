...
When searching for RPOs and PPOs of the truncated \KSe\
(see \refeq{eq:KS}), we need to solve the system of $N-2$ equations
\beq
  \pm{\bf g}(\pm\shift)f^\period{}(a) - a = 0\,,
\ee{eq:system}
with $N$ unknowns $(a, \period{}, \shift)$, where $f^t$
is the flow map of the \KSe.  We use plus sign for RPOs and
minus sign for PPOs (see \refeq{KSposFour}).
...
Let $(\bar{a}, \bar{\period{}}, \bar{\shift})$ be the starting guess for a \rpo\
obtained through a close return within a chaotic attractor.  
We require that the initial integration step size
does not exceed $h_0$, so we round off the
number of integration steps to $n = \lceil \bar{\period{}}/h_0\rceil$, where
...
\bea
 ...
 F^{(m)}&\!=\!& \pm{\bf g}(\pm\shift)f^{\tau'}(a^{(m)}) - a^{(1)} = 0\,,\nonumber
...
to settle on the attractor at some point $\bar{a}$.  This point was taken 
...
$\bar{\period{}}$ and shift $\bar{\shift}$, we located an 
