...
{\color{blue} A typical long orbit is shown in \refFig{f:ks_L22}.}
...
manifolds and homo/heteroclinic connections between \eqva.
   \edit{
We find this representation, introduced by Gibson\rf{GHCW07},
better suited to visualization of nontrivial solutions than
the more standard Fourier mode (eigenvectors of the $u(x,t)=0$
...
\ESedit{Many methods have been developed for visualization of stable and unstable manifolds,
see \refref{krauskopf_survey_2005} for a survey. In high-dimensional flows
visualization of stable manifolds becomes impossible (unless the system can be restricted
to a low-dimensional approximate innertial manifold, see \refref{kev01ks} for example),
while unstable manifold visualization becomes harder as its dimension increases. Here
we concentrate on visualizations of $1$ and $2$-dimensional unstable manifolds as they
provide the basic mechanism that connects neighborhoods of equilibria. Our approach
to visualization is very simple compared to the methods presented in \refref{krauskopf_survey_2005},
yet it is sufficient for our purposes since, as we will see, the unstable manifolds we
study here terminate in another equilibrium and thus there is no need to truck them
for long times.}
...
\ESedit{
Heteroclinic cycles are non-generic in general high-dimensional systems,
but they are well known to be robust in systems with continuous symmetry, see
for example \rf{KrupaRobHetCyc97} for a review. For KSe evidence for the existence
of robust heteroclinic cycles has been provided by Armbruster \etal\rf{AGHks89}
and by Kevrekidis \etal\rf{KNSks90}. Armbruster \etal\rf{AGHks89} study a fourth order
truncation of KSe dynamics on the center-unstable manifold of $\EQV{2}$ close to
its bifurcation of the trivial solution and are able to prove existence of a heteroclinic
cycle for this truncation, see also \refref{AGHO288}. Kevrekidis \etal\rf{KNSks90} study
the dynamics numerically and explain the existense of a robust heteroclinic cycle for
a range of parameters close to the onset of the 2-cell branch in
terms of the structure of the symmetry and flow invariant subspace of the problem.
Here we present an adaptation of the arguments in \refref{KNSks90} to explain the
heteroclinic cycles we have found for $L=22$.
}
For our system size
...
%\begin{figure}[t]
%\begin{center}
%(\textit{a})\hspace{1ex}\includegraphics[width=0.7\textwidth, clip=true]{figs/ks22_rpos_Tdelta.eps}\\
%(\textit{b})\includegraphics[width=0.72\textwidth, clip=true]{figs/ks22_rpos_lyap.eps}
%\end{center}
%\caption{
...
returns to its reflection \ESedit{or its discrete rotation
after a period: $u(x,t+\period{p})=\gamma u(x,t)$, $\gamma\in C_m$ or $D_1$
and is thus periodic
with period $m\period{p}$.}
The dynamics of
...
which possess the symmetry of type {\bf (b)} \ESedit{with $\gamma=\Refl\in D_1$}. Some of the shortest such
