...
after time $m \period{p}$. It is easy to see that this is
only satisfied if $\gamma\in C_m$ or if $\gamma=\Refl$ (see
\refref{golubitsky2002sp} for a general discussion of
conditions on the symmetry of \po s).
}
    \ES{Please recheck that I am correct and we cannot have
    $\gamma\in D_m$.}
    \ES{There is some repetition, we mention orbits with
    reflection both here and in \refeq{KSpos}. RLD: addressed this
    comment by calling \refeq{KSpos} \rpo s with reflection.  Then here we show
    that they are actually pre-periodic.}
