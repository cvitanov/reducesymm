...
for $(N\!+\!1)$-dimensions.
    \edit{
As we note in \refappe{sec:lmderRLD}, of the thousands of orbits
computed in this study only two failed this higher-resolution test.

(RLD: Suggest to replace the above sentence with the following text:)
Therefore, the selection of parameter $L$ for which a structurally
stable chaotic dynamics exists and can be studied is rather subtle.  
We have found that the value of $L = 22$ satisfies these requirements.
In particular, all of the equilibria and relative equilibria persist and 
remain unstable when $N$ is increased from 32 (the value we use in our
numerical investigations) to 64 and 128.  Most of the \rpo s we have
found for this system also exist and remain unstable for larger values
of $N$ as well as smaller values of the integration step size 
(see Appendix~\ref{sec:lmderRLD} for details).
   }
...
Any rational shift $ \Shift_{1/m}u(x)=u(x+L/m)$ generates a discrete \ESedit{cyclic subgroup $C_m$ of $O(2)$, also
a symmetry of KS system. Reflection together with $C_m$ generates another symmetry of KS system,
the dihedral subgroup $D_m$ of $O(2)$. The only non-zero Fourier components of a
solution invariant under $C_m$ are $a_{jm} \neq 0$, $j =1,2,\cdots$,
while for a solution invariant under $D_m$ we also have the condition $\Re a_j=0$ for all $j$}.
...
in the classical papers of
Kevrekidis, Nicolaenko and Scovel\rf{KNSks90}, and Greene and
Kim\rf{ksgreene88}, \ESedit{among others}. The relevant bifurcations up to the
...
\REQV{\pm}{2}. \ESedit{All \eqva\ are in the antisymmetric 
subspace $\bbU^+$, while \EQV{2} is also invariant under $D_2$ and \EQV{3}
under $D_3$.
}
...
\ESedit{Armbruster et. al\rf{AGHks89,AGHO288} and Brown and Kevrekidis\rf{BrKevr96}
	link the birth of \rpo s to an infinite period global bifurcation involving a heteroclinic loop
	connecting equilibria or a bifurcation of \reqva, see also \refref{Krupa90}.
	In \refref{BrKevr96} creation of \rpo\ branches through bifurcation of \po s is also
	reported. 
}
...
However, due to the KS equation invariance under \ESedit{the dihedral $D_n$ and
cyclic $C_n$ subgroups, the following types of \po s are possible:}

{\bf (a)} The \po\ lies \ESedit{within a subspace pointwise invariant under 
the action of $D_n$ or $C_n$. For instance for $D_1$ this would be
the  $\bbU^+$ antisymmetric subspace,
$-u_p(-x) = u_p(x)$, and $u(x,\period{p}) = u(x,0) = u_p(x)$.
The periodic orbits found in \refrefs{Christiansen97,lanCvit07}, for example,
are all in $\bbU^+$, as a result of restricting the dynamics to that subspace by the choice of antisymmetric
boundary conditions. In our case, for $L=22$, the dynamics in $\bbU^+$ are dominated by attracting (within
the subspace) heteroclinic connections and thus we have no periodic orbits of this type, or in
any other of the $D_n$--invariant subspaces, see \refsect{sec:L22}.}

{\bf (b)} \ESedit{The \po\ satisfies
\beq 
	 u(x,t+\period{p})=\gamma u(x,t)\,,
	\label{eq:POspattemp}
\eeq
for some group element $\gamma\in O(2)$ such that $\gamma^m=e$ for some integer $m$
so that the orbit repeats after time $m \period{p}$. It is easy to see that this is
only satisfied if $\gamma\in C_m$ or if $\gamma=\Refl$ (see \refref{golubitsky2002sp} 
for a general discussion of conditions on the symmetry of \po s).}\ES{Please recheck
that I am correct and we cannot have $\gamma\in D_m$.}
\ES{There is some repetition, we mention orbits with reflection both here and
in \refeq{KSpos}.}
If an
...
\ESedit{All periodic orbits we have found for $L=22$ are of type \refeq{eq:POspattemp} with
$\gamma=R$. 
Pre-periodic orbits with $\gamma\in C_n$ have been found
by Brown and Kevrekidis\rf{BrKevr96} for KS system sizes 
larger than ours, but we haven't found them for $L=22$.}
Pre-periodic orbits
