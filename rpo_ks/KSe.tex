...
$G$, \ESedit{the} group of actions $ g \in G $
on a \statesp\ (reflections, rotations, \etc)
is a symmetry of the flow
$\dot{u} = F(u)$ if $g\,\dot{u} = F(g\,u)$.
The KS equation  \refeq{ks} is time
translationally invariant, and space translationally invariant
on a periodic domain under
the 1-parameter group of $O(2)$ rotations:
\beq
g\,\dot{u} = F(g\,u)
    \,,\qquad
...
is an equivalent solution for any
$-L/2 < \shift \leq L/2$,
as is the
...
%square-integrable, real-valued velocity fields periodic on
%the $\Omega = [-L/2,L/2]$ interval,
\RLDedit{real-valued velocity fields periodic and square integrable
on the interval $\Omega = [-L/2,L/2]$,}
\PC{replace ``square-integrable" by whatever the math papers say the
    space is}
\RLD{I think `square integrable' is fine.  Strictly speaking, 
we need to state that our functions are square integrable 
on $\Omega$, so I have re-phrased the sentence a bit.}
\begin{align}
...
$u(x) = u^+(x)+ u^-(x)$, $u^+(x)= P^+ u(x) \RLDedit{ \in \bbU^+}$, 
$u^-(x)= P^- u(x) \RLDedit{ \in \bbU^-}$,
of the space of solutions into irreducible subspaces,
\PC{check $\oplus$ - do they use circle around +?}
\RLD{Yes they do, $\oplus$ means `direct sum' of spaces.}
$
...
\beq
    ...
    P^-=(\matId-\Refl)/2
\ee{P1P2proj}
are the \ESedit{anti}symmetric/symmetric projection operators.
% \beq
% \bbUsymm  = \{u \in \bbU  \: | \; u = \Refl\,u \}
%\label{symmSubspU}
% \eeq
% the subspace of velocity fields invariant under $D_1$.
%    \PC{$\bbUsymm$ definition looks \PCedit{very wrong}}
\ESedit{Applying $P^+,\,P^-$ on the KS equation \refeq{KSO2}}
\PC{and noting that the derivative
$\partial_x$ is odd under reflection. ES: I don't think we need
this here.} we have\rf{KNSks90}
\ESedit{
\bea
 u_t^- &=& - (u^+u^-_x + u^-u^+_x )
                - u^-_{xx} - u^-_{xxxx}
...
If \ESedit{$u^- = 0$}, KS flow is confined to
the antisymmetric \ESedit{$\bbU^+$} subspace,
    \PC{ NOTE:
    a 1/2 factor has crept into $u^-$ evolution, and
    I would like here to introduce the fundamental domain
    $\Omega/D_1 = [0,L/2]$, but that might wait for
    the next paper.
    }
\ES{No $1/2$ factor. Notice that $u^+$ equation for antisymmetric part 
correctly separates.}
\ESedit{
\beq
 u_t^+ = - u^+u^+_x
...
but otherwise the nonlinear terms in \refeq{KSD1} evolution \ESedit{equations}
mix the two subspaces.
    \PC{
    why \refeq{KSD1}? (a) to clarify embedding of
    $\bbU^-$ into $\bbU$
    (b) to explain the desymmetrization of evolution defined on
     $\Omega^{+} = [0, L/2]$ fundamental domain space, with evolution at
    instant of crossing $x=0$ given by reflection $\Refl$.
    }
    \PC{re integrators: make sure $\Refl$ applied after
...
within it. For example, the 1/2-cell translations \ES{moved all references to 1/2-cell translation to one place.}
\beq
...
translations.
The antisymmetric subspace \refeq{KSU-} is unstable
...
\,,
  \label{eq:3dks}
\eeq\ES{Changed signs here, it seems simpler this way.}
with the `time' reversal symmetry,
\[
x \to -x,\quad u \to -u, \quad v \to v, \quad w \to -w \,.
\]\ES{The term $c u$ breaks the symmetry. We may overcome this by changing sign of c as well, but it is consider a parameter, not a variable.}
\PC{might move space average def \refeq{rpo:spac_ave} to here,
    note that
    $\expct{u} = \expct{v} = \expct{w} =0$
    }
 Rewriting \refeq{eq:3dks} as \ES{With the corrected form of \refeq{eq:stdks} we cannot write this}
\beq
\PCedit{
...
\ee{eqvOfEqv}
we see that
for $\expctE<0$, \ES{Isn't $E>0$ by definition?} $u+w$ increases without bound with $x \to \infty$,
