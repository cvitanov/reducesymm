...
\tildeL$. \PCedit{\refFig{f:ks_largeL} presents a typical `turbulent' evolution
...
The color bar indicates the color scheme for $u(x,t)$, \PCedit{used also
...
\end{romannum}
\ES{dropped: as long as all other components of $u(x)$ are
...
with spatial coordinate $x$ playing the role of `time', and the integration constant \expctE\ can be interpreted as `energy',
...
n the Fourier representation the \reqva\ time
dependence is
\beq
...
$\Refl$.  {\Rpo s} \refeq{KSrpos} are periodic in
...
{\bf (b)} If \rpo\ is of reflection type \refeq{KSpos}:
$\Refl\Shift_{\shift_p/L} u(x,\period{p}) =
-u(-x-\shift_p,\period{p}) = u(x,0)$, then it is
preperiodic to a \po\ with period
$2\period{p}$.
\RLDedit{
Indeed, since $(\Refl\Shift_{\shift_p/L})^2 = \Refl^2 = 1$ and the KS solutions
are time translation invariant, it is easy to see
from \refeq{KSpos} that
\[
  u(x,2\period{p}) = \Refl\Shift_{\shift_p/L} u(x,\period{p}) =
  (\Refl\Shift_{\shift_p/L})^2 u(x,0) = u(x,0)\;.
\]
    }
%Indeed,
%\ESedit{
%    from time translational invariance
%\beq
%        u(x,\period{p}/2)  =  \Refl\Shift_{\shift_p/L} u(x,\period{p})\,.
%\eeq
%Applying $\Refl\Shift_{\shift_p/L}$ on both sides,
%\bea
%        \Refl\Shift_{\shift_p/L} u(x,\period{p}/2)      & = & \Refl\Shift_{\shift_p/L}\Refl\Shift_{\shift_p/L} u(x,\period{p})\continue
%                                                        & = & \Refl\Shift_{\shift_p/L}\Shift_{-\shift_p/L} \Refl\Shift_{\shift_p/L} u(x,\period{p})\continue
%                                                        & = & \Refl^2 u(x,\period{p}/2) = u(x,\period{p})\,,
%\eea
%which, using \refeq{KSpos}, gives $u(x,0)= u(x,\period{p})$.
\ES{ I've changed the derivation because I couln't understand it. It seems
to start from what we need to prove,
\begin{eqnarray*}
...
RLD: I think either way is fine.  I just wanted to have a one-liner that
would start with $u(x,\period{p})$ and end with $u(x,0)$.  Of course, to get that
I have implicitly used $(\Refl\Shift_{\shift_p/L})^2 = 1$ to transform
$\Refl\Shift_{\shift_p/L} u(x,\period{p}) = u(x,0)$ into
$u(x,\period{p}) = \Refl\Shift_{\shift_p/L} u(x,0)$.  What if we just say:
``Indeed, since $(\Refl\Shift_{\shift_p/L})^2 = \Refl^2 = 1$ and the KS solutions
are time translation invariant, it is easy to see
from \refeq{KSpos} that
\[
...
\]
''
ES: OK, I get it now. As a one-liner it's fine.
   }
\PC{
back to the drawing board (for those who would rather not
   read the trace formula for continuous symmetries paper):

-- RED ALARM: while I was not watching, you redefined
   the prime period $T_p$ of the relative periodic orbit
   (non-rational shift $\shift_p$) in (2.21) to mean the full
   length of the periodic orbit. This is wrong.
   Relative periodic orbits may be preperiodic to
   a periodic orbit generated by a discrete symmetry,
   or not. The prime period $T_p$ refers to the shortest
   time a trajectory reaches a point that is equivalent to
   the starting point under a symmetry group shift, as in
   (2.21). Please use the shorter version of the derivation
   of (2.23) -- why is (2.21) written 2nd time as (2.22)?
   and restore $T_p/2 \to T_p$ everywhere, including unnumbered
  (why?) equations in sect 7.
-- we have to rephrase/recompute Fig 6.1 and 6.2, and wherever
   these preperiodic orbits appear: in Fig 6.2 (i)-(l):
   for all you have to state 1/2 of the $T_p$ listed now,
   and the corresponding shift reached in 1/2 of what you call
   periodic orbit.
   }
