...
\beq
  (\mathbf{g}(d) a)_k= e^{ik\, \shift /\tildeL}a_k
...
\,.
\ee{KSshift}
This shift is generates the 2-elemnt discrete subgroup
$\{1,\Shift\}$ of the
...
\refeq{KSshift} by half a spatial period $L/2$.
In the Fourier representation \refeq{expan} this
...
\\
PC: do they really show that? I think they must be symmetric under both
{\Refl} and \Shift, otherwise they could be invariant under one,
but traveling, not invariant under the other one. 
\\ES: They provide an arguement for this which did not yet convinced me.
...
this does not refer to behaviour under symmetry
\\ES:
Thus when discussing \eqva\ this conversion is not
meaningful. PC: ``not meaningful?"
\\ES: What I mean is that if Greene and Kim are correct we would get the same eq
uilibria for both
...
for full space and $L=44.0$ for antisymmetric subspace to be completely differen
t.
Of course for $L=22.0$ the full space dynamics will be more rich than the dynami
cs in the
antisymmetric space, but I don't think this can be quantified by dividing $\tilde{L}$ by $2$.
        }.


\refRefs{Christiansen:97,Lan:Thesis} concentrated on
...
of such solutions will be discussed in \refsect{sec:L22}.
...
The \eqv\ condition for the {\KS} PDE \refeq{ks}
is the ODE
\beq
...
3-dim equation? \\
\PCedit{PC: now done - please recheck}}
...
\RLD{In reply to ES: As far as I undestand, this is simply 
the consequence of the fact that, due to the translational symmetry, 
solutions $u(x,t)$ and $u(x+d,t)$ are equivalent. So, it is 
expected that, within a bounded chaotic solution, 
a state $u(x,0)$ can occur again after time $T$, 
but with some shift $d$. Hence, possible existence of \rpo s...}
...
a solution of the system might posess.}  
