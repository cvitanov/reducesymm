...
\,.
\ee{FModInvSymm}
Reflection generates  the dihedral
subgroup $D_1 = \{1, \Refl\}$ of $O(2)$.
...
\end{align}
A continuous symmetry maps each
state
$u \in \bbU$ to a manifold of functions with identical dynamic behavior.
Relation
$\Refl^2 = 1$
...
\beq
    ...
    P^-=(\matId-\Refl)/2
\ee{P1P2proj}
are the symmetric/antisymmetric
...
Substituting $u=u^+ + u^-$ into the KS equation \refeq{KSO2}
and noting that the derivative
$\partial_x$ is odd under reflection, we have\rf{KNSks90}
\bea
\dot{u}^+ &=& - \frac{1}{2}(u^+u^-_x + u^-u^+_x )
                - u^+_{xx} - u^+_{xxxx}
    \continue
\dot{u}^- &=& - \frac{1}{2}(u^+u^+_x + u^-u^-_x )
                - u^-_{xx} - u^-_{xxxx}
...
\label{KSD1}
\eea
The antisymmetric $\bbU^-$ subspace is
an invariant subspace if $u^+ = 0$, but otherwise the
nonlinear terms in \refeq{KSD1} evolution
mix the two subspaces\rf{KNSks90}.
    \PC{
    why \refeq{KSD1}? (a) to clarify embedding of
    $\bbU^-$ into $\bbU$
    (b) to explain the desymmetrization of evolution defined on
     $\Omega^{+} = [0, L/2]$ fundamental domain
    $\bbU^+ +  \bbUsymm$ space, with evolution at
    instant of crossing $\bbUsymm$ space given by reflection $\Refl$.
    }
...
within it. For example, the reflection
(For brevity, here $\Shift = \Shift_{1/2}$)
and 1/2-cell translation
...
at $\tildeL=22/2\pi =
3.5014\dots$, the {\eqva} are the constant solution
...
but with a shift $\shift_p$, and possibly also a reflection.
Due to the reflection symmetry \refeq{KSparity} of KS equation,
every \rpo\
$u(x,0) = u_p(x)$ with shift $\shift$ has a symmetric partner
$u(x,0) = -u_p(-x)$ with shift $-\shift$.

{\Rpo s} were introduced by Poincar\'e in his study of
the 3-body problem\rf{ChencinerLink,rtb}, where
the Lagrange points are the
\reqva.
