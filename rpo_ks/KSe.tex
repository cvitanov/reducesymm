...
Even though our starting point \refeq{ks}
...
together with its $\Shift_{1/2}$ shift within the
antisymmetric subspace.
As discussed earlier, for the $n$-cell \eqv the 
only non-vanishing Fourier coefficients
are $(a_n,a_{2n},\ldots)$.
As the system size $\tildeL$ increases, a rich sequence
...
summarized in \reffig{fig:ksBifDiag}.
...
and dashed curves the \REQV{\pm}{1},  \REQV{\pm}{2} \reqva.
...
$\Shift_{1/2} \Refl u(x,0) = -u(L/2-x,0)$.  \RLD{Why add $\Shift$?}
%    \RLD{
