...
Equation \refeq{ks} can be written in ``potential'' form
\beq
    u_t=E_x
		\,,\qquad
...
\beq
    \expct{E}=\frac{1}{L}\int_0^{L} E(x,t) \, dx
...
\beq
\expct{E} = (u,u) = \sum_{k=1}^{\infty} E_k
\,,\qquad 
E_k = % a_{-k} a_k =
	|a_k|^2
\,.
...
\beq
u^2-u_x- u_{xxx}=\expct{E}
\,,
\label{eq:stdks}
\eeq
where the integration constant
$\expct{E}$
is the mean energy \refeq{ksEnergy}.
...
\lim_{t\rightarrow \infty} {1\over t} \int_0^t d\tau \,
	\frac{1}{L}\int_0^{L} d\pSpace\, \obser(\pSpace,\tau) 
	\,.
\label{rpo:tim_ave}
\eeq
...
\beq
         \obser_p = \frac{1}{\period{p}} \int_0^{\period{p}}d\tau \,
	\expct{\obser}
\,.
\label{rpo:u-cyc}
\eeq
...
Take time derivative of the mean energy density \refeq{ksEnergy},
...
\beq
0= \expct{(u^3)_x} = 3 \expct{ u_x \, u^2}
\,,
\ee{EnNonl0}
...
\beq
	...
	\lim_{t\rightarrow \infty}
		{1\over t} \int_0^t d\tau \, \dot{\expct{E}}
=
...
\bea
\timeAver{E}_p &=& 
{1\over \period{p}} \int_0^\period{p}d\tau \, \expct{E}(\tau)
	\continue
\timeAver{(u_{x})^2}_p &=& 
