...
for KS.} %, with the `hyper-viscosity' $\nu$ fixed to $1$. Other
...
$\period{p}$.  Indeed, \ESedit{from time translational invariance
\beq
...
\eea
which, using \refeq{KSpos}, gives $u(x,0)= u(x,\period{p})$.\ES{ I've changed the derivation because I couln't understand it. It seems
...
\]
''
ES: OK, I get it now. If we need an one-liner it's fine (perharps also mentioning time-translational invariance).}
}
%    \RLD{
...
%PC: good idea
%    }
%in the next half period $\period{p}/2$ such orbit reverses its drift,
%$\Refl\tau_{\shift_p/L} u(-x-\shift_p,\period{p}) = u(x,0)$, and
