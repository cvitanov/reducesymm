...
\refeq{expan} the \Refl\ reflection flips the odd modes,
\beq
  \Refl \, a_k = -a_k^\ast
% a_{2m} \to a_{2m}
         \,,\qquad
...
$\Refl^2=\Shift_{1/2}^2=
(\Refl \, \Shift_{1/2})^2 = 1$
induce linear decompositions
of the space of solutions into
symmetry subspaces\rf{KNSks90}.
Consider the decomposition
$u(x) = u_+(x)+ u_-(x)$,
$u_+(x)= P_1 u(x)$, $u_-(x)= P_2 u(x)$, where
\beq
    \Refl_1=(\matId+\Refl)/2
    \,,\qquad
    \Refl_2=(\matId-\Refl)/2
...
Here we give a brief account of the emergence of \eqva\ and \reqva\
through a series of bifurcations as the system size $L$ increases
from zero.  In this we follow the the classical papers of
%
...
\RLD{Doesn't seem to belong here.} \RLDedit{What is new here is a
detailed exploration of the \eqva\ stable/unstable manifolds for a
specific system size $L = 22$, determination of a large number of
\rpo s, and a preliminary exploration of the relation between the
observed spatiotemporal `turbulent' patterns and the \rpo s.}

\RLD{Maybe find a different place for this paragraph -- somewhere in
the Introduction?}
%
\RLDedit{In the literature \refeq{ks} is sometimes written as
\beq
    u_t=(u^2)_x-u_{xx}- \nu \, u_{xxxx}
    \,,\qquad   x \in [0,L]
    \,,
\ee{ksVisc} or in any number of other variants, all equivalent. The
\KSe\ in  form \refeq{ks} is used, among others, by
\cite{cross93,Mks86,ks04com}. Here we use $L$ as the system
parameter, with the `hyper-viscosity' $\nu$ fixed to $1$. Other
authors vary  $\nu$, with $L$ fixed to either $1$ or $2\pi$.
\PC{remember to mention the scaling of time as well} In what follows
we shall state results of all calculations either in units of the
`dimensionless system size' $\tildeL$, or the system size $L = 2 \pi
\tildeL$. The parameter $\alpha$ of \refrefs{KNSks90,ksgreene88} is
in this convention is $\tildeL=\sqrt{\alpha/4}$. }


For small $\tildeL$ the only \eqv\ of the system is the constant
solution $u(x,t)=0$, denoted $\EQV{0}$ from now on. $\EQV{0}$ is
globally attracting for $\tildeL<1$.  At $\tildeL=n$,
$n=1,2,3,\dots$, the $n$-th harmonic becomes unstable and $\EQV{0}$
undergoes a pitchfork bifurcation into the antisymmetric `$n$-cell'
\eqv, together with its $\Shift_{1/2}$ shift within the
antisymmetric subspace.  As discussed earlier, for the $n$-cell \eqv
the only non-vanishing Fourier coefficients are
$(a_n,a_{2n},\ldots)$.  As the system size $\tildeL$ increases, a
rich sequence of bifurcations follows.  The resulting \eqva\ and
\reqva\ are identified in \refref{ksgreene88}. For the system size
$L=22$ investigated here, all relevant bifurcations are summarized
in \reffig{fig:ksBifDiag}.

\RLD{Do we need this paragraph?}\RLDedit{
% At point $C$ in \reffig{fig:GreeneKim}
At point $C$ in \reffig{fig:ksBifDiag} the $2$-cell state bifurcates
to a type of \eqv\ solution found by La Quey, Mahajan, Rutherford
and Tang\rf{laquey74} and generalized by Greene and Kim who refer to
them as GLMRT \eqva. For a GLMRT solution $u(x)$ is antisymmetric, a
long wavelength distorted $n$-cell state.}


According to the bifurcation diagram in \reffig{fig:ksBifDiag} and
our numerical results, at the system size $\tildeL=22/2\pi =
...
\end{center}
\caption{ The energy \refeq{ksEnergy} of all \eqva\ and \reqva\ that
