...
reflection (`parity' or `inversion') \beq \Refl u(x) = -u(-x)
\ee{KSparity} and the shift \beq \Shift_{1/2} u(x)=u(x+L/2) \,.
\ee{KSshift} This shift is generates the 2-element discrete subgroup
$\{1,\Shift_{1/2}\}$ of the above translational $O(2)$ invariance;
...
Relations $\Refl^2=\Shift^2=1$ induce a linear decomposition of the
space of solutions into 4 invariant subspaces\rf{KNSks90} via
...
the KS equation mixes these subspaces, leading, according to
\refref{KNSks90}, to four invariant subspaces (labeled by the
