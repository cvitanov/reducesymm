...
\\ES: They provide an argument for this which did not yet convinced me.
...
\,.
\]
To calculate the {\stabmat} we cannot directly differentiate
% \refeq{DerMatrix}
\refeq{expan} because the reality condition for $u(x,t)$ has not been taken into
account and we cannot vary $a_{k}$ independently of $a_{-k}$, since $a_{k}=a^*_{-k}$. 
On the other hand, substitution of the reality condition into \refeq{expan} introduces the non-differentiable
complex conjugation function into our equations. To proceed, we split \refeq{expan} in real and imaginary parts by
writing $a_k=b_k+i\, c_k$ to get, for $k,j>0$:

% \index{Kuramoto-Sivashinsky system}
\bea
   	\frac{\partial \dot{c}_k}{\partial c_{j}} & = &  \left(\frac{k}{\tildeL}\right)^2\left(1- \left(k/\tildeL\right)^2 \right)\delta_{kj} - \frac{k}{\tildeL} (c_{k+j}-c_{k-j}) \,, \\
	\frac{\partial \dot{c}_k}{\partial b_{j}} & = &  - \frac{k}{\tildeL} ( b_{k+j}+b_{k-j} )\,, \\
	\frac{\partial \dot{b}_k}{\partial b_{j}} & = & \left(\frac{k}{\tildeL}\right)^2\left(1- \left(k/\tildeL\right)^2 \right)\delta_{kj} +  \frac{k}{\tildeL} (c_{k+j} + c_{k-j}) \,, \\
	\frac{\partial \dot{b}_k}{\partial c_{j}} & = & - \frac{k}{\tildeL} (b_{k+j}-b_{k-j}) \,.
\label{expanMvar}
\eea
