...
\reffig{f:rpo55}.
 ...
 to $2\period{p}$ \po. \\
 ES: Because the shift 10.96 is so close to L/2, the trajectory in state
 space looks like almost retracing itself so the idea of simplification
 was not illustrated. The numerics are good far beyond the second decimal
 figure and there is no reason not to expect rpo's with shift close to L/2,
 so I am not in doubt it is an rpo.
   }
