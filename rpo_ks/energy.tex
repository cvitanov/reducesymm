...
The mean value
% $\timeAver{\obser}$,
of $\obser = \obser(u_q)$ evaluated on
...
\reqva\ follow\rf{Bridges_priv}. For example,
integrating by parts $\expct{u_{x} u_t}$
and
$\expct{u_{xx} u_t}$,
respectively, one obtains
\PC{please rederive these two}
\beq
% \expct{u_{x} u_t} 
%     \qquad \to \qquad
c= \frac{1}{P} \expct{u \, (u_{x})^2}
\ee{Bridges1}
\beq
% \expct{u_{xx} u_t} 
%     \qquad \to \qquad
{\textstyle\frac{1}{2}}\expct{(u_{x})^3}
	= P - \expct{(u_{xxx})^2}
\ee{Bridges2}
    } %end \PublicPrivate{%
...
can be misleading.
For example, \rpo\ $(\period{p},\shift_p) =(32.8,10.96)$
appears well embedded within the turbulent flow,
and the mean value of $\timeAver{P_p}$
evaluated on it,
\reffig{f:ks22rposShort}(c),
