...
around \ES{Corrected: We Taylor expand around guess point $a$ and not $a_p$ 
because this is what we know and we want to be able to write $J$ \etc\ in terms 
of known quantities. This makes $\delta a$ look awkward but there's no other 
way to do it.} $(a, \period{}, \shift)$ to linear order in
...
\eeq
where % dropped due to change in notation (Appendix on Stability): $\jMps^{(\period{},\shift)}_{ij}
...
%        {1}-\mathbf{g}(\shift)\jMps(a,\period{})  & -\mathbf{g}(\shift)v(f^{\period{}}(a))
