...
by solving \refeq{eq:KStan} with initial conditions $b_j(0) = 1 + 0i$ and
$b_j(0) = 0 + 1i$, respectively, for $j = k$ and $b_j(0) = 0$ otherwise.

The stability of a \po\ with period $\period{p}$ is determined by the position
...
Using the real valued representation similar to that in \refeq{eq:FundMat},
...
\bea
    ...
    - \frac{q_k}{2} (a_{I,k+j}+a_{I,k-j})\,,
\continue
    \frac{\partial v_{R,k}}{\partial a_{I,j}} & = &
    - \frac{q_k}{2} (a_{R,k+j}-a_{R,k-j})\,,
\label{expanMvar}\\
