...
\mathbb{C}$, \refeq{eq:KS} represents a system or ordinary differential
...
\beq
  ...
  iq_k{\cal F}_N[{\cal F}_N^{-1}[a]\otimes {\cal F}_N^{-1}[b]]\,,
\ee{eq:KStan}
...
derivatives of the flow map $f^t(a)$ with respect to $a$. Since $a$ and 
...
by solving \refeq{eq:KStan} with initial condition $b_j(0) = 1 + 0i$ and
$b_j(0) = 0 + 1i$, respectively, for $j = k$ and $b_j(0) = 0$ otherwise.

The stability of a \rpo is determined by the eigenvalues of the matrix
$\mathbf{g}(\shift_p)\,J(a_p,T_p)$.