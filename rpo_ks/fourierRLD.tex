...
The Jacobian of the KS flow map, $J(a,t) = \partial f^t(a)/\partial a$, where
the partial derivatives need to be evaluated separately with respect to the
real and imaginary parts of the compoments of complex-valued vector $a$.
...
In fact, if the symmetry is continuous, then it is much more likely to find 
\rpo s, than it is to find exact \po s, since $s = 0$, corresponding to the 
\po , is but one specific value in the continuum of possible values of 
parameter $s$.