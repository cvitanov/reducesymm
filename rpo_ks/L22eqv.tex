...
(a) \includegraphics[height=0.4\textwidth]{figs/ks22manifold1.eps}
(b) \includegraphics[width=0.4\textwidth]%,origin=c]%
...
The \rpo s satisfy the condition \refeq{KSrpos} $u(x+\shift,\period{}) = u(x,0)$,
...
50 \po s, all of which possess the symmetry
$u(x,\period{}/2) =
{\bf R} u(x,0) = -u(-x,0)$
of type (b) discussed in \refSect{sec:KSePO}.

The search has not been exhaustive, and there are likely to be more
orbits with $\period{} < 200$.
...
defined as
\beq
\PCedit{s_j} = \eigRe[j]/\period{} \,,
\ee{FloqExp}
where $\ExpaEig_j = e^{\eigRe[j] \pm i \eigIm[j]}$ are the
Floquet multipliers of $\jMps_p = g(\shift_p)J^\period{p}(a)$.
...
(compare it to the condition \refeq{eq:RPOcond} for \rpo s).
