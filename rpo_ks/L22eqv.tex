...
over 300 \PCedit{\rpo s} %(without reflection)
with $\shift_p > 0$.
...
The horizontal red line at $0.048$
indicates the numerical value  of the largest
Lyapunov exponent of the chaotic attractor.
} \label{f:ks22rposT}
\end{figure}

\PC{In principle, need to recompute pre-periodic orbits for
    \reffig{f:ks22rposT}\,(\textit{b}) because now these are plotted
    at $2\period{p}$. But it is not sufficiently important to delay
    submission. I now dropped ``\rpo s (black) and pre-periodic orbits
    with reflection (green)."
   }
...
\PC{dropped ``Next we describe several types of \rpo s."}
...
\PC{remember:
    ...
    to redraw the figures, pre-periodicity is obvious once
    $\shift_p$ is also stated
    }
\PC{
    What makes me uneasy is the high frequency of these solutions
    - Ruslan, can you run a long search for them for up to $T_p=200$?
    If computing power is the problem, Vaggelis can run it on
    the cluster we have access to. I would like to see whether they
    are as frequent as the card-carrying RPOs, and have similar unstable
    eigs/Lyapunovs and similar nos of unstable ones - that would be
    plotted color-coded in fig 6.1(b), caption for 6.1(a) should go
    back to the original Ruslan's text.

Wonder how this will show up in symbolic dynamics....
    }
We have found over 50
\PCedit{pre-periodic}
orbits with $\period{p} < 100$
...
which takes form
%RLD: changed \shift_p to \shift %PC OK
\beq
 -\mathbf{g}(\shift)a^\ast(\period{p}) = a(0)\,.
\label{KSposFour}
\eeq
\RLD{changed $\shift_p$ to $\shift$} \ES{Shouldn't we have $\mathbf{g}(-\shift)$?}
