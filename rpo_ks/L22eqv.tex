...
number of Fourier modes, \PCedit{typically 128 used in numerical calculations presented here},
suffices to obtain
numerically accurate (within $10^{-5}$) invariant
solutions.

We next investigate the properties of \eqva\ and \reqva\ and \PCedit{determine numerically}
...
and is bordered by the $\eigExp[1]$-PCedit{-eigendirection}\ES{dropped ``of the''}
...
%PC commented out: , especially with longer periods.
...
\end{center}
\caption{
(a) \Rpo s without (black) and with reflection (green)
of \KSe\ with periods $\period{p}$ and
shifts $\shift_p > 0$.
(b) The largest Lyapunov exponents \refeq{FloqExp} of all
\rpo s determined here.  The horizontal red line at $0.048$
indicates the numerical value  of the largest
Lyapunov exponent of the chaotic attractor.
} \label{f:ks22rposT}
\end{figure}

\PC{In \reffig{f:ks22rposT}\,(\textit{a}) plot also \rpo s of type
    \refeq{KSpos}, perhaps in green. Need to replot
    \reffig{f:ks22rposT}\,(\textit{b}) because now these are plotted
    at $2\period{p}$.
 
    RLD: Changed (b) to reflect change in definition of $\period{p}$.  
    Cannot add green dots in (a) because $\shift_p$ is not unique for POs
    (see my comment to \refSect{ssec:po})
   }
...
As \PCedit{was already the case for} the Lyapunov exponents
...
Out of the four \PCedit{leading} exponents, two equal zero, \PCedit{due} to
...
of the dynamics.
%PC commented out:  without significant modification of this structure.
...
unstable, with the largest Lyapunov exponent %, $s_1$,
in the range
...
\section{Preperiodic orbits} \label{ssec:po}
...
after a period: $u(x,\period{p})=-u(-x,0)$.  The dynamics of
...
\PC{
    In \reffig{f:ks22rpos}(\textit{i-l}) half the periods,
    state $\shift_p$'s. I think it is not necessary
    to redraw the figures, preperiodicity is obvious once
    $\shift_p$ is also stated
    }
\RLD{
...
We have found over 50 \rpo s with $\period{p} < 100$
which possess the symmetry
%$u(x,\period{p}/2) = R u(x,0) = -u(-x,0)$
of type {\bf (b)}. % discussed in \refSect{sec:KSePO}.
Some of the shortest such \rpo s we have found are shown in
\reffig{f:ks22rpos}(\textit{i-l}).
\PCedit{
Several were found as
periodic repeats of \rpo s during searches
for \rpo s with non-zero shifts,
while most have been
found as solutions of the relative periodicity
condition \refeq{KSpos} with reflection,
which takes form
\beq
 -\mathbf{g}(\shift_p)a^\ast(\period{p}) = a(0)\,.
\label{KSposFour}
\eeq
...
trajectory.
%current type figure, with chaotic trajectory,
%    all \eqva, $(32.\ldots,11)$ as example of well embedded, and
