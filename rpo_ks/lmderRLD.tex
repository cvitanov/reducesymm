...
\RLD{This trick was discovered by my PhD student Jonathan Crofts on 30
April 2007. PC: make bibtex entry for Croft's PhD thesis, refer to it
here}
...
its own minor drawbacks.  \PC{dropped: It would not be difficult to design more
...
require that the initial step does not exceed $h_0$, so we \PCedit{round off} the
...
%\ES{Should we mention $\lfloor\   \rfloor$ stands for floor function?} 
...
as discussed above.
    \RLD{
The drawback of this scheme is that the \KSe\ is integrated with
