...
% $Author: predrag $ $Date: 2020-07-29 14:32:00 -0500 (Wed, 29 Jul 2020) $
...
Bernoulli map problem as a \emph{temporal lattice} Bernoulli condition,
...
\refeq{tempBernFix} is the same for all lattice states of period $\cl{}$.
...
case, the $[2\!\times\!2]$ {\jacobianOrb} \refeq{tempBernFix}, the unit
...
shown in \reffig{fig:BernCyc2Jacob}, and $N_2=|\Det\jMorb|=3$,
...
vectors \jEigvec[j], each one given by a column of the {\jacobianOrb}
\beq
\jMorb = (\jEigvec[1]|\jEigvec[2]|\cdots|\jEigvec[\cl{}])
\,.
\ee{lattJac}
The {\HillDet} (`discriminant' or volume of the {\fundPip}) is
then
\beq
...
half-open, as indicated by dashed lines in \reffig{fig:BernCyc2Jacob}, so
that their translates form a partition of the whole space.
\refFig{fig:catCycJacob} is another example of a
{\fundPip}.
...
from \refeq{tempBernFix} it follows that
\[
...
theory\rf{CBgetused} this principle is stated as a
...
corrections\rf{CBcount} for orbits of periods $k>\cl{}$ vanish, leading
...
$\jMorb$ to obtain the Green's function \refeq{tempBernFix}.
