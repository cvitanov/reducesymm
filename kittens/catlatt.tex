...
The observation that for {\spt}ly chaotic systems space and time
should be considered on the same footing goes back to the `chronotopic' formulation
of Politi and
collaborators\rf{LePoTo96, LePoTo97, PoToLe98, GiLePo95} who, in their studies
...
%
Lattice recurrence relations\rf{MraRin12} arise in the
Frenkel-Kontorova lattice, Hamiltonian lattice models for ferromagnetism,
many-body quantum chaos\rf{SieRic01,MuHeBrHaAl04,
...
and $\period{}$ the `temporal' lattice period.
    \PC{2020-02-23} { %Woods\rf{Woods12}:
...
A given Bravais lattice $\Lambda$  can be tiled by any of the infinity of
...
Bravais cell of the {Hermite normal form}\rf{Cohen93}, which, for a
...
\refeq{Hermite2d} and the corresponding Bravais lattice
...
\beq
\mathbf{a}_2\times\mathbf{a}^p_{2}=\slant{}\period{p}-\period{}\slant{p}
\ee{primeTiling}
...
\tzeta s (see \refsect{s:bernZeta} and \ref{s:tempCatZeta}).

So far we have taken care only of the invariance under shifts in time and
space directions. To correctly define  a {\em prime}
{\twot}\rf{DasBuchMirror}, we also have take care of invariance of
\catlatt\ equations \refeq{CatMap2d} under the space $\sigma_{x}$ and
...
While the {\catlatt} equations are invariant under the point group
\refeq{eq:C4v}, the individual \catlatt\ \twots\
either have no symmetry at all (they are, after all, `turbulent'),
or are invariant
under subgroups of space group $p4mm$. In what follows we
% shall use the standard crystallographic methods to describe \catlatt\ in
% its first Brillouin zone by
quotient only the translational symmetries, and postpone dazzling the
captive reader with the full \Dn{4} point group reduction to a later,
more ponderous publication.
...
\BravCell{\speriod{}}{1}{\slant{}}
%$\BravCell{\speriod{}}{1}{0}_\slant{}$
