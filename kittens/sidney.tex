...
\item[2020-09-3 Sidney]
I was typing my description into "summary" textbox above the commit to master button. Obviously I was incorrect, I'll try to type in the "description" for this commit. 
\begin{itemize}
	\item[A5.3 Sidney]
I'm not actually quite sure what's wrong with my given definition. From your previous comment (A5) it seems that $\textbf{M}$ is a set of coordinates (the location of the blade of grass) and $\textbf{\Phi}_M$ is the value at that coordinate (the height of the grass at that point). Perhaps I forgot that these lattice states are for periodic orbits, so I forgot the second coordinate (period of length n).
	\item[A8.1]
