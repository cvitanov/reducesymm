...
For the Bernoulli map its stretching uniformity allows the use of combinatorial methods for lattice points. For temporal (not spatiotemporal) the number of periodic lattice states is the same as the volume of the fundamental parallelpiped, so the magnitude of the determinant of the orbit Jacobian. The block $\textbf{M}$ can be used as a 2D symbolic representation of the lattice system state. For a given admissible source block $\textbf{M}$, the periodic field can be computed by:
$$\textbf{\phi}_{i_1j_1}=\sum_{i_2=0}^2\sum_{j_2=0}^1\textbf{g}_{i_1j_1,i_2j_2}\textbf{M}_{i_2j_2}$$
\end{description}
