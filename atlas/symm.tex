...
(d)
 ...
 \caption[$\On{2}_\theta \times \SOn{2}_z$ symmetry of flow in a stream-wise
          periodic pipe]{
\DBedit{A fluid state in a stream-wise periodic pipe translated or rotated is
...
downstream with constant {\phaseVel} $\velRel$. (from \wwwcb{})} %DB 4/13/2012: Think this edit makes caption easier to read
...
\DBedit{Solutions of pipe flow remain physically the same under azimuthal rotations and stream-wise
translations (which become \SOn{2} rotations in numerical stream-wise
periodic pipes) but correspond to different points in \statesp\ and may look very different in visualizations.} %DB 4/13/2012: What was here previously wasn't a sentence, it was more like a thought...}
...
\DBedit{The strange attractor of the \cLf\ is shown in
...
flows settle into solutions with low friction. Since drifting along 
the non-shape-changing symmetry directions is energetically cheap, 
rather than do work, solutions tend to drift along. Thus, the physically
important shape-changing dynamics are smeared out.} %DB 4/13/2012: I think this is the general idea, but not sure how much of a claim this is
...
whose velocity field lies within the group tangent space, such that
\(
...
\DBedit{Evolution in time decomposes the \statesp\ into a spaghetti of 1\dmn\
trajectories $\ssp(\zeit)$, each determined by a single point $\ssp(0)$
on it. Additional continuous symmetries stratify the \statesp\ into an onion, each 
layer a group orbit (\reffig{fig:BeThMovFr}). Taking a section of codimension 1 allows 
us to `quotient' the continuous time parameter $\zeit$. For unstable trajectories one needs 
a set of short-time recurrence separated points $\{\sspRed_n\} = \{\ssp(\zeit_n)\}$, 
which can be used to capture a reduced (but complete!) representation of the chaotic flow.

But what about the additional symmetries? We are free to replace a given flow 
