...
\PCedit{symmetry-reduced block-diagonal, irreducible form;}
...
case, the 1-parameter compact continuous group \SOn{2} symmetry (also
known as the `circle group' $S^1$ and as \Un{1}). Our tool of choice is
the linear implementation of the
\mslices.\rf{rowley_reconstruction_2000,BeTh04,SiCvi10,FrCv11} Here, we
extend}
this local method to a global reduction of a turbulent flow by defining
local `charts', their borders, and the ridges that glue these linear tiles
into an atlas that spans the ergodic \statesp\ region of
interest.
\PCedit{While `charts' and 'atlases' are standard tools in geometry, the
algorithm for explicit construction of a symmetry-reduced \statesp\
presented here is, to our knowledge, new.}
We strive to explain the key geometrical ideas in simple but
...
~~(b)
  \begin{picture}(1,0.9071451)%
    \put(0,0){\includegraphics[width=\unitlength]{A27traj.pdf}}%
...
(c)
    ...
    \put(0,0){\includegraphics[width=\unitlength]{A27gOrbit.pdf}}%
...
~~(d)
    ...
    \put(0,0){\includegraphics[width=\unitlength]{A27wurst.pdf}}%
...
symmetry shifts, tangent to the \edit{$N$-dimensional} group orbit
$\pS_\ssp$.
    (b)
\edit{Each point has a unique trajectory (blue) under time evolution.}
    (c)
\edit{Each point also belongs to a group orbit (green) of
...
lie far from each other in \statesp.}
    (d)
\edit{Together, time-evolution and group actions trace out a \wurst\ of
...
\PCedit{for example,} might be a distance between digitized
...
papyrus. \DBedit{But what about for hydrodynamic flows where the dimensionality D of the \statesp\ is very large? The point of the
cartographical enterprise undertaken here is that while it is impossible to
visualize the $(D\!-\!2)$
\dmn\ {\poincBord} of the $(D\!-\!1)$
...
(d)
 ...
 \caption[$\On{2}_\theta \times \SOn{2}_z$ symmetry of flow in a stream-wise
          periodic pipe]{
\PCedit{
Symmetry equivalent states: a pipe flow solution translated or rotated is
also a solution.
(a) An instantaneous state of the  fluid  is indicated by a `swirl' -
here the reader has to imagine a particular disposition of instantaneous
velocity fields across the entire pipe. The same state rigidly
...
(c) translated by $\shift$ and rotated azimuthally by $\gSpace$
    (the state is $\SOn{2}_\gSpace \times \SOn{2}_z$ equivariant), and
(d) reflected and rotated azimuthally by $\gSpace$
    (the state is $\On{2}_\gSpace$ equivariant).
Symmetry equivalent states and time evolution:
A \emph{\reqv} is a solution of equations of motions that retains its
    shape while rotating and traveling downstream with constant
    {\phaseVel} $\velRel$.
A \emph{\rpo} $\pS_p$ is a \emph{time dependent}, shape-changing
    state of the fluid that after a period $\period{p}$ reemerges as (c),
    the initial state translated by $\shift_p$ and rotated by $\gSpace_p$.}
% (from \wwwcb{}).
...
Suppose \edit{you are computing a set of} numerically exact invariant solutions
...
The \chartBord\ ${\cal}$ is the \DBedit{$(D\!-\!N\!-1)$\dmn\ } hyperplane that contains
...
subspaces are high-dimensional and have their own dynamics.
\edit{Physicists, such as general relativists, often work in invariant
...
initial chart $\pSRed{}^{(1)}$ is a \DBedit{($D\!-\!N$)\dmn\ } hyperplane. If we
pick another {\template} point $\slicep{}^{(2)}$, it comes along with its
own \slice\ hyperplane $\pSRed{}^{(2)}$. Any pair of \DBedit{$(D\!-\!N)$\dmn\ }
local \slice\ hyperplanes intersects in a {ridge}, a \DBedit{$(D\!-\!N-\!1)$\dmn\ }
...
and it would be nice to use tools of differential geometry, but this is 
not possible in the high-dimensional \statesp\ of hydrodynamics turbulence. 
The only feasible way to chart this space is to (1) quotient all continuous 
symmetries, and (2) tile it with flat \DBedit{$(D\!-\!N)$\dmn\ } tiles, 
or charts. We do it step by step, starting with a set of \template s and using 
...
possible to compute 60,000b\rpo s this way.\rf{SCD07} Once we have more
...
replace the \DBedit{large number of} equivariant \statesp\ coordinates of hydrodynamic
...
{\phaseVel}, the method of connections is inherently local. The two
...
as the \NSe) does closely parallel the reduction of gauge symmetry in
