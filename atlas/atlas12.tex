...
\edit{Here, we extend} this local method
to a global reduction of a turbulent flow by defining
...
the onset of turbulence. Physicists love to exploit symmetries,\rf{Kerswell12} but
nature often ignores them. While the flow equations may be invariant under $\Group$, turbulent solutions are not. Furthermore, the highly symmetric solutions may lie far from the
regions of \statesp\ explored by turbulence\rf{ACHKW11} and thus be of limited usefulness in understanding its dynamics.}
...
cartographical enterprize undertaken here is that while it is impossible to
...
\SOn{2} symmetries of numerical pipe flow sweep out contorted and hard to visualize $T^2$ tori
...
The \chartBord\ ${\cal}$ is the \KCedit{$(d\!-\!N\!-1)$\dmn\ } hyperplane that contains
...
subspaces are high-dimensional and have their own dynamics. \DBedit{General
relativists and other physicists often work in invariant subspaces, as this is easier than solving the full problem.\rf{SCD07} This approach results in highly symmetric solutions,\rf{SKMacHH09,HGC08} whose dynamics may be quite different from those that guide turbulence in the full \statesp\ (for a striking example, see \refref{ACHKW11}).}

There is yet another, much kinder type of a border: a \emph{ridge}. Our
initial chart $\pSRed{}^{(1)}$ is a \KCedit{($d\!-\!N$)\dmn\ } hyperplane. If we
pick another {\template} point $\slicep{}^{(2)}$, it comes along with its
own \slice\ hyperplane $\pSRed{}^{(2)}$. Any pair of \KCedit{$(d\!-\!N)$\dmn\ }
local \slice\ hyperplanes intersects in a {ridge}, a \KCedit{$(d\!-\!N-\!1)$\dmn\ }
...
\KCedit{$(61,506\!-\!N)$\dmn\ } tiles, or charts. We do it step by step,
