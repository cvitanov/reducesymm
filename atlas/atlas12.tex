...
the two templates (\reffig{fig:RoessFarEq}\,(b)). Choosing sections is a
...
(c)
  ...
  outer {\eqv} $\slicep{}^{(+)}$  and its unstable eigenvector.  Note the
  ridge (solid line): the chart $\PoincS_{+}$ stops at the ridge, and it
...
(d)
  ...
  the two sections, indicated by the solid line in the three figures)
...
\begin{figure}
  ...
  \CLf, $d=5 \to 3$~dimensions $\{x_1,x_2,z\}$ projections:
  (a)
...
Symmetry-reduced \cLf, $d=4 \to 3$~dimensions $\{x_2,y_2,z\}$ projections:
(a)
Strange attractor of \reffig{fig:CLf01group}\,(a) reduced to a
single \slice\ hyperplane, using as the \template\ the \reqv\ $\slicep =
\ssp_{\REQV{}{1}}$. The dynamics exhibits singular jumps due to
