...
We \ESedit{extend here} this local method
...
are not. Furthermore, the highly symmetric solutions often lie far from the
regions of \statesp\ explored by turbulence.\rf{ACHKW11}}
...
relativists and other physicists love to work in invariant subspaces
where solutions are highly
symmetric,\rf{SKMacHH09,HGC08} as that is easier
than solving the problem that needs to be solved.\rf{SCD07}
Unfortunately, the dynamics within an invariant subspace often has little, if anything, to do with the turbulence in the full \statesp\ (for
...
nonlinear flows.
\ESedit{Note that, while in the method of co-moving frames
one defines a mean phase velocity, the method
of connections is inherently local (the two methods coincide for 
relative equilibria).
}
