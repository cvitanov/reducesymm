...
\,.
\) %\ee{PCsectQ1}
\index{slice} capturing the symmetry reduced dynamics.
...
\item[Ridge]
The intersection of a pair of \slice\ hyperplanes ($\pSRed{}^{(1)}$ and $\pSRed{}^{(2)}$) for which any point $\hat{x}*$ satisfies
\(
\braket{\sspRed^*}{\sliceTan{}{}^{(1)}} =
\braket{\sspRed^*}{\sliceTan{}{}^{(2)}} = 0
    \,,
\) %ee{ridge}
, i.e., a toll bridge crossed by any transit from a chart to the next chart.
