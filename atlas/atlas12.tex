...
the first determination of \rpo s that help shape the turbulence observed
...
transformations traces out a {group orbit} %(or, from now on, just \emph{orbit}) DB 4/17: I don't think we ever refer to the group orbit in the paper as just an orbit)
\(
...
set of nearby states with qualitatively similar dynamics. Our main task in what follows will be to make this
...
{points} $\slicep{}^{(j)}$, $j=1,2,\cdots$ in the \statesp\ that
are the most dynamically important and the most frequently revisited features of the
flow.
...
(ii) In \refsect{s:symm}, we discuss the effect of continuous symmetries on
    nonlinear flows, and \DBedit{in \refsect{s:slice} we use the lessons learned from
    our discussion of \PoincSec s to aid us in the reduction of continuous symmetries, and, thus, enable us to
    commence a systematic charting of the long-time dynamics of high-dimensional flows (\refsect{s:chart})}.
