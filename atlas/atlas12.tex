...
A \PoincSec\ is {not} a projection onto a lower-dimensional space (in
sense that a photograph is a 2-dimensional projection of a 3-dimensional
...
A \emph{\reqv} is a solution of equations of motion that retains its
    shape while rotating and traveling downstream with constant
    {\phaseVel} $\velRel$.
A \emph{\rpo} $\pS_p$ is a \emph{time dependent}, shape-changing
    state of the fluid that after a period $\period{p}$ reemerges as (c) or (d),
...
\eea
\edit{
Here all coordinates and parameters are real, and in our calculations}
the parameters are set to Siminos' values\rf{SiminosThesis} $\RerCLor=28,\,
\ImrCLor=0,\, b=8/3,\, \sigma=10,\, e= 1/10$.
\edit{
The \cLe\ are a simple example of a dynamical system with a continuous
...
(d)~~~~
 ...
 \end{center}
  \caption{
(a)
The group orbit $\pS_{\ssp(0)}$ of \statesp\ point $\ssp(0)$
\edit{(an $N$\dmn\ manifold through $\ssp(0)$)}, and the
...
(c)
  \edit{A symmetry reduction scheme $\pS \to \pSRed$ is a rule that
  prescribes $\LieEl(\zeit)$ and thus} replaces a group orbit
  $\pS_{\ssp}\subset\pS$ through \ssp\ by a single point $\sspRed \in
  \pSRed$.
(d)
  \edit{In this paper $\LieEl(\zeit)$ is fixed variationally, by the
...
\,\slicep}$ vanishes \edit{(to streamline the exposition we have assumed here
$\SOn{n}$ symmetry, so $\Lg$ is antisymmetric)}. Therefore  $\sspRed$, the
...
\Wurst, sliced.
      ...
      of the same \rpo, the good close one $\sspRed_p$, and the bad distant one, on
...
replace the \edit{large number of} equivariant \statesp\ coordinates of
...
\edit{$\velRel_p = \gSpace_p/\period{p}$}, and there is no single
...
\edit{
Method of connections, however, does not reduce the dynamics to a
