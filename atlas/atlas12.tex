...
Everybody encounters a symmetry sooner or later, so the literature on symmetry
...
common: they will not work for high-dimensional systems. %DB: I think original phrasing is a little too aggressive. They work where they work.
...
of singularities, and it is not altogether clear how one would rewrite the
\NSe\ in such a format, or integrate them numerically. For pipe flows, one
would need cylindrical Bessel functions eigenbases, which are never used in
numerical work. A more sophisticated  approach would be to rewrite the dynamics in terms of
...
$(u_1,u_2,u_3,...,u_m)$. As the dimension of the problem increases, the number of these
...
scheme\rf{gatermannHab} for high-dimensional flows. It is unclear how and why one would
replace the 61,506 equivariant \statesp\ coordinates of hydrodynamic turbulence with a vast number of
...
mechanics\rf{MaWe74} or using Lie symmetry reduction to rederive Eulerian
...
We are indebted to
S.~Froehlich,
%E.~Siminos, DB: Removed Siminos as he is now a co-author
S.A.~Solla,
and
R.~Wilczak
