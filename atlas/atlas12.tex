...
\begin{quotation}
\DBedit{Today, it is possible to take a stroll through 61,506\dmn\ \statesp\ of
...
breaks all symmetries, and for nonlinear systems, rather than simplifying 
our task, symmetries obscure the essential dynamics. While evolution in
time decomposes the \statesp\ into a spaghetti of time trajectories,
continuous spatial symmetries stratify it like the layers of an onion.
In this geometrical tour of dynamics, we unravel this mess and pick a single
representative point for each trajectory (section it) and  group orbit 
(slice it). Once the fog of symmetries is out of the way, one can identify and
...
are the dynamically most important, frequently revisited features of the
flow.
...
a fixed oriented hypersurface $\PoincS$ of codimension 1. For
...
\emph{section}) we shall consider here. Such a section captures important
features of the flow in a neighborhood of a given \template.
...
eigenvector of $\slicep{}^{(+)}$ (\reffig{fig:RoessFarEq}\,(c)), thus
...
between the two templates (\reffig{fig:RoessFarEq}\,(d)). Such a choice
...
(c)
  ...
  indicated by (yellow) vector. Note the ridge (brown line): the chart
...
about the chaotic flow. We can thus chart the regions of \statesp\ of
interest, by a picking a sufficient number of \template s and the
associated charts over their neighborhoods, each bounded by \poincBord s
and ridges.
...
representing a trajectory by a sequence of time-integration step
separated points, this is no reduction of a flow to a codimension 1
...
close to the \template, the group tangent vector $t(\hat{x}')$ lies in
...
the \poincBord\ \refeq{eq:sspRSing} for evolution in time. This is also a
linear condition, and it defines the \chartBord\ ${\cal
S}$,\rf{SiCvi10,FrCv11} a hyperplane in which both points $\sspRSing$ and
their group tangents $\groupTan(\sspRSing)$ lie in the \slice\
hyperplane,
\beq
...
\eeq
    \PCedit{
or vanish, $\groupTan(\sspRSing)=0$. While the analogous points for
section \refeq{eq:sspRSing} were \eqva, captured only it the section cut
through them, vanishing of a group action implies that $\sspRSing$
belongs to an invariant subspace, and -comforting to know- by its
definition every {\chartBord} automatically includes
all invariant subspaces. For \cLe\ \refeq{eq:CLeR} the invariant subspace
is the 1\dmn\ $z$-axis, with trivial dynamics, $z=-bz$, but in general
they are high-dimensional spaces, with their own dynamics. General
relativists and physicists in general love to study
them,\rf{SKMacHH09,Christiansen97,lanCvit07} as that is easier than doing
the real problem.\rf{SCD07} It was shown in \refref{ACHKW11} that the
dynamics within an invariant subspace has very little to do, if anything,
with the turbulence in the full \statesp.
    }

There is yet another, much kinder kind of a border: a ridge. Our initial
...
remains within a flow-invariant subspace for all times. By its
definition, every \chartBord\ automatically includes all invariant
subspaces.
        }
...
solutions. The \slice\ that we construct here is \emph{emphatically}
stationary.
...
considered here, and anyway, our goal is different. Rather than to reduce
