...
that the Hamiltonian has been brought to the block-diagonal, irreducible
rep form, anything else would be sheer masochism. Dynamical theory of
turbulence is in its infancy, and this is not yet a practice for
nonlinear numerical simulations and experimental data collection of
systems such as fluid turbulence. We show here how to bring the numerical
/ experimental data to a symmetry-reduced format, \emph{before} any
analysis of takes place.}
...
between the two templates (\reffig{fig:RoessFarEq}\,(b)).
    \PCedit{
Choices of sections is a dark art: in the example at hand the dynamics of
interest is well captured by these two charts - if that were not the
case, one might had to interpolate, by inserting another chart between
the two constructed so far.
    }
    \DB{2012-04-17}{Not sure this last bit about the choice of
    orientation is convincing. See blog for details. {\bf Predrag}: is it
    better now?}
