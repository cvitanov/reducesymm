...
`template'. Together these charts provide an atlas of a slice -a finite
...
between the two templates (\reffig{fig:RoessFarEq}\,(b)). Such a choice guarantees that both neighborhoods will be well represented.
...
<<<<<<< .mine
The strange attractor of the \cLf\ \reffig{fig:CLf01group}\,(a), in its present state, is a complete mess! Without a proper account of the symmetries at hand we merely observe the flows settling into solutions with low friction. Since drifting along the
non-shape-changing symmetry directions is energetically cheap, solutions tend to drift along rather than dance into far-reaching neighborhoods. Thus, the physically
important shape-changing dynamics are smeared out in favor of a ``downhill drift'' along the symmetry. Yet embedded within the drifters lie a multitude of unique, symmetry respecting trajectories.
...
point on the group orbit? No, you would like to compute it only once. The strategy for pickout out that one solution is called \emph{symmetry
...
(a)
Chart $\pSRed \supset \pS/\Group$ lies in the $(d\!-\!N)$\dmn\
...
61,506~dimensions. The only feaasible way to chart this space is to (1)
quotient all continuous symmetries, and (2) tile it with
$(61,506-N-1)$\dmn\ tiles, or `charts'. Here is the tool kit:

\begin{description}

\item[\Template]
Pick a {\template} $\slicep$ such that $\Group$ acts on it
regularly, with group orbit is of dimension $N$.
...
\item[Moving frame]
For any $\ssp$, the \slice\ condition $\braket{\sspRed}{\sliceTan{}}=0$ on $\ssp =
\LieEl(\gSpace)\sspRed$ determines the moving frame, \ie, the group
action $\LieEl(\gSpace)$ that brings $ \ssp$ into the \slice\ hyperplane.
\index{moving frame}
...
intersects in a {ridge}, a hyperplane $\PoincS{}^{(12)}$ shared by a pair
of charts,
\(
...
\braket{\sspRed^*}{\sliceTan{}{}^{(2)}} = 0
    \,,
\) %ee{ridge}
a toll bridge crossed by any transit from a chart to the next chart.

\item[Chart]
The neighborhood of a \template\ $\slicep{}^{(j)}$, bounded by the
{\chartBord} and the ridges to other such linear neighborhoods, comprises
a \emph{chart} $\pSRed{}^{(1)} \supset \pS/\Group$. The borders ensure
