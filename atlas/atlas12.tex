...
\newcommand{\version}{atlas ver. 1.0, Apr 21 2012}
...
`template'. Together these charts provide an atlas of the symmetry-reduced \emph{slice} of \statesp \DB{04-23}{At this point in reading the paper, people will have no idea what a slice is}, charting
...
(a)
The stable manifold of the inner {\eqv} $\slicep{}^{(-)}$  is
1-dimensional and the unstable manifold is a spiral-out focus. For the
outer {\eqv} $\slicep{}^{(+)}$,  the stable manifold is a spiral-in focus
(basin boundary for initial conditions that either fall into the chaotic
attractor, or escape to infinity) and the unstable manifold is
1-dimensional.
(b)
...
(d)
  ...
  the two sections, indicated by the dashed blue line in the three figures)
  lies approximately between the \template s. Section hyperplanes
...
extreme contraction rate of the attractor, the section in
...
chaotic flow. We can thus chart interesting regions of \statesp\ by a
...
\label{eq:CLeR}
\eea
The parameters are set to Siminos's values\rf{SiminosThesis} $\RerCLor=28,\,
...
(d) \begin{picture}(1,0.87317006)%
  ...
  trajectory (dark blue) is shown by the cyan surface. Following $\cycle{01}$ for 15 more periods (faint dotted lines) starts filling out this
...
Symmetry-reduced \cLf, $d=4 \to 3$~dimensional $\{x_2,y_2,z\}$ projections:
 (c)
 Strange attractor (a) reduced to a single \slice\ hyperplane, using as
 the \template\ the \reqv\ $\slicep = \ssp_{\REQV{}{1}}$. The dynamics
 exhibits singular jumps due to forbidden crossings of the \chartBord. In
 contrast to the 1\dmn\ \poincBord s of \reffig{fig:RoessTrjs}, here
 \chartBord s are 3\dmn\ and hard to visualize.
 (d)
...
singularities. The ridge acts as a \PoincSec\ $\PoincS$ with red or
...
elliptical in $d=5 \to 3$~dimensions projections. Nevertheless, even
...
symmetry transformations. $\Norm{\LieEl(\gSpace)\slicep}= \Norm{\slicep}$ is a constant,
...
than solving the problem that needs to be solved.\rf{SCD07} Unfortunately,
the dynamics within an invariant subspace has often very little to do, if
anything, with the turbulence in the full \statesp\ (for a striking
example, see \refref{ACHKW11}).

There is yet another, much kinder type of a border: a \emph{ridge}. Our initial
...
\slice\ hyperplanes intersects in a {ridge}, a $(d\!-\!2)$\dmn\
...
starting with a set of \template s and using them to construct charts of each neighborhood, and then building up an atlas of the \emph{\slice}, chart by
chart, which captures all of the reduced dynamics of interest (but not all possible dynamics). Here are the steps along
the way:
...
\DBedit{
\item[\ChartBord]
The set of points $\sspRSing$ on a \slice\ hyperplane whose group orbits tangentially graze the hyperplane, such that
...
The rest is geometry of hyperplanes and has nothing to do with dynamics. Group
orbits $\pSRed{}^{(j)}$, group tangents $\groupTan(\slicep{}^{(j)})$, and the
associated charts are purely group-theoretic concepts. The
\slice, \chartBord\ and ridge conditions \refeq{PCsectQ0},
\refeq{sliceSingl0} and \refeq{ridge} are all linear conditions which
...
and not [...]? a brief tour of the more familiar symmetry reduction schemes is called for. They all have
one thing in common: they will not work for high-dimensional nonlinear systems.
...
representations,\rf{PCgr} is only partially illuminating; linear theory works
quite well for linear unitary operators or close to a bifurcation, but,
as we tried to show in this pictorial tour, the way symmetries act on
nonlinear systems is much subtler. For flows with strongly nonlinearly
...
under \DBedit{$\SOn{n}$} transformations.
...
played any role our discussion of symmetry reduction. Conversely, most
...
solutions of a turbulent flow can be plotted together: all symmetry-equivalent states are represented by a single point,
...
D.B.\ was supported by NSF grant CBET-0853691.
\end{acknowledgments}
