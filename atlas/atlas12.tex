...
symmetry reduction.\rf{rowley_reconstruction_2000,BeTh04,SiCvi10,FrCv11}
    ...
    \PCedit{
Would a chemist set out to compute a spectrum of -let's say- helium,
without first making sure that the Hamiltonian has been brought to the
block-diagonal, irreducible rep form? That would be sheer masochism. Yet
it is a custom in nonlinear numerical simulations and experimental data
collection of system such as fluid turbulence to ignore symmetries. That
is unwise. We show here how to bring the numerical / experimental data to
a symmetry-reduced format, \emph{before} any analysis of takes place.}
...
\ee{innerproduct}
    \PCedit{
For experimental data, the norm might some smoothed distance between two
images used in pattern recognition.}
...
\emph{section}) that we shall consider here. One can choose a section such that it
contains a \template\ of interest. Properly oriented, such a section can capture important
features of the flow in the neighborhood of the section-fixing \template .

%DB 4/17: Final sentence of previous paragraph and first sentence of next paragraph basically said the same thing in original version.
...
guarantees that both neighborhoods will be well represented.\DB{2012-04-17}{Not sure this last
bit about the choice of orientation is convincing. See blog for details}
...
(c)\includegraphics[width=0.27\textwidth,clip=true]{RoessSctAtlas3}
\end{center}
...
a flow at a sequence of instants in time. \DBedit{While `strobing' is what
...
the \wurst\ traced out by the very simplest, short \rpo\ $\cycle{01}$\DB{2012-04-18}{Should we explain the naming of the cycles or at least point readers to the appropriate reference. Is that Siminos's thesis? DasBuch?} shown
...
are related, in general, by an \DBedit{arbitrary , DB:Are these shifts really arbitrary? I would think that they are actually specific for two GIVEN points on a group orbit. A single point is related to another non-specific point on its group orbit by an arbitrary shift.} time dependent {\em moving frame} transformation $\LieEl(\zeit)$, such that $\ssp(\zeit)=\LieEl(\zeit)\,\sspRed(\zeit)$.(from \wwwcb{}).
  }
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

To summarize: continuous symmetries in the dynamics stratify the \statesp\
into an onion, each layer a group orbit (\reffig{fig:BeThMovFr}). How are we to sort out this mess? Luckily, all the points on a group orbit are physically equivalent, so we are
...
now offer a pictorial tour of this %save for the one bold incursion,\rf{ACHKW11}, DB: Obviously other people have looked at the method of slices... we cite them!
hitherto uncharted territory. %uncharted: unexplored, unknown. unchartered: without a charter, lawless.

First, pick a \template\ $\slicep$ and use the freedom to shift and rotate it (\reffig{fig:BeThMovFr}\,(b)) 
...
\braket{\sspRed}{\sliceTan{}} = 0
    \,.
\ee{PCsectQ0}
\DBedit{As $\ssp$ varies in time, the {\template} $\slicep$ tracks the motion
...
$\{\gSpace(\zeit)\}$, as depicted in \reffig{fig:slice}\,{(a)}.
$\pSRed$ is thus a $(d\!-\!N)$\dmn\ hyperplane normal to
...
$N$\dmn\ manifold and, even for $\SOn{2}$, is usually only topologically
...
(a)
  \begin{picture}(1,0.87085079)%
    ...
    \put(0.00008985,0.37305068){\color[rgb]{0,0,0}\rotatebox{0.0313674}{\makebox(0,0)[lb]{\smash{$\ssp(\zeit)$}}}}%
...
\DBedit{As shown in \refref{DasBuch}\DB{04-19-2012}{Who actually showed this first... fix reference accordingly}, one can write the equations for the flow in the \reducedsp\ $\dot{\sspRed} =
\velRed(\sspRed)$ as
\bea
...
which confines the motion to the \slice\ hyperplane. Thus, the dynamical system $\{\pS,\map^t\}$ with continuous symmetry
\Group\ is replaced by the {\reducedsp} dynamics $\{\pSRed,\mapRed^t\}$;
The velocity in the full \statesp\ $\vel$ is the sum of $\velRed$, the
...
close to the \template, the group tangent vector \DBedit{$t(\sspRSing)$} lies in
...
the \poincBord\ \refeq{eq:sspRSing} for evolution in time. \DBedit{This is also a
linear condition and defines the \chartBord\ ${\cal
S}$,\rf{SiCvi10,FrCv11}, a $d-N-1$-dimensional manifold, which contains all the points $\sspRSing$ whose group tangents lie in the \slice\ hyperplace, i.e.,}
\beq
...
\eeq
    \PCedit{
\DBedit{${\cal S}$ also contains all points for which $\groupTan(\sspRSing)=0$. While in \PoincSec s \refeq{eq:sspRSing}the analogous points were \eqva (only captured only if the section cut
through them), points with vanishing group action belong to an invariant subspace, and -comforting to know- by
definition every {\chartBord} automatically includes \emph{all} invariant
subspaces.} 

While for the \cLe\ \refeq{eq:CLeR}, the invariant subspace is the 1\dmn\
$z$-axis, with trivial dynamics, $z=-bz$, in general they are
high-dimensional spaces, with their own dynamics. General relativists and
physicists in general love to work in invariant subspaces where solutions
are highly symmetric,\rf{SKMacHH09,Christiansen97,lanCvit07,HGC08} as
that is easier than doing the full problem\rf{SCD07}. However, the dynamics within an invariant subspace has very little to do,
if anything, with the turbulence in the full \statesp\ (for a striking
example, see \refref{ACHKW11}).
    }

\DBedit{DB: 04-19-2012: Consider moving this whole part on ridges and such to section \ref{s:chart}}
...
hyperplane {\PoincS} (drawn in \reffig{fig:A29-2slices}\,(c) as a `line'
and in \reffig{fig:A29-1ridge} as a `plane' of intersection of two
volumes) shared by a pair of charts and thus satisfies both \slice\
conditions \refeq{PCsectQ0} of the chart,
\beq
\DBedit{\braket{\sspRed^*}{\sliceTan{}{}^{(1)}} = 0}
\mbox{ and }
\DBedit{\braket{\sspRed^*}{\sliceTan{}{}^{(2)}} = 0}
    \,.
\ee{ridge}
...
to \refeq{sliceSingl0} and \refeq{ridge} as the \emph{border conditions}.

    \DBedit{DB 04-19-2012: Is fine but probably better in section \ref{s:chart}
...
(b) \begin{picture}(1,0.87317006)%
    \put(0,0){\includegraphics[width=1.2\unitlength]{CLE2slice2na}}%
