...
Despite this inconvenience, we can make way in unraveling the spaghetti of 1\dmn\ trajectories with the notion of recurrence. To quantify how
close the state of the system at its present state is to a previously visited state we
...
\,.
\ee{innerproduct}
Given a notion of distance we can talk about a 'neighborhood,' the open set of
nearby states. Our main task in what follows will be to make this precise by defining a chart over a neighborhood and its borders.
...
velocity either vanishes (\eqv) or is orthogonal to the section normal $\hat{n}$,
\beq
...
$\slicep{}^{(+)}$, which we pick as {\em \template s} (\refFig{fig:RoessTrjs}) for our sections.
...
(b) \includegraphics[width=0.16\textwidth]{CLE
 \end{center}
  \caption{\label{fig:cLe-2charts}
(a)
% \mycomment{\DBedit{\Huge [DRAW THIS!]}}{}
Strange attractor of the \cLe\ in a single \slice\ hyperplane, \template\
...
marked green and purple. The ridge can act as a \PoincSec\ with either
green or purple intersecting the chart.
  }
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
of singularities, and it is not altogether clear how would one rewrite
...
relations amongst them. There is no guiding principle for picking set of
...
There are dynamics informed approaches, foremost among the method of
{co-moving frames}. Visualizing a single `relative' trajectory in its
co-moving frame, moving with the mean {\phaseVel} of a given solution, is
useful if we are concerned with that individual solution, and the tiny
\rpo s (modulated-amplitude waves) that have bifurcated off
it.\rf{duguet08,mellibovsky11} A co-moving frame is useless if we are
...
There are sophisticated approaches to reduce symplectic symmetry of
...
equations, we seek to formulate a computationally straightforward general
...
Marsden\rf{Smale70I,AbrMars78,rowley_reduction_2003}, illustrated
...
Wilczek\rf{ShWi89a,ShWi06}: on can observe a swimmer (or our dancer) from
a fixed \slice\ frame, or bringing her back to observe only the
...
reduction at all and yield no insights into geometry of nonlinear flows.
...
\statesp s by integrating the associated {reconstruction equations} \refeq{reconstrEq} and \refeq{EqMotMFrame}.
