...
\ES{2012-04-24}{How do we enforce the specific section condition in this case? 
...
(a)
 \ESedit{The inner {\eqv} $\slicep{}^{(-)}$  is a (spiral-out) saddle-focus 
  with a 2-dimensional unstable manifold and a 1-dimensional stable manifold.
  The outer {\eqv} $\slicep{}^{(+)}$ is a (spiral-in) saddle-focus, 
  with a 2-dimensional stable manifold (basin boundary for initial conditions 
  that either fall into the  chaotic attractor, or escape to infinity) and
  a 1-dimensional unstable manifold.}
(b)
...
extreme contraction rate of the attractor, \ESedit{its intersection with} 
the section in 
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ES{2012-04-24}{I think we should, at least in an Appendix state which 
templates we use here, to comply with the usual reproducibility requirements.}
...
even the \wurst\ traced out by \ESedit{one of} the very simplest\ES{2012-04-24}{There's an 
even simpler $\cycle{1}$-cycle.}, short \rpo\
...
Suppose your day job is computing invariant solutions\ES{2012-04-24}{Would this cause confusion
to non-cyclists?} of the \NSe. Do you
...
(b)
  \begin{picture}(1,0.87085079)%
    ...
    \put(0.15576193,0.40769256){\color[rgb]{0,0,0}\rotatebox{0.0313674}{\makebox(0,0)[lb]{\smash{$\ssp(\period{\ESedit{p}})$}}}}%
...
\ssp(\period{\ESedit{p}})$ returns to the group orbit of $\ssp(0)$ after a time
...
includes \emph{all} invariant subspaces.\ES{2012-04-24}{A naive question:
...
initial chart $\pSRed{}^{(1)}$ is a ($d\!\ESedit{-\!N}-\!1$)\dmn\ hyperplane. If we
pick another {\template} point $\slicep{}^{(2)}$, it comes along with its
own \slice\ hyperplane $\pSRed{}^{(2)}$. Any pair of $(d\!\ESedit{-\!N}-\!1)$\dmn\
local \slice\ hyperplanes intersects in a {ridge}, a $(d\!\ESedit{-\!N}-\!2)$\dmn\
...
\ES{2011-04-24}{This seems to say that any flow invariant subspace is (pointwise) 
invariant under the group action, while only the oposite holds: any (pointwise)
group-invariant subspace is flow invariant. Also does this definition, based on
group tangent, cover discrete groups?}
...
borders\ES{2011-04-24}{I would say \emph{The ridges} instead of
\emph{The borders} which could mean \ChartBord s. Do I get it wrong?} 
ensure
...
a discrete sequence of points.\ES{2012-12-24}{I disagree with this statement,
