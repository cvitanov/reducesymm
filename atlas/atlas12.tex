...
\eea
\edit{
Here, and in our calculations, all coordinates and parameters are real.}
The parameters are set to those used by Siminos'\rf{SiminosThesis} $\RerCLor=28,\,
...
is a constant. \DBedit{For $\SOn{n}$ $\Lg$ is antisymmetric, so the group 
tangent vector $\sliceTan{}$ evaluated at $\slicep$ is normal to $\slicep$ 
and the term $\braket{\slicep}{\Lg\,\slicep}$ vanishes.}. Therefore  $\sspRed$, the
...
qualitatively captures neighboring group orbits until, for a point $\sspRSing$ 
not so close to the \template, the group tangent vector $t(\sspRSing)$ lies in
...
\begin{figure}
  ...
  \begin{picture}(1,0.65502164)%
    \put(0,0){\includegraphics[width=\unitlength]{A29sliceWurst.pdf}}%
