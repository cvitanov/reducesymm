...
\beq
   S = \{e,\sigma\LieEl_z\}
   % S = \{ e,\, \sigma\,\LieEl(0,L/2) \}
\ee{ShiftReflOnl}
    \PC{will explain here why this is physically wrong
    \citep{lanCvit07,SCD07}, but pedagogically right.}
\ie, velocity fields \refeq{pipeSymms} that satisfy
\(
[u,v,w,p](r,\theta,z) =
...
100,000$-dimensional {\statesp}) onto stationary Frenet-Serret frames
given by unit vectors in the directions
$\{\groupTan_z(\slicep),\groupTan_\theta(\slicep),\normVec_z(\slicep)\}$,
see \refeq{FrenetFrame}. The state in (a) is a lower-branch travelling
wave $N2\_LB$, whereas in (b) it is a snapshot from a turbulent run.
    \PCedit{
The group orbits are generated by all possible axial shifts
$\LieEl(0,\shift)\,\ssp$, and plotted relative to a template point
$\slicep$. In (a) both $\ssp$ and $\slicep=\ssp_\mathrm{N2\_LB}(0)$
belong to the ``lower-branch'' \reqv\ $\pS_\mathrm{N2\_LB}$ described in
\refsect{s:eqbSols}; in (b) $\ssp$ is a turbulent state and
$\slicep=\ssp_\mathrm{ML}$. Group orbits are only topologically circles;
for strongly nonlinear, turbulent states many Fourier modes are
of comparable magnitude, with their sums resulting in highly convoluted
group orbits such as (b).
  }
\end{figure}
