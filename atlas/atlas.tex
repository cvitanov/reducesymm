...
through 61,506. Charting this world is a geometer's task, and we will map
it using a measuring tape, but first one has to deal with symmetries.
...
symmetries, and rather than simplifying our task, for nonlinear dynamics
symmetries obscure the essential dynamics. Evolution in time decomposes
the \statesp\ into spaghetti of time trajectories, continuous spatial
symmetries stratify it into layers of an onion. There are many methods
exist to 'quotient out' symmetries in low-dimensional systems, but so far
only one that can be mustered for extremely high-dimensional flows of
turbulent fields. In this geometrical tour of dynamics we show you how to
cut this knot and pick a single point for each trajectory (section it)
and each group orbit (slice it). Once the fog of symmetries is out of the
way, one can identify and describe the prominent fluid structures by
...
describe the dynamics in terms of the their heteroclinic connections.
\end{quotation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
transformations. $\Norm{\LieEl(\gSpace)\slicep}$ is a constant, the group
tangent vector $\sliceTan{}$ evaluated at $\slicep$ \refeq{eq:tang} is
normal to $\slicep$, and the term $\braket{\slicep}{\Lg \,\slicep}$
vanishes ($\Lg$ is antisymmetric). Therefore  $\sspRed$, the point on the
group orbit of $\ssp$ that lands in the \slice, satisfies the
\emph{\slice\ condition}
\beq
...
the onion formed by group-orbits. Both are triggered by similar
conditions: oriented piercing of the section, piercing of the slice. Just
...
Once symmetry reduction is achieved, and all solutions of a turbulent
...
invariant solutions and their stable / unstable manifolds is now made
possible.
