...
orbits traverse its slice hyperplane, including the group orbit of the
second {\template} $\ssp'{}^{(2)}$. Replace the second {\template} by its
closest group-orbit point $\slicep{}^{(2)}$, \ie, the point in \slice\
$\pSRed{}^{(1)}$.
    (b)
Now that the group orbits have been reduced to points, consider the two
slices through the two templates. As these two points are the closest
points viewed from either group orbit, they lie in the slice of each
\template. However, the two tangent vectors $\sliceTan{}{}^{(1)}$ and
...
As a generic \slice\ \refeq{PCsectQ0} is the set of all
...
divergence in {\phaseVel} \refeq{reconstrEq} is an artifact of the
