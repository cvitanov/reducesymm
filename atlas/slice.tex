...
its integral keeps track of the group shift in the full \statesp.
    ...
    \KCedit{
Final comment for slicing: while the action of slicing and Poincar\'e
sections are seemingly different actions, they are in fact very similar
as both cut symmetries. In particular, Poincar\'e sections cut time
symmetries and slices cut symmetries present in the dynamics. For
instance, in a Poincar\'e section we enact the evolution operator until a
certain condition is met: the section has been pierced; in a slice, we
enact the group operation until a point mets the slice condition. Just as
a Poincar\'e map goes bad, it is not unreasonable to expect a slice to go
bad. While there are connections between the two, they are decisively
different.
    } %end \KCedit

    \PCedit{
The procedure is akin to (but distinct
from) cutting across continuous-time parametrized trajectories by means
of Poincar\'e sections. A \PoincSec\ reduces a continuous time trajectory to a
sequence of points. \Slice, however, is emphatically \emph{not} a
\PoincSec; it replaces a trajectory by a symmetry-reduced
trajectory.
    }
