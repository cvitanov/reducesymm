...
Suppose on your day job you are computing invariant solutions of \NSe.
...
sections. We now offer a pictorial tour of this hitherto unchartered
territory, save for the one bold incursion\rf{ACHKW11}.

Pick a \template\ $\slicep$ and use the freedom of moving frame
...
that lands in the \slice, satisfies the \emph{\slice\ condition}
\beq
\braket{\sspRed}{\sliceTan{}} = 0
    \,.
\ee{PCsectQ0}
The \slice\ so defined is thus a $(d\!-\!N)$\dmn\ hyperplane normal to
the $N$ \template\ group tangents evaluated at the \template, sketched in
...
\reducedsp\ flow, $\dot{\sspRed} = \velRed(\sspRed)$, confined to the
\slice, $\sspRed(\zeit) \in \pSRed$, as
\bea
...
\label{reconstrEq}
\eea
so $\vel$, the velocity in the full \statesp\ is the sum of $\velRed$,
