...
In the following two subsections we illustrate the effect the choice
of \slice\ point has by using our \cLe\ example.
...
condition \refeq{PCsectQ} which we can solve to obtain
the moving frame map and the invariant coordinates explicitly.

Choose \slice\ point in one of the irreducible subspaces of
...
In general, when a \csection\ $K$ is defined through a relation of the form
$\overline{x}_i=0$ then we call $K$ a \emph{coordinate \csection}.
