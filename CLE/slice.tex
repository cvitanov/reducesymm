...
such that the next segment initial point is on a slice
chosen as a $(d\!-\!1)$-dimensional
...
\,.
\ee{PCsectQ}
\ES{move in moving frames: In other words, \slice\ is a Poincar\'e section for group
orbits. It is locally isomorphic to $\pS/\Group$.} For any
$\sspRed \in \bar{\pS}\equiv\pS/\Group$, $\sspRed = \LieEl(\theta)\, \ssp$ is
defined to be the rotation of $\ssp$ that lies in the \slice.
\PublicPrivate{}{As noted in section \refsect{sec:mf} it is not
as straightforward to write explicit equations for the moving frame
associated to such a slice as it is for a coordinate slice. Nevertheless,
we can always solve numerically for the unique group element that maps
any point back to the slice.
}%end PublicPrivate
...
\noindent
By equivariance\ES{In equivariance condition \refeq{eq:equivFinite}
the group element $\gamma$ is time-independent. The condition corresponds
...
action is free and a slice can be defined. This is the source of all the 
problems we have.} 
one can always\ES{substitute ``always'' with ``at least locally''.}
...
\subsection{Method of moving frames, differential formulation-II}
...
in \refsect{sec:SymmDyn}. For infinitesimal  $d\theta$ we
...
Lorenz flow \refeq{Lorenz}.
    ...
    \reqv\ calculation of \refeq{sec:StabEq},
