...
$\dot{\phi}_1=\dot{\phi}_2=0$ we have a group orbit
of \eqva\ instead).
The \reqv\
$\REQV{}{1}$ is given by
\bea
...
For the \refeq{eq:CLeR} parameter values, the \reqv\ is at
\beq
...
Calculation of \REQV{}{1} stability eigenvalues (see
\ref{s:StabReq} for a calculation of stability of \reqva\ in
equivariant variables) yields a weakly unstable spiral-out
\eqv\
\beq
...
The large imaginary part of $\eigExp_{1}$ in \refeq{eq:CLeREQBstab}
implies that the simulation has to be run up to time of
order of at least 70 for the strange attractor in
\reffig{fig:CLE} to start filling in.
