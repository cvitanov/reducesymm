...
We choose slice-fixing point $\slicep=(0,\,1,\,y_1^{o}/x_2^{o},\,x_1^{o}/x_2^{o},\,y_2^{o}/x_2^{o},\,0)$ 
with $\ssp^{o}= \ssp_{\REQB{1}}=$\ES{Fill this in.} on the group orbit of \reqv.
We can use \refeq{cLeMF} to compute $\theta$ for any point $\ssp$, or keeping up
with the numerical approach of this section, use a Newton's method with initial
guess the angle from previous point along the trajectory and with first point
satisfying orientation condition that results in minimum distance from \slicep.
Projections of \cLe\ dynamics to the slice defined in this way are
shown in \reffig{fig:CLEmfReqb1}. Singularities of the same nature as the ones 
in \refsect{s:cleCoordSlice} are also present here.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In \reffig{CLEmfSingSet}(a) we observe that with slice fixed by
...
\reffig{fig:CLEmfSingSet}(b), \reffig{fig:CLEmfAdHoc}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%computed with vaggelis/testing/flows/CLEfinalTmp.nb
\begin{figure}[ht] \label{fig:CLEmfSingSet}
\begin{center}
