...
% $Author$ $Date$
%
In his seminal paper, E. Lorenz\rf{lorenz} reduced both
...
paradigmatic flow.\JFG{Suggestion: In his seminal 
paper, E. Lorenz\rf{lorenz} reduced the continuous 
time and discrete spatial symmetries of the 3-dimensional 
Lorenz equations, resulting in a 1-dimensional return 
map that yields deep insights\rf{tucker1-2} into the 
nature of chaos in this flow.} For strongly contracting, 
low-dimensional flows, Gilmore, Lefranc and
Letellier\rf{gilmore2003,GL-Gil07b} systematized
...
`turbulence' or `spatio-temporal chaos.'\JFG{Similar to 1st sentence 
of previous paragraph: too much going on, and the structure doesn't 
indicate what's most important. I suggest dropping ``close to $\ldots$.''} Such flows have
...
the simplest \JFGedit{possible} continuous (rotational) spatial 
symmetry, the symmetry-induced drifts obscure the underlying
hyperbolic dynamics. The question that we address here is how
\JFGedit{one can} construct suitable return maps for arbitrarily
...
\cLe\ (CLE), \JFGedit{a} 5-dimensional set of ODEs that we use
...
efficient method to compute symmetry invariant bases \JFGedit{which}
goes back to Cartan, and in \refsect{sec:CLeMovFr} we
illustrate \JFGedit{this method in application} to the \cLe. The method
...
restrictions, with generic \JFGedit{trajectories} within a slice bound to encounter
singularities.

In \ref{s:StabReq} we derive an expression for the stability
of \reqva\ (traveling waves) in reduced \JFGedit{spaces}
