...
in the limit of $x=x_1+i x_2$ approaching zero. For the limit to exist 
we must get a direction independent result as $x\rightarrow 0$.
...
on the complex $x$-plane along which we approach zero. In terms of projecting dynamics 
...
points have exactly $x=0$ (in CLE this does not happen). Since the subspace $x=0$ 
is not flow invariant we do not need to worry about dynamics that stay within the subspace. 
We need to worry about the equilibria that exist in this subspace though, here the equilibrium 
at the origin, as there is no way to transform them to the new variables. 
For KS $\EQV{2}$ and $\EQV{3}$ belong to such a subspace.
 
that the denominator is singular on
