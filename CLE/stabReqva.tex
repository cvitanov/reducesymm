...
fact that we still work in the equivariant variables in which
a marginal direction due to $\SOn{2}$ symmetry exists. Being
able to compute stability eigenvalues of \reqva\ directly in
equivariant variables is not a importantt in \CLe\ since the
system can be easily rewritten in polar representation \refeq{eq:CLePolar}
but it can be extremelly useful in higher dimensions where an\
explicit change of variables is intractable. 
