...
or in matrix form
\beq
	\mathbf{\bar{\Mvar}}=\PperpOp \mathbf{A}-\frac{1}{\groupTan(x)^2}\left( \left[\vf \cdot \groupTan(x)\right] \left(\PperpOp \Lg\right) +\groupTan(x) \otimes \left[\vf \cdot \left( \PperpOp \Lg\right)\right] \right)\,,
	\label{eq:reqvStab}
\eeq
