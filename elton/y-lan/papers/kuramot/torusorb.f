...
c d,even, trunc,2^n  
      ...
      parameter(d=16,trunc=128,dtr=d*trunc,m=2)
      double precision tiny,hmin,eps,dh,hdid,hnext,omg0
      double precision nu,dydx(d),yerr(d),ytemp(d),yscal(d)
      double precision a(d),x(dtr),xx(dtr),time,time1,b(d)
      integer m1,m2,m3,resterm,count,ct2,pn,cycle,index(d)
      double precision p,pi,jbt(d,d),jb(d,d,trunc),xold(dtr)
      double precision dv(d,d),limit,invtr,y(dtr),t1,dnu,L
...
c      dh=1e-3
      ...
      invtr=dble(1.)/trunc
      t0=2*pi/trunc
c      omg=2*pi*((sqrt(7.D0)-2)/2.)
c      omg=2*pi*((sqrt(5.D0)-1)/2.)
      omg=2*pi*(pi-3) 
c L=40.95,L=41.35     
...
c      open(2,file='antorb4b.dat',status='old')
      open(3,file='test4.dat',status='old')
c      open(3,file='torusks412a.dat',status='old')
      open(4,file='torusks412b.dat',status='unknown')
c      open(5,file='torussmF.dat',status='unknown') 
      read(3,'(F12.6)') omg
      do 500 i=0,trunc-1,1
        ...
        do 510 j=1,d,1
          read(3,'(F12.6)') x(m1+j)
510     continue
c        x(m1+1)=0.+i*t0+0.01*sin(i*t0)
c        x(m1+2)=omg+0.013*cos(i*t0)
500   continue
      close(3,status='keep')
      go to 20
...
c        if(dh.lt.0.5e-5.or.count.gt.1000) go to 10 
        ...
        do 4200 i=1,trunc,1
          ...
          do 4210 j=1,d,1 
            ...
            b(j)=a(j)
4210      continue
c          call f1(d,nu,a,dydx)
c          call jacobj(d,nu,a,jbt)
          tp=0
          p=0.06
          pn=1
        call antprop(L,d,nu,b,dydx,yscal,yerr,ytemp,time1,tp,p,pn)
          tp=1
        call antpropj(L,d,nu,a,dydx,yscal,yerr,ytemp,time1,dv,
     \jbt,tp,p)
c          time=2*pi
c          write(*,*) 'before',a
...
1000  continue
      ...
      subroutine jacobjb(d,eps,a,btemp,dv,t)
      integer d
      ...
      subroutine f1a(d,eps1,eps2,eps3,a,dydx)
      integer d
      ...
      subroutine jacobja(d,eps1,eps2,eps3,a,dv)
      integer d
