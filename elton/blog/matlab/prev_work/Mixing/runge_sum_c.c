...
static void sum_c(double *kk, double *r, SUM_PARMS *p); /* this routine is identical in mk_vgrid_c.c and in runge_sum_c.c */
...
{   /* kk is output (3 values), r is input (3 values), p is ptr to structure containing needed parameters for summing */
    ...
    for(my = 0; my < My;++my)
    {
        ...
        while(wptr < wend)
        {
            /* only compute the real part of the product, that saves half the mults */
