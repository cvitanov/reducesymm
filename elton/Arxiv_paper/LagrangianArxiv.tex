...
and a $2D$ unstable manifold which is locally a plane \refeq{sp1_evec1}-\refeq{sp1_evec3}. The fact that one of the eigenvalues for the unstable manifold of $SP_1$ is much larger than the other is apparent in the figure by the fact that the trajectories in the unstable plane become quickly contracted in one of the dimensions, and the trajectories appear to leave along a nearly one-dimensional structure in the $y$-direction. $SP_2$
 ...
 Next we bring trajectories originating near $SP_1$ into the picture to see how the manifolds of this stagnation point connect with those in \reffig{eltonFig:hetero1}, producing the full dynamical portrait within $\Omega$.  The result is
 shown in \reffig{eltonFig:hetero2}. Compare to \reffig{eltonFig:stagps_label2} to see the locations of the stagnation points. The relation of the
...
\bibliography{../../bibtex/pipes} % 2024-11-23 Predrag removed JE's {LagrangianArxiv}

% \bibliography{pipes}  % Use when JE loading in Overleaf
