% reducesymm/elton/Arxiv_paper/LagrangianArxiv.tex
%                        pdflatex LagrangianArxiv; biber LagrangianArxiv
...
\,,
\ee{NavStokesDiff}
with boundary conditions $\bu = 0 $ at $y \pm 1$. The equilibrium 
velocity fields we study satisfy \refeq{NavStokesDiff}. This equation is 
...
incompressibility and boundary conditions) forms a vector space. 
...
of the expansion of a velocity field $\mathbf{u(x)}$. The form of the 
expansion is 
\begin{equation}
...
velocity field at every point along a trajectory. We are able to perform 
...
\subsection{A \stagp\ has a partner, symmetric about another {\stagp}}
...
and run for a relatively short time. Here, the tracers are shown for 
$\bu$ defined as the difference from laminar flow. Just from evolving the 
...
\refsect{sec:symm_stag}, implies the existence of 4 \stagp s 
...
\refsect{sec:symm_stag}, we refer to these new numerically discovered 
...
alternate domain of investigation that will be conventient to sometimes 
use is 
\[ %beq 
...
\subsection{Flow portraits and {\hc}s}
...
\subsection{Tracer dynamics and analysis}
...
we know from the discussion in \refsect{sec:symm_stag} that there will be 
...
\subsection{More symmetries $\to$ more stagnation}
...
subspace of velocity fields and $EQ_8$ is in $S_8$ (\refsect{sec:PCF_symm}
and \refsect{sec:symm_stag}). 
...
\bibliography{../../bibtex/pipes} % 2024-11-23 Predrag removed JRE's {LagrangianArxiv}
