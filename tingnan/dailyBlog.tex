...
\refFig{7diskFundDflips}\,(a): Pavel's three generators. The standard notation
...
I have checked a few cycles and their new symbolic representations. For example, $\cycle{026}$ and \cycle{0260628}. For cycle $\cycle{026}$ and $w=0.3$ the symbolic sequence is $f\ell\,sf\ell f\,fs$. For $w=2.0$ the sequence is slightly different, $f\ell\,s\ell f \ell\,fs$, but with the equivalence relation $f\ell f=\ell f \ell$ the sequence in the end is the same. Similar results con be obtained for $\cycle{0260628}$. I have also checked a few cases when long flights are involved, and the same conclusion holds: the symbolic sequence we proposed is invariant to parameter changes. I cannot mathematically prove it but I have some interesting discovery: even if $w$ changes, the position of collision on the circle always belongs to the same triangular piece (fundamental domain). Any idea about this? If this is correct there must be something very fundamental there. I will check the period-2 cycles proposed.

\begin{figure}

\includegraphics[width=\textwidth]{cycle026.pdf}
\caption{3-cycle \cycle{026} for w=0.3 (left) and w=2.0 (right). }
\end{figure}

\begin{figure}

\includegraphics[width=\textwidth]{cycle0260628.pdf}
\caption{7-cycle \cycle{0260628} for w=0.3 (left) and w=2.0 (right). }
\end{figure}
...
\item[2014-06-03] Finally I got Gaspard's book from the GIL service. Here are updates on reciprocal lattice.
...
Now with some quantities defined let us proceed to the fourier transforms of the quantities we are interested.
