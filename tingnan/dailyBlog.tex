...
From the table \reftab{TCELL2} we can see that convergence does not increase. I also included the cycle 2 table, with updated numerical values as well as the floquet multiplier, in table \reftab{tab:length2withfloquet}.

I also tried to compute the diffusion coefficients for other parameters (e.g., $w=0.2$). Then I noticed that I can get negative values for Lyapunov exponents (as sometimes $T$ averaged from the truncation could be negative). Is it something normal (because of pruning)?
...
\item[2014-05-28 Predrag]
Corrected, updated  \reftab{tab:ListLength2} using Tingnan's
\reftab{tab:length2withfloquet}.
...
because of the new length 6 periodic orbit, of multiplicity only 2?
This cycles is a fixed point in the fundamental domain, so taken care of
...
isotropic (not so on the square lattice - diagonals
$\spaceAver{n_{x}n_{y}}$ are different from $\spaceAver{n_{x}n_{x}}$. You can
probably find the proof or a reference to it in the literature on lattice
gases in 2 dimensions. If that is true, your $\cl{}=6$, $\cl{}=7$ and
$\cl{}=8$ are still to be debugged.

%%%%%%%%%%%%%%\item[Figure ?]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
\begin{center}
\includegraphics[width=0.59\textwidth]{7diskFundDflips}
\end{center}
\caption{
The three generators of tiling of the plane by a fundamental domain:
two generators of \Dn{12} tiling, $s$ reflection across the short
disk-disk separation and $\ell$ reflection across the long disk-disk
separation;
and
a translation generator $f$ that pivots a disk center to disk center by
flip across the symmetry line normal to the short disk-disk separation.
    }
...
\end{center}
\caption{
(a) Elementary cell short 2-cycle , with multiplicity 6 corresponds to
the fixed pint $f$ in fundamental domain, $\cycle{02}=\cycle{f^2}$;
(b) Elementary cell short clockwise 3-cycle $\cycle{084}$, and
counterclockwise 3-cycle $\cycle{048}$, each with multiplicity 3, in
fundamental domain $\cycle{048}=\cycle{(rf)^3}$.
    }
\label{7diskFundDflips2}
\end{figure}
%%%%%%%%%%%%%%\item[Figure 4]%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\item[2014-06-01 Predrag]
\refFig{7diskFundDflips}: Three Pavel's generators. More standard
notation is \refeq{D12generators}, with $s=\sigma_{dj}$ reflection across
the short disk-disk separation and $\ell=\sigma_{vj}$ reflection across
the long disk-disk separation generators of \Dn{12}. The entire space group is then generated
by adding a translation generator $f$ that pivots a disk center to disk
center by flip across the symmetry line normal to the short disk-disk
separation. Few relations
\[
\ell r = C_6^-
\,,\qquad
r \ell =  C_6^+
...
For practice, let's
work out a few  cycles composed of short flights only. There are
two short paths from disk to disk: $f$ and $sf$.
\refFig{7diskFundDflips2}: Conversion elementary cell $\to$ fundamental
domain for the short 2-cycle and 3-cycle.

\item[2014-06-02 Tingnan]

Let us expand the $\spaceAver{\period{}}_\zeta$ explicitly to a few orders, e.g., 4th and 5th order:
\begin{align*}
\spaceAver{\period{}}_\zeta
  &=\sum^{\prime}(-1)^{k}(\tau_{p_1}+\cdots+\tau_{p_k})/\vert\ExpaEig_{p_1}\cdots\ExpaEig_{p_k}\vert\\
...
From what I see, beginning from $n=4$ the pseudo-cycles (in our case composed of two short prime cycles) start to contribute to the space averages, and their contributions are larger than those of prime cycles of the same topological length. Many cycles have long topological length but short flight time, I will try to identify a few of those. 

Beginning from $n=6$, I am missing cycles that are very close to the tangent, because sometimes the path intersects with a disk due to slight numerical differences. Say if a cycle is of multiplier $12$ due to rotational symmetry, I sometimes only find 11 or 10 from this family. Again, this is probably another argument of why we needs to go to the fundamental domain. 
