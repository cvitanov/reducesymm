...
Here I listed all the intermediate quantities. The sudden jump of diffusion coefficient is from the decrease of the mean lattice displacement. 
\begin{table}[htbp]
  \centering
  \caption{Intermediate quantities from cycle expansion. $T=\sum^{\prime}(-1)^{k}(\tau_{p_1}+\cdots+\tau_{p_k})/\vert\Lambda_{p_1}\cdots\Lambda_{p_k}\vert$, $\mu=\sum^{\prime}(-1)^{k}(\mu_{p_1}+\cdots+\mu_{p_k})/\vert\Lambda_{p_1}\cdots\Lambda_{p_k}\vert$, and  $N_{xx}=\sum^{\prime}(-1)^{k}(n_{xp_1}^2+\cdots+n_{xp_k}^2)/\vert\Lambda_{p_1}\cdots\Lambda_{p_k}\vert$}. 
