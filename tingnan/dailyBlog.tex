...
\caption{7-cycle \cycle{0260628} for $w=0.3$ (left) and $w=2$ (right). }
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{7diskFUndDtilesEquiv.pdf}
\end{center}
...
I think we should propose a separate label $\circ$ to indicate the collision on the disk, i.e. $\cycle{f\circ}=\cycle{06}$. Without the additional symbol, when we are doing the string reduction things could go wrong. 

\item[2014-07-14 Tingnan]

The topological robustness of the cycle inspired me to look at the cycle in fundamental domain from a different perspective. Before we write the explicit symbol string using the 3 generators, let us just visualize a flight between two nearest disk. I label the triangular pieces with numbers (Fig.~\ref{fig:topoCombo}). A flight between two disk can be represented by the combination of two labels (numbers). With symmetry been considered, there are 12 topologically distinct combinations (out of 18 possible combinations):
\bea
...
\begin{figure}
\begin{center}
\includegraphics[width=0.5\textwidth]{topologicalCombinationZoom.pdf}
\end{center}
\caption{topological combination of fundamental domain.}
\label{fig:topoCombo}
\end{figure}
Now a fundamental domain fixed point should correspond to a symmetric (or anti-symmetric) combination:
\bea
