...
Dettmann's review\rf{Dettm14}, \arXiv{1402.7010} is something that you
...
\item[2018-11-26 Burak]
Klages \etal\rf{KGSSR18} {\em Normal and anomalous diffusion in soft {Lorentz} gases}
(and \rf{GilGall18})
study Lorentz gas with Fermi potentials instead of hard disk scatterers. The main
result is in Fig. 2, which shows that the diffusion coefficient as a function of
lattice spacing is quite irregular. They claim that such irregularities only
observed in very narrow parameter ranges if the hard-shell scatterers are used
and, for an explanation, they demonstrate appearance and disappearance of various
periodic orbits.  In particular, in the superdiffusive cases parameter values,
there exist "ballistic" periodic orbits (Fig. 3 (a,b)).

Theoretical parts are a little bit handwavy, although I'm not sure if it can be
done better with symbolic dynamics etc. Nevertheless, I enjoyed reading the paper
and they also mention possibilities of getting ballistic transport in experiments
of ``artificial graphene.''

\item[2019-01-23 Predrag]
Gil-Gallegos \etal\rf{GKSR18}
{\em Energy-dependent diffusion in a soft periodic {Lorentz} gas}: ``
The periodic Lorentz gas recently became relevant as a model for electronic
transport in low-dimensional nanosystems such as molecular graphene, with the
hard Lorentz gas scatterers replaced by soft potentials. Here we study diffusion
in a soft Lorentz gas with Fermi potentials under variation of the total energy
of the moving particle. We identify three different dynamical regimes: (i) the
onset of diffusion at small energies; (ii) a transition where for the first time
a particle reaches the top of the potential, characterized by the diffusion
coefficient abruptly dropping to zero; and (iii) diffusion at high energies,
where the diffusion coefficient increases according to a power law in the energy.
''
...
Landau and Lifshitz\rf{Landau70}
% see Theory of Elasticity
show that elastic energy of the triangular lattice is isotropic, (invariant
under lattice rotations). The theorem applies to 2nd order tensors constructed
of spatial displacements, and from it follows that the diffusion tensor on the
hexagonal lattice is isotropic.
(That is not the case on a square lattice - diagonals
$\spaceAver{n_{x}n_{y}}$ are different from $\spaceAver{n_{x}n_{x}}$).

If the theorem applies, Tingnan's
...
\item[2016-01-06 Predrag] Bunimovich and Sinai\rf{BunSin81}
...
bounded by
\beq
    \braket{v(\zeit)}{v(0)}< C\exp(-\kappa \zeit^\gamma)
\,.
\ee{velAutoFinHor}
The paper is all about probabilities and measures,
but we need to cite it, Bleher is clear on that.

\item[2016-01-06 Predrag] Bleher\rf{Bleher92} 59 (!) pages
{\em Statistical properties of two-dimensional periodic Lorentz gas with
infinite horizon} is important.

He credits \refrefs{Sinai70,Gallavotti75,BuSiCh90} for the formulation of
periodic Lorenz gas, and the previous results on periodic Lorenz gas (seo we
cites them in the introduction of our paper\rf{ZhCvGo15}).

                                                    \toCB
For the finite horizon case he uses the Bunimovich and Sinai\rf{BunSin81}
bound on the velocity autocorrelation function \refeq{velAutoFinHor} to
...
case:
\beq
\lim_{\zeit \to \infty} \frac{\expct{x(\zeit)-x(0)}}{(t \log t)^{1/2}}
= \eta
\,,
\ee{BleherDiff}
...
Bunimovich, Sinai and Chernov\rf{BuSiCh90} have a succinct definition of
...
introduced in \refref{AdWei67} (in which a Markov partition was
constructed for an automorphism of the two-dimensional torus with
elements of the form of the `present' parallelograms). In
\refrefs{Bowen70,Sinai68} the term `rectangle' was used for the same
purpose).''
...
\refrefs{BunSin80,BunSin81}. Properties of a smooth system with finite
...
might ask yourself - who the hell was that? Every pack of humans is
...
to cite him once, for his book\rf{RKbook07} which has one chapter on
billiards. Rateitschak and Klages\rf{RK02} is also on billiards, but
...
Klages and Dorfman\rf{KlaDor97}
 {\em Dynamical crossover in deterministic diffusion},

Rajagopalan and Sabir\rf{RS01} {\em Analysis of chaotic motion and its
...
Klages\rf{KlaKelHow08,KniKla11a,KniKla11b,KlaKelHow08} diffusion in maps

R. Klages, \arXiv{0804.3068},
...
''
                                                    ...
                                                    \toCB
Klages\rf{KKCSG07} is on anomalous diffusion in intermittent maps.
perhaps cite alongside Artuso\rf{ACL93,ACL94,art03,ArtCri03,ArtBur14}.



    \item[2016-01-09 Predrag] Run into more references:

\refref{VTB97} {\em Equivalence of Irreversible Entropy Production in
Driven Systems: An Elementary Chaotic Map Approach}

Harayama and Gaspard\rf{HG01}
   {\em Diffusion of particles bouncing on a one-dimensional periodically
   corrugated floor}

Stoop and Steeb\rf{SS97}
 {\em Chaotic family with smooth {Lyapunov} dependence}

Harayama and Gaspard\rf{HKG02}
   {\em Deterministic diffusion in flower-shaped billiards},

    \item[2016-01-11 Predrag] Asamizuya\rf{Asa06} writes in
...
\refref{BunM,ChMePe87,HaSh92}. In ChaosBook, the method was
(re)invented by P. Dahlqvist, Sect 37.3 {\em Least action method},
see Remark 37.9 for earlier sources,
\refrefs{Sieber91,steiner}.

Chen, Meiss and Percival\rf{ChMePe87}
...
rigidity of planar convex domains} Jacopo De Simoi\rf{ADSK14} credits
1927 George Birkhoff\rf{Birkhoff55} for defining billiard periodic orbits
...
Casas and Ram{\'i}rez-Ros\rf{CaRa12}
...
find lots of periodic orbits in dimension 2 and higher.
    ...
    \item[2016-02-18 Predrag]                      \toCB
Hasselblatt and Katok\rf{HK03} {\em A First Course in Dynamics: with a
Panorama of Recent Developments}, P.~122, Fig.~4.2.15 chooses to define
the `fundamental domain' (our elementary cell) of the translation group
as a parallelepiped one triangle-high, three triangles wide, rather than
our hexagon. Why? instead of identifying three pairs of opposite edges of
a hexagon, one identifies two pairs of opposite edges of a
parallelepiped, thus quotiented dynamics is on a 2-torus, just as for
tiling with squares.

    \item[2016-02-22 Predrag]                      \toCB
I sketch here one, perhaps wild, direction that you are completely free
to ignore, but if you do learn something interesting while pursuing it,
please report back.

One thing we should cite in \refref{ZhCvGo15} is that
Hestenes\rf{Hestenes02} gives an algebraic formulation of the geometric
notion that all symmetries can be generated from reflections in
planes\rf{Coxeter89}, thus replacing the additive translation group by
the multiplicative group operations, and putting rotations and
translations on the same footing. In our planar hexagonal lattice, all
rotations and translations are generated by three reflections. We should
probably have a look at Coxeter\rf{Coxeter89} discussion of planar
tilings.

Here is the problem of space groups in the nutshell. The Euclidean
invariance on Newtonian space-time (including its subgroups, such as the
discrete space groups), and the \Poincare\ invariance of
special-relativistic space-time has one awkward feature: the space is
non-compact (homogeneity), while rotations are compact (isotropy). That
leads to the conceptually awkward situation of mixing a group of
additions (translations) with a group of multiplications (rotations). To
work with such group we \emph{first} translate objects to the
\emph{origin} and \emph{then} rotate them with the respect to the origin.
That's not nice, because by translation invariance any point is as good
as any other, there is no preferred origin. There is no reason why one
should translate first, rotate second. What one needs is a formalism that
implements translations and rotations on the same footing.

If I understand Hestenes\rf{Hestenes66} right (also David Finkelstein and
perhaps Holger Beck Nielsen have told me things in this spirit) a way to
accomplish that is to replace the flat translational directions by a
compact manifold where translations and rotations are non-commuting
multiplicative group operations. More precisely, in {\em Spacetime
physics with geometric algebra}\rf{Hestenes66} Hestenes  advocates
redoing \emph{all physics} in terms of Clifford algebras (or his version of
them, the `spacetime algebra'):

``I claim that the physics curriculum at all levels can be thoroughly
unified and considerably simplified by adopting STA (spacetime algebra)
as the core mathematical language of physics.   The language is fully
developed and ready to use.   Setting the politics of curriculum reform
aside, let us consider how a forward-looking physics department could
incorporate STA into its curriculum.''
``But, if Geometric Algebra is so good,'' Hestenes has
often been asked, ``why is it not more widely used?'' ``I can only reply:
Its time will come!''

And indeed, it is quite conceivable that the condensed matter physicists
might be happy to learn the space groups as they have learned them for a
century (they do not actually learn the space groups, they look up the
characters in a famout thick book), and have no motivation to replace
that by a smarter and deeper theory. Just sayin' - practical people that
they are, they are hostile to too much theorizing.

Hestenes {\em The Genesis of {Geometric Algebra}---a personal
retrospective}\rf{Hestenes15} is a fun read. He entered UCLA as a
pre-medical major in preparation to be a missionary doctor. [...] He was
John Wheeler's postdoc. Wheeler neglected him fully, like he had also
neglected Roger Penrose. Etc, etc. And, if you want to waste lots of
time, there is always \HREF{http://web.mit.edu/redingtn/www/netadv/} {The
Net Advance of Physics}.

A part of the Hestenes program is redoing crystallography. What follows
are a few snippets from Hestenes\rf{Hestenes02}
\HREF{http://geocalc.clas.asu.edu/pdf/crystalsymmetry.pdf} {paper} (see
also Hestenes and Holt\rf{HesHol07}):

There are many kinds of symmetry, but the symmetries of rigid bodies are
the most important and useful, because they are the most ubiquitous as
well as the most obvious. A geometric figure or a rigid body is said to
be ``symmetrical'' if there exist isometries (symmetries) which permute
its parts while leaving the object as a whole unchanged. The symmetries
of a given object form the symmetry group of the object.
Every symmetry group is a subgroup of the group of all such isometries,
the Euclidean group $E(3)$.

Every such symmetry operation can be given the mathematical form
\beq
S : x \to x' = \underline{R} x + a ,
\ee{Hestenes02_1}
where x designates a point in the object, \underline{R} is an orthogonal
transformation with the origin as a fixed point, and the vector $a$
designates a translation. In most applications the operator \underline{R} is
represented by a matrix [ R ], so composition of transformations is
achieved by matrix multiplication.  This practice has two drawbacks,
however.  First, use of matrices requires introducing a coordinate
system, and that brings in arbitrary features that complicate problems.
Second, matrix elements are usually difficult to interpret geometrically.
\emph{Geometric algebra} avoids these drawbacks with the coordinate-free
canonical form
\beq
\underline{R} x = \pm R^\dagger x R,
\ee{Hestenes02_2}
where R is an invertible multivector, called a versor, with even (odd)
parity corresponding to the plus (minus) sign\rf{Hestenes99}. The versor
in \refeq{Hestenes02_2} has been normalized to unity, so its reverse is
equal to its inverse,
\beq
R^\dagger = R^{-1}
\ee{Hestenes02_2a}
When R is even, \refeq{Hestenes02_2} describes a rotation, and R is
often called \emph{rotor} or a \emph{spinor}.

Next, the paper provides a complete treatment of the point groups in two
and three dimensions. The main result is that each of the point groups in
three dimensions can be generated from a set of at most three symmetry
vectors that are tied directly to features of the object.  This leads to
a new systematic notation and classification scheme for symmetry groups
from which one can directly write down the generators for any point
group.

As the symmetry vectors generating the point group can be identified with
lattice vectors that generate the lattice, geometric algebra can simplify
crystallography. In other words, the point group can be generated
multiplicatively from the objects on which it operates. Contrast this
with the usual approach which develops the group elements and the lattice
as separate entities related only indirectly.

The point group of a lattice leaves a lattice point fixed. To get the
complete symmetry group of a lattice, one needs to combine the point
group with translations. This raises another problem with the standard
representation for a symmetry by \refeq{Hestenes02_1}, namely: The
orthogonal group is multiplicative while the translation group is
additive, so combining the two destroys the simplicity of both. The
source of this problem can be traced to the fact that equation
\refeq{Hestenes02_1} singles out one point, the origin, for special
treatment.   This paper shows how geometric algebra provides an elegant
solution of this problem with a simple new multiplicative representation
for the space groups generated directly from lattice vectors.

Paper introduces a new homogeneous formulation of Euclidean geometry that
treats all points equally, generates an algebra of points, lines and
planes and creates the desired multiplicative model of the Euclidean
group.  This model provides a precise algebraic formulation of the
geometric notion that all symmetries can be generated from reflections in
planes\rf{Coxeter89}. Thereby, it provides a new
algebraic foundation for geometric intuition and a powerful tool for
computational geometry.

Section {\em 2. Point Groups in Two Dimensions} motivates the approach by
working out the hexagon dicyclic group \Dn{6} of order 12. Then all 2-
and 3-dimensional point groups are speedily classified - seems easier
than the usual presentation.

Section {\em 5. Homogeneous Euclidean Geometry} embeds 3-dimensional
Euclidean space into a Minkowski space in such a way that all
points are treated equally, without singling out an origin.
The squared Euclidean distance between any two points is given by inner
product between the points. The oriented line (or line segment)
determined by two distinct points is represented by a trivector. The
tangent vector and the length of the line segment are given by products.
Two points determine an oriented plane as well as a line.

Section {\em 6. Symmetries from Reflections}: \emph{Every} symmetry in
Euclidean group $E(3)$ can be generated from reflections in planes. In
particular, any rotation about a given line can be reduced to a product
of reflections in two planes that intersect in that line, and \emph{any
translation} can be reduced to a product of reflections in two parallel
planes. In the homogenous model, Euclidean group $E(3)$ is reduced to a
multiplicative group of versors.

Section {\em 7. The Space Groups}:
The translation group of a crystal is an additive group generated by
three vectors, while the double point group is a multiplicative group
generated by at most three vectors. Consequently, the space group can be
characterized by a set of relations among these two sets of generators.
Indeed, we can choose three linearly independent vectors from the two
sets and write the others in terms of them.  Thus, every element of a
space group can be expressed in terms of three vectors which generate
translations by addition and orthogonal transformations by
multiplication.

Section {\em 7.1 Planar Space Groups}
In two dimensions there are 17 space groups, which
he succeeds in describing in terms of two symmetry vectors.

My problem is that I do not see a way to a decomposition to irreps in
this formalism, and how to compute things we care about (characters,
etc). Maybe that is discussed in Hestenes and Holt\rf{HesHol07}. I have
not read it.
