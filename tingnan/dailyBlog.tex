...
\[
\zeta(0,0)=1-\sum_{n_p\leq2,3}t_{p}=1-\sum_{n_p\leq2,3} \frac{1}{\Lambda_{p}}
\]
...
is different. And I am not sure about this sentence in Shreiber's paper:

 "The right and left turns are not distinguished - instead,  one reads off a symbol whether the next
...
\begin{align*}
\left\langle \phi\vert\mathcal{L}(y,x)\vert\rho\right\rangle  &= \int_{M}dy\int_{M}dx\phi(y)\delta(y-f^{t}(x))\rho(x)\\
...
where we have used the fact that $\mathrm{det}\vert a\vert=1$(e.g.
rotation or translation). The product of two group actions $b^{-1}a$
determines the relative ``dispalcement'' in the group space. We
can then write the evolution operator using the left regular representation
$D(h)_{ba}=\delta h,b^{-1}a$, as:
\[
\mathcal{L}(y,x)=\sum_{h,ab}^{\vert G\vert}D(h)_{ba}\delta(\mathbf{h}^{-1}\tilde{y}-f(\tilde{x}))=\sum_{h,ab}^{\vert G\vert}D(h)_{ba}\mathcal{L}(\mathbf{h}^{-1}\tilde{y},\tilde{x})
\]
...
\[
\mathcal{L}(z,y)\mathcal{L}(y,x)=\sum_{hh^{\prime},abb^{\prime}}^{\vert G\vert}D(h^{\prime})_{b^{'}b}D(h)_{ba}\mathcal{L}(\mathbf{h}^{\prime-1}\tilde{z},\tilde{y})\mathcal{L}(\mathbf{h}^{-1}\tilde{y},\tilde{x})
\]


We notice that $D(h^{\prime})_{b^{'}b}D(h)_{ba}$ multiplies as matrices.

The trace formula in the fundamental domain can now been written as:
\begin{align*}
\mathrm{tr}\mathcal{L} &= \int_{\tilde{M}}d\tilde{x}\sum_{h,aa}^{\vert G\vert}D(h)_{aa}\mathcal{L}(\mathbf{h}^{-1}\tilde{x},\tilde{x})\\
 &= \int_{\tilde{M}}d\tilde{x}\sum_{h}^{\vert G\vert}\mathrm{Tr}D(h)\mathcal{L}(\mathbf{h}^{-1}\tilde{x},\tilde{x})
\end{align*}


\begin{align*}
\mathrm{tr}\mathcal{L}^{2} &= \int_{\tilde{M}}d\tilde{x}\int_{\tilde{M}}d\tilde{y}\sum_{hh^{\prime},ab^{\prime}}^{\vert G\vert}D(h^{\prime})_{ab}D(h)_{ba}\mathcal{L}(\mathbf{h}^{\prime-1}\tilde{x},\tilde{y})\mathcal{L}(\mathbf{h}^{-1}\tilde{y},\tilde{x})\\
 &= \int_{\tilde{M}}d\tilde{x}\int_{\tilde{M}}d\tilde{y}\sum_{h^{\prime}h}^{\vert G\vert}D(h^{\prime}h)_{aa}\mathcal{L}(\mathbf{h}^{\prime-1}\tilde{x},\tilde{y})\mathcal{L}(\mathbf{h}^{-1}\tilde{y},\tilde{x})\\
 &= \int_{\tilde{M}}d\tilde{x}\int_{\tilde{M}}d\tilde{y}\sum_{h^{\prime}}^{\vert G\vert}\mathrm{Tr}D(h^{\prime}h)\mathcal{L}(\mathbf{h}^{\prime-1}\tilde{x},\tilde{y})\mathcal{L}(\mathbf{h}^{-1}\tilde{y},\tilde{x})\\
 &= \int_{\tilde{M}}d\tilde{x}\sum_{h^{\prime}h\equiv e}^{\vert G\vert}\mathrm{Tr}D(h^{\prime}h)\mathcal{L}(\mathbf{h}^{\prime-1}\tilde{x},\mathbf{h}f(\tilde{x}))\\
 &= \int_{\tilde{M}}d\tilde{x}\sum_{h^{\prime}}^{\vert G\vert}\mathrm{Tr}D(h^{\prime})\mathcal{L}^{2}(\mathbf{h}^{\prime-1}\tilde{x},\tilde{x})
\end{align*}


Next, we will derive the formula for the spectral determinant in the
fundamental domain.

\begin{align*}
\mathrm{Tr}\mathcal{L}^{t} &= \int_{\mathcal{M}}\mathcal{L}^{t}(x,x)dx\\
 ...
 &= \sum_{a}^{G}D_{aa}(h)\sum_{\tilde{x_{i}}:\mathbf{h}f^{t}(\tilde{x_{i}})=\tilde{x_{i}}}\frac{e^{\beta\cdot A^{t}(\mathbf{a}\tilde{x_{i}})}}{\vert\mathrm{det}(1-\mathbf{h}J^{t}(\tilde{x_{i}}))\vert}\\
 &= \sum_{a}^{G}\sum_{\tilde{x_{i}}:f^{t}(\tilde{x_{i}})=\tilde{x_{i}}}\frac{e^{\beta\cdot A^{t}(\mathbf{a}\tilde{x_{i}})}}{\vert\mathrm{det}(1-J^{t}(\tilde{x_{i}}))\vert}
\end{align*}

Now I seem to run into an issue: the trace only depends on those
standing global orbits, and it does not take any conbributions from
...
\begin{align*}
\left.\frac{\partial s}{\partial\beta}\right|_{\beta=0} & =\lim_{t\to\infty}\frac{1}{t}\left.\frac{\partial\ln\left\langle e^{\beta\cdot A^{t}}\right\rangle }{\partial\beta}\right|_{\beta=0}\\
...
operator:
\[
\mathcal{L}^{t}(y,x)=\delta(y-f^{t}(x))e^{\beta\cdot A^{t}(x)}
\]
It is a linear operator and has its eigen ``vectors'' (in the function
space) and corresponding eigenvalues:
\[
[\mathcal{L}^{t}\rho_{\beta}](y)=\int_{\mathcal{M}}dx\delta(y-f^{t}(x))e^{\beta\cdot A^{t}(x)}\rho_{\beta}(x)=e^{ts(\beta)}\rho_{\beta}(y)
\]
...
and set $\mathcal{L}\equiv\mathcal{L}^{1}$. By evaluating the zeros
of the determinant: 
\[
\mathrm{det}\vert1-z\mathcal{L}\vert=\prod_{k}(1-z/\lambda_{k})
\]
, one can get all eigenvalues $\lambda_{k}$s of the linear operator.
The LHS can be re-written as exponentials:
\[
\mathrm{det}\vert1-z\mathcal{L}\vert=\exp\left(-\sum_{n}^{\infty}\frac{z^{n}}{n}\mathrm{Tr}\mathcal{L}^{n}\right)
\]
Now the cycle expansion will help us determine the trace of the linear
operator. 

\begin{align*}
\mathrm{Tr}\mathcal{L}^{n} & =\int_{\mathcal{M}}\mathcal{L}^{n}(x,x)dx=\int_{\mathcal{M}}dx\delta(x-f^{n}(x))e^{\beta\cdot A^{n}(x)}\\
 & =\sum_{x_{i}:f^{n}(x_{i})=x_{i}}\frac{e^{\beta\cdot A^{n}(x_{i})}}{\vert\mathrm{det}(1-J^{n}(x_{i}))\vert}
\end{align*}
The summation is over all the periodic orbits of length $n$. We may
write it in terms of prime (distinct) periodic orbits:
\[
\mathrm{Tr}\mathcal{L}^{n}=\sum_{p}n_{p}\sum_{r=1}^{\infty}\frac{e^{\beta\cdot rA_{p}}}{\vert\mathrm{det}(1-J_{p}^{r})\vert}\delta n,n_{p}r
\]


where $r$ is the repeatability and $n_{p}$ is the length of the
prime orbit. The fact: prime periodic orbit of length $n_{p}$ will
contribute to the sum $n_{p}$ times for each of the points along
the orbit. Now substitute the trace formula to the determinant, the
summation over $n$ removes the awkard delta function:
\begin{align*}
\mathrm{det}\vert1-z\mathcal{L}\vert & =\exp\left(-\sum_{n}^{\infty}\frac{z^{n}}{n}\mathrm{Tr}\mathcal{L}^{n}\right)\\
 & =\exp\left(-\sum_{n}^{\infty}\frac{z^{n}}{n}\sum_{p}n_{p}\sum_{r=1}^{\infty}\frac{e^{\beta\cdot rA_{p}}}{\vert\mathrm{det}(1-J_{p}^{r})\vert}\delta n,n_{p}r\right)\\
 & =\exp\left(-\sum_{p}\sum_{r=1}^{\infty}\frac{1}{r}\frac{z^{n_{p}r}e^{\beta\cdot rA_{p}}}{\vert\mathrm{det}(1-J_{p}^{r})\vert}\right)
\end{align*}


The spectral determinant could be further approximated using the expanding
eigenvalues of the jacobian matrix:
\begin{align*}
\vert\mathrm{det}(1-J_{p}^{r})\vert & =\left|\prod_{e}(1-\Lambda_{p,e}^{r})\prod_{c}(1-\Lambda_{p,c}^{r})\right|=\vert\Lambda_{p}\vert^{r}\left|\prod_{e}(1-\frac{1}{\Lambda_{p,e}^{r}})\prod_{c}(1-\Lambda_{p,c}^{r})\right|\\
 & =\vert\Lambda_{p}\vert^{r}
\end{align*}
Where $\vert\Lambda_{p}\vert^{r}$ is the product of all the expanding
eigenvalues. So the spectral determinant is now replaced by the dynamical
zeta function:
\begin{align*}
\frac{1}{\zeta(z,\beta)} & =\exp\left(-\sum_{p}\sum_{r=1}^{\infty}\frac{1}{r}\frac{z^{n_{p}r}e^{r\beta\cdot A_{p}}}{\vert\Lambda_{p}\vert^{r}}\right)\\
...
Where 
\[
t_{p}=\frac{z^{n_{p}}e^{\beta\cdot A_{p}}}{\vert\Lambda_{p}\vert}
\]
is the ``weight'' associated with each prime cycle (for flows we
replace $z$ with $e^{-s}$ and $n_{p}$ with $T_{p}$). For each
$\beta$, the zeros of dynamical zeta functions could be evaluated
numerically. 
