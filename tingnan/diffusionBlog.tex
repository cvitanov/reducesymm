% GitHub cvitanov/reducesymm/tingnan/diffusionBlog.tex
...
\item[2014-05-05 Tingnan] Adapted from \cite{Kimball01} on soft billiards. Let $\b_i$ and $\phi_i$ be the impact parameter and propagation angle(in the global frame) before i-th collision. We can then compute the Monodrome matrix. Suppose the scattering funtion is provided by $\theta(b)$, we have:
\[
\phi_{j+1} = \phi_j + \theta_j(b_j);
\]
\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{Kim_Fig1}
\end{center}
\caption{
	A portion of a path;
}
