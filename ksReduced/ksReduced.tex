...
\documentclass[aip,cha,showpacs,reprint]{revtex4-1} % preprint
                \else
\documentclass[aip,cha,showpacs,twocolumn,reprint]{revtex4-1}
...
\RLD{2012-01-06: I find it rather hard to follow what exactly you are doing above.  I'm wondering whether it is possible to have something more straightforward.  For example, reading \rf{AGHO288} awhile ago, I was thinking we could just use their SO(2) invariant variables.  For the 1st and 2nd Fourier modes, they take $|a_1|^2$, $|a_2|^2$, and $a_1^2 \bar{a_2} + \bar{a_1}^2 a_2 = 2 r_1^2 r_2 \cos(2\theta_1 - \theta_2)$, where I use overbar to denote complex conjugate.  For every additional mode $k$, we can add two more variables: $|a_k|^2 = r_k^2$ and $a_1^k \bar{a_k} + \bar{a_1}^k a_k = 2 r_1^k r_k \cos(k\theta_1 - \theta_k)$.  In these variables the equilibria will not collapse into the origin.}
...
\RLD{2012-01-06: Because of this choice of coordinates $v_k$, I find it rather hard to understand exactly what I'm looking at.  I understand when coordinates are the GSO of eigenvectors, but I cannot imagine what transformations \refeq{eq:SO2polarExt} do to them.  I would think that, since we have introduced invariant coordinates, why not just use some of them as coordinate axes for the plots.  Then we know exactly how the coordinates in the plots are defined.}}
\end{figure}
