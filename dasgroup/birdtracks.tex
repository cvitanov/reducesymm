...
I am interested in the "quantum marginal problem" (QMP): is
...
$|\psi>$ on $J$ such that the partial trace of $|\psi> <\psi|$ over the
Hilbert space complementary to $X_{S_i}$ equals $\rho_i$? That is, in
standard notation, is there a $|\psi>$ such that
$\Tr_{J\setminus{S_i}}[|\psi> <\psi|] = \rho_i$ for each $i$?
...
{Alexander Klyachko}\rf{Klyachko06}
...
\arXiv{2211.00685},
gives a general (but abstract) solution to QMP.
...
This literature will take some/much time to digest. In case it helps, here is
a Thomas the graduate student video on this paper:
...
plus another video on The Problem maybe useful (have not listened to it)

\videoLink{youtube.com/watch?v=90OExPCuiPg} Otfried Guehne :
{\em The Quantum Marginal Problem} (2022)

\item[2023-01-03 Erik Aurell]

The Otfried Guehne video by  is up to 32 min at least partly about other (but
related) problems. I read the paper he is referring to (though not in great
detail). I know other work by his postdoc Nguyen. With Johannes Berg and
Riccardo Zecchina Nguyen wrote a very good review on the "inverse Ising
problem" (classical systems).

At 32 min Guehne starts on the problem as I see it.
...
much easier later paper for Gaussian states by Eisert and collaborators. 

At around 43minutes or so comes the construction of Guehne which is in the
...
of course be wrong on this point (2). After 44 minutes I think Guehne again
looks at a special case, maybe not generally useful.

The last part of the talk is I think a bit inconclusive (or not quite
up-to-date). The case "AME(4,6)"  was solved in
%
    Suhail Ahmad Rather, Adam Burchardt, Wojciech Bruzda, Grzegorz
    Rajchel-Mieldzioć, Arul Lakshminarayan, Karol Życzkowski\rf{RBBRLZ22}
    {\em Thirty-six entangled officers of Euler: Quantum solution to a
    classically impossible problem} (2022).

I am not sure if there is not some almost-cheating behind this.  {\.{Z}}yczkowski
was one of the co-authors of the list of open problems Guehne refers to.
Maybe {\.{Z}}yczkowski knew that he could solve that problem when he put together
the list of open problems together with Pawel Horodecki. I do not know.
