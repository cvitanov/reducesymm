...
`templates.' In spirit it is akin to the
\HREF{http://en.wikipedia.org/wiki/Vector_quantization}{`vector
quantization,'} (`block quantization,'  `pattern matching
quantization'), a data compression method where sets of points are
clustered by their distance to `prototype' or `reference' points. It
...
dimension; \ie, what in the theory of dynamical systems is called `symbolic dynamics.'
...
\,.
\]
\ie, $\braket{\sspRed}{\sliceTan{}}=c$. In the case
of a slice hyperplane, 
the extremal distance condition \refeq{PCsectQ0} sets $c=0$, so all our slice
hyperplanes
include the point at the origin, $\sspRed=0$. A
$(d\!-\!1)$-dimensional hyperplane $\pSRed$ embedded in a $d$-dimensional space
and passing through the origin is defined by the normal vector \sliceTan{},
...
The intersection of two $(d\!-\!1)$-dimensional
hyperplanes is a $(d\!-\!2)$-dimensional
hyperplane - the `ridge' ( `boundary,'  `edge'), 
a hyperplane which also goes through the
origin. If you are an ant crawling along the trajectory $\sspRed{}^{(1)}(\tau)$
symmetry-reduced with respect to the first slice $\pSRed{}^{(1)}$, 
you do not need to
compute this hyperplane. The moment 
$\braket{\sspRed{}^{(1)}(\tau)}{\sliceTan{}{}^{(2)}}$ changes sign, you the
ant have crossed the ridge, we symmetry-reduce with respect to the second
slice, 
...
fix the relative phases of different \reqva.
