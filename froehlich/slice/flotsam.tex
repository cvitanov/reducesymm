...
``
An inconvenience inherent in the linear slices formulation is that they
are local, and the reduced flow encounters {\sset s} in subsets of the
\reducedsp, with the reduced trajectory exhibiting large, slice-induced
jumps. This \sset\ is introduced by and depends on the \slice-fixing
condition. We have shown, in the $5$-dimensional \cLe\ example, that the
...
not intersect the {\sset}. The trick is to construct a good set of
symmetry in\-vari\-ant {\PoincSec}s, and that is a dark art \edit{for
systems of dimension higher than three}, with or without a symmetry. In
