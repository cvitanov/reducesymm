...
Stefan, I am a bit worried about the proof of section \refsect{sect:slices}. Do you take it to account
...
\\
Labels `suspectEigVecs' `eq:ipLaser' `eq:CLEip'
`eigVecQ1' `exer:PCsectionCLe' `exer:2mEigenE0' multiply defined.
\\
References `fig:CLEE0' `fig:CLEE0'  `exer:SO2cSect'
 `exer:SO2cSect' `EqMotionMovFramePC' `exer:csectionCLe'
 `exer:csectionCLe'
...
$\dotProd{f(\ssp)}{f_0(\ssp+c)}$, where the inner product is the
standard inner product on $L^2[0,2\pi]$ \refeq{innerL2}.

If $c$ maximizes $\dotProd{f(\ssp)}{f_0(\ssp+c)}$, then assuming
differentiablilty it must be a critical point \beq
\partial_c \dotProd{f(\ssp)}{f_0(\ssp+c)}=0
\eeq which is equivalent to $\dotProd{f(\ssp)}{f_0'(\ssp+c)}=0$.
Shifting the integral by $c$ we get the condition \beq
\dotProd{f(\ssp-c)}{f'_0(\ssp)}=0. \eeq
...
$c(\tau)$ such that $\dotProd{\bar{u}}{u'_0}=0$, where
...
trajectory is found by maximizing $\dotProd{f(\ssp)}{f_0(\ssp+c)}$,
which is the function used in \refsect{sect:slices}.
