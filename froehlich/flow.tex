...
The \jacobianM\ also maps the initial, Lagrangian coordinate frame to the Eulerian coordinate from at time $t$. To see this, consider two points that are an infinitesimal time apart along a trajectory: $\delta \xInit = f^{\delta t}(\xInit)-\xInit=v(\xInit)\delta t$. We also know $f^t\left\{f^{\delta t}(\xInit)\right\}=f^{\delta t}(f^t(x+0))$, where $f^{\delta t}(f^t(\xInit))=f^{\delta t}(x(t))=x(t) + v(x(t)) \delta t$ and $f^t(f^{\delta t}(\xInit))=f^t(\xInit+v(\xInit) \delta t) \approx f^t(\xInit) + J^t(\xInit) v(\xInit) \delta t$. Putting these two equations together we get the desired result, $v(x(t))=J^t(\xInit)v(\xInit)$.

If the equations of motion is all that is known about the system then it is simple to calculate the stability matrix but the trajectories are not known so it is not possible to calculate the \jacobianM\ using the given formula. Thankfully there are other methods of calculating the \jacobianM; in ChaosBook.org\rf{DasBuch} it is shown that the \jacobianM\ matrix is related to the stability matrix by $\frac{d}{dt} J^t(x)=\Mvar (x) J^t(x)$ or equivalently $J^t(\xInit)=e^{\int^t_0 d\tau \Mvar (x(\tau))}$. When using a numerical routine to integrate the equations of motion, using a discrete approximation of either of these two formulas requires minimal additional programming effort.
...
In order for a trajectory to stay in the group orbit, its velocity must always point in the tangent direction to the group action, so we must have $v(x) = c (t)\, T\cdot x(t)$ where $c(t)$ is a scalar function in terms of time. We also know that $x(t) = g(t) x(0)$ for some element $g(t)$ of the symmetry group. Plugging this into the first equation, we get that
\bea
v(g(t) \xInit) = c(t)\, T\cdot g(t) \xInit \cont
g(t) v(\xInit) = g(t) c(t) \, T\cdot \xInit\cont
v(\xInit) = c(t)\, T\cdot \xInit
\eea
both $v$ and $T\cdot \xInit$ are independent of time and we also have that $c = c(0)$ at $\xInit$, so $c(t) = c(0)$ for all $t$, so $c$ is constant. $c$ is called the 'angular' velocity of the {\reqv}.
