...
\begin{quote}
{\bf Abstract:} Stefan, write this: often! this might be the only part
...
\,.
\]
Define the group action tangent at $\ssp$ as
\beq
...
[...]
    \PC{incorporate this into the text:``
We use $\groupTan(\sspRed)_a$ notation (rather than
...
\label{def:slice}
\index{slice}
A dot product $\dotProd{\ssp_1}{\ssp_2}$,
$\ssp_1, \ssp_2\in \pS$. The associated norm is
$|\ssp|^2 = \dotProd{\ssp}{\ssp} \neq $
unless $\ssp = 0$. Matrix representation $\LieEl$ of a
compact Lie group \Group\ acts as
$\dotProd{\ssp_1}{\LieEl\ssp_2} =
  \dotProd{\dual{\LieEl}\ssp_1}{\ssp_2} $,
where $\dual{\LieEl}$ is the hermitian conjugate of $\LieEl$.
    \PC{Define these properly.
    When we get to fluids, it's not trivial - we will
    use `energy norm.'
    Perhaps instead of `dot product' use `bilinear form' or ?}
\end{definition}
...
\index{slice}
\textbf{\Slice}:
Pick any non-zero vector $\slicep \in \pS$.
...
\) %ee{PCsectQ1}
is
normal to all group tangents
$ \sliceTan{a}= \groupTan_{a}(\slicep) = \Lg_a \slicep$ at the
\emph{slice-fixing point} $\slicep$ a \emph{\slice}.
As
$
\dotProd{\slicep}{\sliceTan{a}}=\dotProd{\slicep}{\Lg_a \slicep} =0
$
by the antisymmetry/antihermiticity of \Lg, the condition that
a point \sspRed\ lies in the \slice\ \pSRed\ is
    \PC{I believe any \slicep\ will do, even an invariant
        point in $\pS_\Group = \Fix{\Group}$, \refeq{dscr:InvPoints},
        as long as $|\slicep| \neq 0$.
        For example, a point on $z$ axis for \cLe. Recheck}
    \SF{If \slicep\ is invariant under the group, then isn't $\sliceTan{a}=0$ so the slice is the entire
        space?}
        \PC{Note one is defining a slice-fixing \emph{ray} $const \slicep$,
        not a point - the condition is satisfied by any multiple of
        \sliceTan{a}.
        }
\beq
...
Let $\ssp \in \pS$ be a point in the \statesp.
By assumption the Lie group of symmetries is compact
and connected
    \PC{consider omitting `connected;' for a group like $\On{n}$
        you have two connected components, proof presumably still works?}
    \SF{I guess we don't need connected, we just need a representation of the group elements that is differentiable,
        and there is one for $\On{n}$ isn't there?}
so we can represent any group element by $\LieEl=e^{\gSpace \cdot \Lg}$ where $\gSpace \cdot \Lg=\sum \gSpace_a \Lg_a$. We want to show that there is a point on the group orbit of $\ssp$ which lies in \pSRed, \ie\ there exists a group element $\LieEl=e^{\gSpace \cdot \Lg}$, $\sspRed= \LieEl \ssp \in \pSRed$ such that $\dotProd{\LieEl \ssp}{\sliceTan{a}}=0$ for all $a$.

Consider $f(\gSpace)=\dotProd{e^{\gSpace \cdot \Lg} \ssp}{\slicep}$, the projection of the group orbit of $\ssp$ onto the slice-fixing point \slicep. As long as $\ssp$ is not in the invariant subspace \refeq{dscr:InvPoints}, the projection of its group orbit $f$ is a continuous and differentiable function of \gSpace\ (if
$\ssp$ is invariant, its group orbit is itself, just a point).
    \SF{$f$ is constant if the point is in the invariant space so it is still continuous and differentiable and the
        group orbit is a single point so it is compact. Doesn't the proof still work for it then? (its just that it
        will be in the slice for all $\theta$)}
 If $\gSpace_E$ is an extremum of $f$ then all of the first order partial derivatives of $f$ vanish at $\gSpace_E$, $\frac{\partial f(\gSpace_E)}{\partial \gSpace_a} =\dotProd{\Lg_a e^{\gSpace_E \cdot \Lg} \ssp}{\slicep}=0$. $\Lg_a$ is anti-hermitian, so
$0=\dotProd{\Lg_a e^{\gSpace_E \cdot \Lg}\ssp}{\slicep} =\dotProd{e^{\gSpace_E \cdot \Lg} \ssp}{-\Lg_a \slicep}$, and
\[
\dotProd{e^{\gSpace_E \cdot \Lg} \ssp}{\sliceTan{a}} =0
\,.
\]
We therefore have that $e^{\gSpace_E \cdot \Lg} \ssp$ is normal to all the group tangents of $\ssp$ so it is in \pSRed. This means that the slice condition is satisfied by $\sspRed_E = e^{\gSpace_E \cdot \Lg} \ssp$ corresponding to an extremum of the $f$, and thus $\sspRed_E$ is in the slice. All that is left to do then is to show that $f$ has extrema. But as the group is compact, the group orbit of every point in \pS\ is compact, so its projection on the slice-point $\slicep$ has at least two extremal points, and thus every group orbit outside the invariant subspace
intersects the \slice.
For example, group orbits of \SOn{2}\ are topologically circles, and their
...
\bea
\dot{\gSpace_a}(\sspRed) &=& \frac{\vel(\sspRed)^T \sliceTan{a}}
