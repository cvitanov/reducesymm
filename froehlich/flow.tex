...
Consider the displacement of an infinitesimally close neighbor $x+\delta x$. Taylor expanding the flow equation $\dot x = v(x)$ we find that
\beq
...
quintessentially continuous symmetry effects, to \refsect{SF:relStab}.
...
If the equations of motion is all that is known about the system then it is simple to calculate the stability matrix but the trajectories are not known so it is not possible to calculate the \jacobianM\ using the given formula. Thankfully there are other methods of calculating the \jacobianM; in ChaosBook.org\rf{DasBuch} it is shown that the \jacobianM\ matrix is related to the stability matrix by
...
When using a numerical routine to integrate the equations of motion, using a discrete approximation of either of these two formulas requires minimal additional programming effort.
...
The Jacobian of a periodic orbit has marginal eigenvalues when there is either a continuous symmetry (which can use to reduce the problem) or a non-hyperbolicity in the flow (which is not as easy to deal with).

To see that continuous symmetries have a corresponding marginal eigenvalue, let $\ssp$ be any point on a periodic orbit p with period $T_p$. If $\Group$ is any element in the Lie group for the continuous symmetry, then $f^t(\Group \ssp) = \Group f^t(\ssp)$. We already know that $\jMps^t(\ssp) \delta \ssp = \delta f^t(\ssp)$. If we let $\Group = 1+\delta \theta \Lg$ be any infinitesimal Lie group element and $\delta \ssp = \ssp - \Group \ssp = \ssp - (1+\delta \theta \Lg) \ssp = \ssp-(\ssp + \delta \theta \Lg \ssp =\delta \theta \Lg \ssp$ then $\delta f^{T_p}(\ssp) = f^{T_p}(\ssp) - f^{T_p}(\Group \ssp)= f^{T_p}(\ssp) - \Group f^{T_p}(\ssp) = \ssp - \Group \ssp = \delta \theta \Lg \ssp$.
This gives us $\jMps_p(\ssp) \delta \theta \Lg \ssp = \delta \theta \Lg \ssp \Rightarrow \jMps_p(\ssp) \groupTan(\ssp) = \groupTan(\ssp)$ where $\groupTan(\ssp)$ is the group tangent at $\ssp$. 
...
so $\velRel$ is constant. If \SOn{n} is a rotational symmetry,
