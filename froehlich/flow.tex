...
Our goal is to show that if we choose the slice to be the hyperplane normal to the group tangents at a point, then this slice interesects all group orbits.\\
\\
...
We want to show that there is a point on the group orbit of $\ssp$ which lies in $\bar{\pS}$, \ie\ there is a group element $\LieEl=e^{\gSpace \cdot \Lg}$ such that $<\LieEl \ssp|\groupTan_a(\ssp')>=0$.\\
\\
To show this we will look at the function $f_{\ssp}(\gSpace)=<\ssp'|e^{\gSpace \cdot \Lg} \ssp>$.\\
$f_{\ssp}$ is a continuous and differentiable function by how it was defined (the inner product is linear so it is continuous and $e^{\gSpace \cdot \Lg}$ is differentiable so $f_{\ssp}$ is also).\\
This means that if $\gSpace_0$ is an extremum of $f_{\ssp}$ then all of the first order partial derivatives of $f_{\ssp}$ will be zero at $\gSpace_0$ since $f_{\ssp}$ is differentiable.\\
We have that $\frac{\partial f_{\ssp}}{\partial \gSpace_a}=0$, so $<\ssp'|\Lg_a e^{\gSpace \cdot \Lg} \ssp>=0$.\\
$\Lg_a$ is anti-hermitian, so $0=<\ssp'|\Lg_a e^{\gSpace \cdot \Lg} \ssp>=<-\Lg_a \ssp'|e^{\gSpace \cdot \Lg} \ssp>=-<\Lg_a \ssp'|e^{\gSpace \cdot \Lg} \ssp>$ giving us that $<\groupTan_a(\ssp')|e^{\gSpace \cdot \Lg} \ssp>=0$.\\
We therefore have that $e^{\gSpace \cdot \Lg} \ssp$ is normal to all the group tangents of $\ssp$ so it is in $\bar{\pS}$.\\
This means that any extrema of $f_{\ssp}$ are in the slice.\\
All that is left to do then is to show that $f_{\ssp}$ has extrema.\\
\\
Define $h(y)=<\ssp'|y>$.\\
$h$ is continuous since inner products are continuous.\\
