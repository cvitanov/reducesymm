...
\eea
where
$\jEigvec[j]\left(\ssp'\right)=\jMps^t\left(\ssp\right)\jEigvec[j]\left(\ssp\right)
.$
So $\ExpaEig_j$ is a Floquet multiplier for $\jMps_p\left(\ssp'\right)$ too.
...
{\FloquetM} for a \rpo\ as the {\jacobianM} evaluated
...
orbit p with period $T_p$. If $\LieEl$ is any element in the
the continuous symmetry Lie group $\Group$, then $f^t(\LieEl \ssp)
= \LieEl f^t(\ssp)$. We already know that $\jMps^t(\ssp)
...
or
\beq
 \jMps_p(\ssp) \groupTan(\ssp) =
\groupTan(\ssp)
\,,
\ee{SF:symMargEig}
where $\groupTan(\ssp)$ is the group tangent
at $\ssp$.
...
f^T\left(\ssp\right) \Rightarrow 0 =
\ssp-f^T\left(\ssp\right)$ (where $f^T$ is the trajectory at
time $T$). We are looking for zeros of the function
$F\left(\ssp,T\right) = \ssp - f^T\left(\ssp\right)$, so we
can use Newton's method to find the zeros. Let
$\left(\ssp,T\right)$ be an initial guess for Newton's method
close to an actual solution $\left(\ssp + \Delta x,T + \Delta
T\right)$. Then $0 = \ssp +\Delta \ssp - f^{T+\Delta
T}\left(\ssp + \Delta \ssp\right)$. Taking the Taylor series
to linear order we get the Newton iteration $0 \approx \ssp-
...
$\jMps\left(\ssp\right) v\left(\ssp\right) =
v\left(\ssp\right) \Rightarrow
\left(I-\jMps\left(\ssp\right)\right) v\left(\ssp\right) =
0$. This means that $I-\jMps\left(\ssp\right)$ is
non-invertible so as $\ssp$ approaches the periodic orbit
$\left(I-\jMps\left(\ssp\right)\right) v\left(\ssp\right)
\approx 0$. If $\Delta x$ is in the direction of
...
transverse to the flow during Newton's method. We can do this
by requiring $\Delta x$ satisfy something like
$v\left(\ssp\right) \cdot \Delta \ssp = 0$.
...
multiplier. So very every unit Floquet multiplier we have
this difficult and need to add another constraint. It was
already shown that for every dimension of a continuous
symmetry there is a unit Floquet multiplier. For each of
these symmetry dimensions you can just add a constraint
...
both equal to 0. This cost function in a sense measures how
...
orbit or learn that there aren't any near our initial guess
