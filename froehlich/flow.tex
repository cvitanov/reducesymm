...
The \jacobianM\ also maps the initial, Lagrangian coordinate frame to the Eulerian coordinate from at time $t$. To see this, consider two points that are an infinitesimal time apart along a trajectory: 
$\delta \xInit 
  = \flow{\timeStep}{\xInit}-\xInit \approx \velField{\xInit}\timeStep$. 
We also know $\map^t \circ \flow{\timeStep}{\xInit}=\map^{\timeStep} \circ \flow{t}{\xInit}$, 
where $\flow{\timeStep}{\map^t(\xInit)}=\flow{\timeStep}{\ssp(t)}
...
Putting these two equations together we get the desired result, $\velField{\ssp(t)}=\jMps^t(\xInit) \velField{\xInit}$.

If the equations of motion is all that is known about the system then it is simple to calculate the stability matrix but the trajectories are not known so it is not possible to calculate the \jacobianM\ using the given formula. Thankfully there are other methods of calculating the \jacobianM; in ChaosBook.org\rf{DasBuch} it is shown that the \jacobianM\ matrix is related to the stability matrix by $\frac{d}{dt} \jMps^t(x)=\Mvar (x) \jMps^t(x)$ or equivalently 
