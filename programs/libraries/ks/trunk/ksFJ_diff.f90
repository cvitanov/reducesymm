...
!!!!
complex(dpc), dimension(size(bc)/2+1) :: ai,af,adum,aidum,afp,afm,Rai,Raf
...
real(dp), dimension(size(bc),size(bc)) :: R_r
real(dp) :: da=0.00000001_dp
...
do i=2,d/2+1	
	...
	call etdrk4DiagDriverS(ti,aidum,Nsteps,tf,afp,f0,f1,f2,f3,e,e2,Nplt,SetNlin_KS)
	aidum(i)=real(ai(i))-da + ii*aimag(ai(i))
