...
%  function g = ksfmf5(t, a, d, h, C, alpha) 
%  Associated flow for the full KSFM
%  5 - for relative periodic orbits (RPOs)
...
global TT A F FT NFEVAL PHASE DD
  ...
  if nargout > 1,
    ...
    jac = [[dug dtg ddg]; [-alpha(1).*dtg'; -alpha(2).*ddg']*[dug dtg ddg]];
    jac(na+1,:) = jac(na+1,:) - alpha(1).*g'*[dutg dt2g ddtg];
    jac(na+2,:) = jac(na+2,:) - alpha(2).*g'*[ddug ddtg dd2g];
    jac = C*jac;
  end
  
  g = C*[g; [-alpha(1).*dtg'; -alpha(2).*ddg']*g]; 
%  g(1:2) = 1e4.*g(1:2);  jac(:,1:2) = 1e4.*jac(:,1:2);
return;
