...
number of positive/marginal Floquet (or Lyapunov) exponents, twice its
...
section-fixing points, such that each is approximately tangent to the
...
subspace\rf{Christiansen97,lanCvit07}; examples of flows with continuous
symmetries are the \KS\ flow on a periodic domain\rf{SCD07}, the pipe
flow\rf{ACHKW11} and the \pCf\rf{GHCW07}.

We need to make a global chart by deploying linear \Poincare sections in
...
the dimensionally \reducedsp\ $\pS^{(?)}$ by deploying linear \Poincare
sections across neighborhoods of the qualitatively most important
patterns. We shall refer to these states as \emph{\template s}, each
...
\,,
\ee{locTransvVar1}
locally normal to the $\vel(\slicep)$ at the \template\ point.
(For a motivation, see
\refappe{s-POs-flows} \emph{Newton method for flows}.)
Each \Poincare section $\PoincS{}^{(j)}$, provides a local chart for a
...
to talk about local charts here, rather than \Poincare sections.
