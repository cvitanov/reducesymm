...
\ES{How could a trajectory be confined to a {\Poincare} section? You seem
to talk about local charts here, rather than {\Poincare} sections.}. The
...
\reffig{f_1_08_1} helps visualize the problem; imagine that the
tiles belong to the
...
While it has been demonstrated in \refref{SiCvi10}  that the \mframes, with a judicious choice of the {\template} and {\PoincSec}, works for a system as simple as the \cLf, one still has to show that the method can be implemented for a truly high-dimensional flow. Siminos\rf{SiminosThesis} has used a modified \mframes\ to compute analytically a 128\dmn\ invariant basis for \reducedsp\ $\PoincS = \pS/\SOn{2}$, and shown that the unstable manifolds of \reqva\ play surprisingly important role in organizing the geometry of \KS. In \refref{SCD07} it was found that the coexistence of four \eqva, two \reqva\ (traveling waves) and a nested \fixedsp\ structure in an effectively $8$-dimensional \KS\ system complicates matters sufficiently that no dimensionality reduction by the \mslices\ has been attempted so far. More importantly, a dimensionality reduction of pipe flows remains an outstanding challenge\rf{ACHKW11}.
...
developed while reading Roweis and Saul\rf{RoSa00}. I believe what we ought to 
...
to implement the final step in the procedure of Roweis and Saul\rf{RoSa00}, 
...
it in \refsect{sec:chart} would work. {\Poincare} sections are designed
