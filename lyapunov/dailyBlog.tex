...
Concerning Froyland \etal's paper,
 while it may complement some aspects
 that are not discussed in other reviews,
 I don't think it has priority over them.
In particuler, their "improvement" on Ginelli \etal's method is quite minor
 and in my view can be even harmful, losing speed (and simplicity of the code)
 for somewhat decreasing a risk of numerical instability;
 however, this instability can be overcome easily
 in Genelli \etal's original algorithm,
 by changing the time step
 and the period between consecutive Gram-Schmidt orthogonalizations.
Anyhow, strangely enough, they do not compare their improved algorithms
 with the original ones
 when evaluating the performance of the different schemes.
They seem to like so much the singular value decomposition
 which is a numerically heavy task to do,
 and do not discuss practical issues like memory storage
...
\item[2013-02-21 Kazumasa]
Predrag, you are the first (and so far only) person
 who found this online article and gave me a sign.
I am delocalized over real and \textit{in silico} worlds..!
 (mainly out-of-equilibrium stat. mech. probed by liquid-crystal convection)
