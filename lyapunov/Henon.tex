...
In 1985 Grassberger and Kantz\rf{GrKa85} formulated the `partition conjecture'
    \ifdasbuch
    \PublicPrivate{}{
...
Ishii\rf{Ishii97} for the Lozi map, and A. de~Carvalho and
...
standard map by Dullin \etal\rf{HamSteDuMei04}, among others. For large
...
\remark{Lozi map pruning:}{ \label{rem:LoziMapPrun}
    \ifdasbuch
(continued from \refexam{exmp:LoziMap} and
\refrem{rem:HenLoz})~~
    \else
    \fi
\PC{We $\to$ refrefs}
The simplest illustration of pruning fronts is afforded by the Lozi map
unstable \po s structure, without the H\'enon attractor subtleties
introduced by the possible coexistence of stable periodic orbits, or
worries about the uniqueness of periodic points corresponding to a given
symbol sequence. The Lozi map is linear and computationally fast, so the
strange attractor can be covered by hundreds of thousands of periodic
points without straining patience of a laptop. Analysis of the Lozi map
    \ifdasbuch
\refeq{e_lozi_def}
    \else
    \fi
is much simpler than that of the H\'enon map, as the stable / unstable
manifolds $\hat{W}^s$ / $(\hat{W}^u)$ are piecewise linear and easily
plotted, all \pturn s of $W^u$ lie on the line $x=0$, and the symbol
corresponding to a point $(x_n,x_{n+1})$ is given by the sign of
$x_{t+1}$. For a study of bifurcations in the Lozi map, see
\refref{BCOR11}.
\index{Lozi map!bifurcation}
\index{bifurcation!Lozi map}
    }%end  \remark{Lozi map pruning:}
...
Kantz\rf{GrKa85}, however is a key contribution, and Peter understood
...
Click
\HREF{http://chaosbook.org/library/AGIP90}{here} for \refref{AGIP90}.
Some nice figures, very well written, worth a look.
They write: ``
Our final conclusion is that both the Grassberger and Kantz\rf{GrKa85}
...
H\'enon parameters choice. However, Lan\rf{lanCvit07} was able to find a
