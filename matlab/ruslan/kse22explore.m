...
% there exist also two TWs with speeds -c.
...
%% Unstable manifolds of the Trevelling Waves
%  TW1 has two pairs of unstable complex conjugate eigenvalues and thus two
%  unstable planes.  TW2 has one real unstable eigenvalue, so the unstable
%  manifold is one-dimensional.
  clear; load kse22orbits; k = 2; tend = 200;
  a0 = re(k).a + 1e-4.*re(k).evec(:,1);
  [tt, aa] = ksfmedt(L, tend, a0, h, 2); [xx, uu] = ksfm2real(aa, L);
  figure(1); clf; set(gcf,'pos',[5 100 200 800]);
  pcolor(xx,tt,uu'); caxis([-3 3]); tall(1.1); shading flat;

%%
%
  clear; load kse22orbits;  k = 1; 
  m = 1; tend = 50; epsilon = 1e-3;
  %m = 3; tend = 70; epsilon = 1e-2;
  ere = real(re(k).eig(1)); period = 2*pi/imag(re(k).eig(1));
  v = gsorth([real(re(k).evec(:,m)) imag(re(k).evec(:,m)) real(re(k).evec(:,6))]);
%  for delta = (0:0.1:6)*pi/3,
%    a0 = re(k).a + 2e-2.*(v(:,1).*cos(delta)+v(:,2).*sin(delta));
  for delta = (0:0.02:1)*ere*period,
    a0 = re(k).a + 1e-2.*exp(delta).*v(:,1);
    [tt, aa] = ksfmedt(L, tend, a0, h, 2); 
    va = aa(1:2:end,:) + 1i*aa(2:2:end,:);
    va = va.*exp((-2i*pi*re(k).c/L).*(1:31)'*tt);
    aa(1:2:end,:) = real(va);  aa(2:2:end,:) = imag(va);
    if 0, [xx, uu] = ksfm2real(aa, L);
      figure(1); clf; set(gcf,'pos',[5 100 200 800]);
      pcolor(xx,tt,uu'); caxis([-3 3]); tall(1.15); shading flat; pause;
      title(num2str(delta)); end
    if delta == 0, av = v'*aa;
    else aa = [aa repmat(NaN,size(aa,1),size(av,2)-size(aa,2))]; av = [av; v'*aa]; end
  end
  figure(2); set(gcf,'pos',[5 400 420 430],'paperpos',[5 8 11 11]); clf;
  ax1 = axes('pos',[0.12 0.08 0.83 0.90]);
    plot3(av(1:3:end-2,:)',av(2:3:end-1,:)',av(3:3:end,:)','-','color',[.4 .4 .4]); 
  hold on;
    if 1, phase = 0:0.01:1;  ve = eq(3).a(1:2:end)+1i*eq(3).a(2:2:end);
    ve = repmat(ve,1,length(phase)).*exp(-2i*pi*(1:31)'*phase);
    ae = zeros(size(aa,1),length(phase)); grid on;
