...
persistent hax hp1 hp2 hp3 hp4 hp5 hp6 hp7 nitr x k Ap ddo an han stopcnt

%  optimValues,  
%  residual = norm(optimValues.fval).^2,
  stop = 0;  np = 5;  danmax = 2e-3;
  switch state
  case 'init'
...
%    disp(size(exp((-2i*pi*PHASE./DD).*k)));
    ...
    nitr = input('NITR = ');  stopcnt = 0;
  case 'iter',  residual = norm(optimValues.fval).^2;
    ...
    if size(an,2)>=np, an = [an(:,2:end) a]; han = [han(2:end) residual]; %optimValues.firstorderopt]; %residual]; 
    else an = [an a]; han = [han residual]; end %optimValues.firstorderopt]; end %residual]; end
    ...
    disp(sprintf('%5d %5d %10.6f %9.6f %13.6e %13.6e %12.3e %9.1e %9.1e %9.1e', optimValues.iteration, ...
         optimValues.funccount, TT(end), PHASE, residual, dd(end), dstp, han(end)./(han(end-1)-han(end)), dand, danp));  ddo = dd(end);
% additional stopping criterium based on slow convergence
    if han(end)./(han(end-1)-han(end)) > 2e4, stopcnt = stopcnt + 1;
    else stopcnt = 0; end
% curve search
    ...
    if optimValues.iteration >= nitr,  nitr = optimValues.iteration + input('NITR = ');  else,  drawnow;  end
    if stopcnt == 10,  nitr = optimValues.iteration + input('NITR = '); stopcnt = 0; end
