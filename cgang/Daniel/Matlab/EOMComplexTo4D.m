...
syms x1 y1 x2 y2 a1 a2 b1 b2 c1 c2 mu1 mu2 e1 e2 real
...
% eq2 = subs(x2dot,{x1,x2,y1,y2},{r1*cos(theta1),r1*sin(theta1),r2*cos(theta2),r2*sin(theta2)})...
%       -r1dot*sin(theta1) - r1*cos(theta1)*theta1dot;
% eq3 = subs(y1dot,{x1,x2,y1,y2},{r1*cos(theta1),r1*sin(theta1),r2*cos(theta2),r2*sin(theta2)})...
%       -r2dot*cos(theta2) + r2*sin(theta2)*theta2dot;
% eq4 = subs(y2dot,{x1,x2,y1,y2},{r1*cos(theta1),r1*sin(theta1),r2*cos(theta2),r2*sin(theta2)})...
...
% disp('for {x1,x2,y1,y2} and solving for the velocities in these coordinates, we get:')
...
% disp('Recasting the equations in terms of phi = 2*theta1 - theta2,')
% disp('we can reduce the problem to a 3D system')
% r1dot = simplify(subs(r1dot,2*theta1-theta2,phi))
% r2dot = simplify(subs(r2dot,2*theta1-theta2,phi))
% phidot = simplify(subs(2*theta1dot - theta2dot,2*theta1-theta2,phi))
