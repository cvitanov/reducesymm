...
function [E,Lambda] = PKPointStability(X1,Y1,X2,Y2,params)
...
x1dot = a1*x1^3 + b1*x1*x2^2 + c1*x1*x2 + a1*x1*y1^2 + b1*x1*y2^2 + mu1*x1 + c1*y1*y2;
y1dot = a1*x1^2*y1 + c1*x1*y2 + b1*x2^2*y1 - c1*x2*y1 + a1*y1^3 + b1*y1*y2^2 + mu1*y1;
x2dot = a2*x1^2*x2 + c2*x1^2 + b2*x2^3 + a2*x2*y1^2 + b2*x2*y2^2 + mu2*x2 - c2*y1^2 + e2*y2;
y2dot = a2*x1^2*y2 + 2*c2*x1*y1 + b2*x2^2*y2 - e2*x2 + a2*y1^2*y2 + b2*y2^3 + mu2*y2;

% Define vectors of coordinates and time derivatives
X = [x1 y1 x2 y2]';
V = [x1dot y1dot x2dot y2dot]';
...
A = subs(A,{x1,y1,x2,y2,a1,a2,b1,b2,c1,c2,mu1,mu2,e2},...
    {X1,Y1,X2,Y2,A1,A2,B1,B2,C1,C2,MU1,MU2,E2});
