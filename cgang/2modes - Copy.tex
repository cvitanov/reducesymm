...
\newcommand{\version}{atlas ver. 1.0, Oct  1 2013}
...
If a dynamical system is $\Group$-equivariant under actions of
an $N$\dmn\ symmetry group $\Group$, its $d$\dmn\ \statesp\ is foliated
by $N$\dmn\ group orbits, and the symmetry-\reducedsp\
$\pS/\Group$ is $(d\!-\!N)$\dmn.
The simplest continuous symmetry groups are the 1-parameter rotation
group $\SOn{2}$ (a compact group) and the 1-parameter translation group
$T(1)$ (a noncompact group).
...
to study $\SOn{2}$-equivariant flows. Nonlinear flows, and in particular
nonlinear PDEs, such as the \KS\ expressed
in terms of complex Fourier coefficients $a_k(t)$:
\beq
\dot{a}_k= \pVeloc_k(a)
     = ( q_k^2 - q_k^4 )\, a_k
...
\ee{2m:expan}
(where $q_k = k/\tildeL$)
and \NS\ flows, mix an infinity of such
Fourier components. In practice, they are represented by
a finite number of Fourier modes; the most radical truncation that
still captures some qualitative features of chaotic flows is to keep
only a pair of Fourier modes. In complex representation they transform
as
\beq
...
particular, the parameters $(\mu_j-\ii\, e_j)$ is one of the ways in
...
possible if the dynamics is to be chaotic.
...
\left( \begin{array}{cccc}
      ...
      \end{array} \right)
\ee{LGTwoMode}
and the {\stabmat} $\Mvar = {\pde \vel}/{\pde \ssp}$
...
\end{widetext}
\normalsize
given in \refeq{2modeStabMatrix} into the
...
% (\ie, summing over $\velRel_a \groupTan_a$)
...
Consider a \statesp\ constructed from two complex Fourier modes
$m=(1,2)$, with the $\SOn{2} \simeq \Un{1}$ group action given by
rotation\rf{Dang86,AGHO288,PoKno05} \refeq{Dang86(1.1)aa}.
