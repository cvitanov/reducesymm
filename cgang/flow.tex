...
\beq
	...
	\label{e-1stmodeTransform}
\eeq
which yields $\sspRedC_1 = r_1$ and $\sspRedC_2 = r_2 e^{\ii \Phi}$. Corresponding
ODEs were obtained in \refref{PoKno05} by chain rule and substitution. 
Note that \mslices\ provides a general form \refeq{e-so2red1stmode} for 
symmetry reduced evolution 

\subsection{Slicing two modes}
\label{s-slicing}

As shown in \refeq{e-1stmodeTransform}, the \slice\ defined by the template
\refeq{firstmodetemp}, fixes the phase of the first Fourier mode to $0$. 
In a similar fashion, we can define a slice by fixing the phase of the second 
mode, in which case the symmetry reduced Fourier modes are obtained by
\beq
	\sspRedC_n = e^{-\ii n \phi_2/2} \sspC_n \, .
	\label{e-2ndmodeTransform}
\eeq
Note, however, $\phi_2 \in (0, 2 \pi]$ in practice can have discontinuities
of $2 \pi$, and this would translate to the transformation of the $\sspRedC_1$ 
by \refeq{e-2ndmodeTransform} as a phase discontinuity by $\pi$. This discontinuity
can be fixed by another transformation: Simply doubling the phase of 
$\sspRedC_1$ of \refeq{e-2ndmodeTransform}. We are going to refer the symmetry
reduced representation that was obtained by this procedure by $\tilde{\sspC}_n$
