...
\revision{long-term averages of observables, as well as their higher moments, 
can be generated from the derivatives of $s(\beta)$:
\bea
    \langle \obser \rangle &=& 
    \lim_{\zeit \rightarrow \infty} \frac{1}{\zeit} \langle \Obser^\zeit \rangle = 
        \left. \frac{\partial s_0 (\beta) }{\partial \beta} \right|_{\beta = 0} 
        \, , \continue
        \Delta_{\Obser} &=& 
        \lim_{\zeit \rightarrow \infty} \frac{1}{\zeit} 
        \langle (\Obser^{\zeit})^2 - \langle \Obser^\zeit \rangle^2 \rangle = 
        \left. \frac{\partial^2 s_0 (\beta) }{\partial \beta^2} \right|_{\beta = 0}
    \, ,
    \label{eq-moments}
    \\
    \, &\vdots & \nonumber
\eea
We will refer to $\Delta_{\Obser}$ in \refeq{eq-moments} as generalized 
diffusion constant, as it characterizes the growth of of the standard
deviation of the integrated observable. 
The integrated observable $\Obser$ is not necessarily a spatial 
displacement and can be any observable in a chaotic flow, hence the 
term `generalized diffusion constant'.} 
