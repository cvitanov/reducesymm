...
multiplicative weight $\exp(\beta \Obser^\zeit(\ssp))$
    \DB{2014-11-03}{What does ``attaching'' mean in this context... might be
    lacking technical rigor here. Predrag 2015-05-03 Better now?}
...
\refeq{e-EvOper} corresponding to the leading eigenvalue (\ie, the one
with the largest real part) for a given $\beta$,
\beq
    \left[ \Lop^t \rho_{\beta} \right] (\ssp)
    =
    e^{t s(\beta )} \rho_{\beta}(\ssp)
    \, .
    \label{eq-EigenvalueRel}
\eeq
Here $s(\beta)$ is the eigenvalue of $\Aop$. If the system under study is
...
infinitesimal times, one finds that the \revision{long-time averages of
observables, as well as of their higher moments, are given by the
...
\bea
    ...
    \, &\vdots & \nonumber
\eea
Since the integrated observable $\Obser$ can be any observable in a chaotic
flow, not only spatial displacement, we refer to
$\Delta_{\obser}$, the growth rate of the standard deviation of the
integrated observable $\Obser$, as the `generalized diffusion constant'.
    \PC{2015-05-03I have changed Burak's $\Delta_{\Obser}$ to
    $\Delta_{\obser}$, as $\Obser^\zeit$ depends on time, but $\obser$
    does not.}
		\DB{2015-05-06}{Why not just use $\Delta\obser$? That notation is pretty typical
		in quantum mechanics, right? Like when we write the Heisenberg uncertainty principle?}
...
\ie, the shortest \po s of period $T_p$. $\Obser_p$ is the value of
...
over the delta function contributes the factor of $1/\oneMinJ{r}$.
...
approximate the map in \reffig{fig:psectandretmap}\,(d) in such a way
...
\revision{Defining
\bea
    \langle \Obser \rangle_N &=& - \left. \partial F_N / \partial \beta
                                \right|_{\beta=0, s=s (0)}
                                \, , \label{e-ObserAvg} \\
	\langle \period{} \rangle_N &=& \left. \partial F_N / \partial s
                            \right|_{\beta=0, s=s (0)}
                         \, , \label{eq-Tavg} \\
    \langle \Obser^2 \rangle_N &=&
    	- \left. \partial^2 F_N / \partial \beta^2
          \right|_{\beta=0, s=s (0)}
          \, , \label{e-Obser2Avg} \\
    \langle \Obser \period{} \rangle_N &=&
    	  \left. \partial^2 F_N / \partial \beta \partial s
          \right|_{\beta=0, s=s (0)}
          \, , \label{e-ObserTAvg}
\eea
we write the \cycForm s as
\bea
    \langle \obser \rangle_N
            &=&  \left.
                  - \frac{\pde F_N}{\pde \beta} \,\Big/\, \frac{\pde F_N}{\pde s}
                  \right|_{\beta=0, s=s (0)} \, , \continue
            &=& \frac{\langle \Obser \rangle_N}{\langle \period{} \rangle_N } \, ,
    \label{e-Avga} \\
   \Delta_{\obser, N}
            &=&  \small{
  - \left.\frac{
          \frac{\pde^2 F_N}{\pde \beta^2}
           \,+\, 2 \frac{d s}{d\beta}
              \frac{\pde^2 F_N}{\pde \beta  \pde s}
           \,+\, \left(\frac{d s}{d \beta}\right)^2
                      \frac{\pde^2 F_N}{\pde s^2}
           }{\frac{\pde F_N}{\pde s}}\right|_{\beta=0, s=s (0)}}
           \, , \continue
           &=&
   \frac{ 1~}{\langle \period{} \rangle_N}
   \langle (\Obser-\period{}\,\langle  \obser \rangle)^2 \rangle_N
    \label{e-Avgsigma} \, ,
\eea
...
derivative, however, we can also develop \revision{an} interpretation for it by looking
...
\revision{corresponding diffusion constant as
\beq
...
\DB{2014-11-11}{What's with $G69$ here}.\refeq{e-GoldenMeanCycleExpansion} This claim is clearly supported by the
