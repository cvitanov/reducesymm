...
matrix $\monodromyRed$, which is the $(d-N-1)\times(d-N-1)$ reduced Jacobian for the \rpo\ 
evaluated on a Poincar\'e section in the \reducedsp . Eigenvalues of 
$\monodromyRed$ are those of the \rpo\ Jacobian \refeq{e-rpoJacobian} without 
the marginal ones. 
...
to the \refeq{e-tracem0} after the replacement 
...
continuous factorization \refeq{e-ReducedTraceFormula} manifest
...
\refeq{e-ReducedTraceFormula}. Moreover, the dynamical zeta function 
\refeq{e-DynamicalZeta}, which we will obtain after some algebraic manipulation 
...
\refeq{e-ClassicalTraceFormula} as it is the general formula, however, we are 
going to use the $0$th irreducible subspace contribution to the trace 
\refeq{e-tracem0} in the applications. As we stated earlier, \refeq{e-tracem0}
...
the classical trace \refeq{e-ClassicalTraceFormula}.
...
\beq
1 / \zeta = \prod_p (1 - t_p) \, \mbox{where}, \, t_p = \frac{1}{|\ExpaEig_p|} e^{\beta \Obser_p - s T_p} z^{n_p} .
\ee{e-DynamicalZeta}
...
\beq
    D = \frac{1}{2 d} \sigma_{\dot{\theta}}^2
      = \frac{\langle (\dot{\theta} - \langle \dot{\theta} \rangle)^2 \rangle}{2} ,
\eeq
