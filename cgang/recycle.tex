...
Integral along the parallel direction contributes the factors of
...
topological length up to $\cl{\pi} \leq N$,
\beq
    F_N(\beta , s) = 1 - \sum_{n=1}^{N} Q_n(s, \beta ) \, .
    \label{e-NthOrderSpectDet}
\eeq
In what follows, we shall drop the subscript, $F_N \to F$, but actual
calculations are always done for a range of finite topological truncation
lengths $N$. Remember that we are searching for the eigenvalues $s (
\beta)$ of the operator \Aop\ in order to compute the moments
\refeq{eq-moments}. $s ( \beta)$ are located at the zeros of the spectral
...
solution of $F(0, s(0)) = 0$ is small, but not exactly $0$. This
...
to the drifts in the configuration space. We define the
\revision{corresponding diffusion coefficient as
\beq
...
in \reftab{t-DynamicalAverages}
and \reftab{t-DynamicalAveragesNoGrammar}. Take, for example, the
...
most unstable cycles, thus, obtaining a slight underestimate of the Lyapunov
