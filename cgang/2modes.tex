...
\revision{There is an extensive literature on equivariant dynamics, 
which can be traced back to Poincar\'e's work on the 3-body problem
\rf{Poinc1896}. Early references in the modern dynamical systems 
...
by Golubitsky and Stewart \rf{golubitsky2002sp}, and Field 
\rf{field2007}. Our aim here is much more modest: We would like to 
provide a hands-on introduction to some of the concepts from 
equivariant dynamical systems theory, with an emphasis on the
application of the periodic orbit theory to these systems. To this end,
we undertake an applied approach, and illustrate each concept on a 
...
This paper provides a step-by-step tutorial on how periodic orbit 
theory can be applied to flows with continuous symmetries. 
\revision{While the example studied here has no physical significance,}
such an analysis should ultimately be applicable to turbulent 
flows, once sufficiently many exact invariant solutions become 
