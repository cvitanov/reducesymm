...
the group action. We demonstrate the method of slices by applying it on a
radically simple SO(2) invariant system where only two Fourier modes are present. 
We start with the most general two-mode system and discuss its representations
in the SO(2) equivariant state space, polar coordinates and invariant polynomial
basis and develop a method of extracting the \eqva\ of the system in the
invariant polynomial representation. We then focus on two examples with topologically
different chaotic dynamics corresponding to two the different choice of 
parameters and apply the method of slices on these systems. We show 
that one can determine the \rpo s of the system by constructing Poincar\'e
return maps on the slice hyperplane. We visualise each step of the study 
...
Today, it is possible to  [blah blah].
    \PC{why does the first page have only the title and authors?}
    \DB{It's something weird with the Chaos style file and the fact that we have been compiling as preprint instead of reprint. If you toggle this it looks fine. Do we have a plan for where we want to submit this?}
    \end{quotation}
...
by substitution $z_1 = x_1 + i\,x_2$, $z_2 = y_1 + i\,y_2$,
\bea
...
$(2e_1-e_2)$ matters, so for simplicity we set $e_1=0$. In the real
representation \refeq{2mode4D}, the $\SOn{2}$ group action
\refeq{Dang86(1.1)aa} is given by
$\ssp'= \exp\left( \theta \Lg\right)\ssp$,
where $\transp{\ssp} = \{ x_1, x_2,y_1, y_2\}$, and $\Lg$ is the
...
\,.
\ee{LGTwoMode}
That the real \twoMode\ system \refeq{2mode4D} is equivariant under
infinitesimal $\SOn{2}$ rotations is checked by inserting the Lie algebra
generator \refeq{LGTwoMode} and the {\stabmat} $\Mvar = {\pde \vel}/{\pde
\ssp}$ into the equivariance condition\rf{DasBuch}
    \PC{define $\groupTan(\ssp)$}
    \PC{No point of having the explicit formula
        \refeq{2modeStabMatrix} here, right?}
\beq
  \groupTan(\vel)  - \Mvar(\ssp) \, \groupTan(\ssp) =0
  \,.
\ee{inftmInv}
    \PC{some of this text copied from ChaosBook.org continuous.tex}
For a point $\ssp_\stagn$ in a \reqv\ orbit, the flow and group tangent
vectors coincide,
\(
