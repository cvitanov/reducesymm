...
local {\slice}s, implement symmetry reduction on the
...
The choice of a {\slice} is arbitrary and thus, after some
...
the {\slice} in the $k=1$-irreducible subspace which
introduces $b_1$ and $c_1$ through the substitution of
\refneq{eq:SO2stand} into \refneq{eq:SO2norm}. Note that the
{\slice} does not exist when $b_1=c_1=0$, as there the
...
that if we set up a {\slice} by $c_i=0$ then the
