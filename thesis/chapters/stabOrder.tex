...
% 1/\Lambda_0 = .18241823856341201233
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ES{I didn't expect so wild fluctuation in \reffig{fig:zetaStabOrder1}.
    ``At least they are all on the same plane.'' I'm checking my numerics again.
    Even though it's a simple algorithm there are many loops
    so there is room for error.
\\PC: As you are not writing up what you are doing, I have no way
of knowing whether this is right, or what. If the plot is of all
...
on the top as partially not shadowed cycles with lots of zeros. In \refref{AACII}
you can see that there is some trouble with hyperbolicity - in particular,
you should exclude the $\cycle{0}$ from the $1/\zeta(0,0)$, replace it
by the square root. Starting with $\ln M(\stabCutoff) \approx 7$ the
appearance of higher errors is suspicious, and by
