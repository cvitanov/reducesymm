...
$\ssp$, $\sspRed = \LieEl(\theta)\, \ssp$ is defined to be
the rotation of $\ssp$ that lies in the \slice. In the
formulation of Fels and
Olver\rf{FelsOlver98,FelsOlver99,OlverInv} such a map from a
\statesp\ point $\ssp$ to the group action $\LieEl(\theta)$
is called a \emph{moving frame}.
...
non-zero $\ssp_1^{(1)}$. The {\em entire} \statesp\ is then
...
$\sspRed_1=0,\,\sspRed_2>0$. Start at $\sspRed^{(0)}$ with $\sspRed_1^{(0)}
=0$, evolve for a finite time to $\ssp^{(1)}$. Compute
...
to satisfy the $\sspRed_1=0$ \slice\ condition. Repeat. The
trajectory remains in the 4$\dmn$ $\sspRed_1=0$
\reducedsp.
    \PC{shorten this, it is a repeat of the figure caption}
...
the group action that rotates  $\ssp$ into $\sspRed$ at time
$t$. The time derivative is then $\dot{\ssp}=
\vel(\LieEl\sspRed) = \dot{\LieEl}\sspRed + \LieEl\velRed$,
where the \reducedsp\ velocity fields is
...
\,,
\]
where we have used the equivariance condition. Substitution
\[
\LieEl^{-1} \dot{\LieEl} =
\LieEl^{-1}\frac{d~}{dt} e^{\gSpace \cdot \Lg } =
\dot{\gSpace} \cdot \Lg
\]
yields the equations for the flow in the
$(d\!-\!N)$-dimensional \reducedsp:
\beq
\velRed(\sspRed) = \vel(\sspRed) - \dot{\gSpace} \cdot \Lg \, \sspRed
\,,
\ee{reducFlow}
for any factorization of the flow of form
$\ssp(t)= \LieEl(t)\sspRed(t)$. To actually integrate these
equations we first have to define the flow factorization by imposing
conditions on $\sspRed(t)$, and then integrating phases $\gSpace(t)$
on a given \reducedsp\ trajectory $\sspRed(t)$.

Taking the time derivative of the fixed slice condition
\refeq{PCsectQ} we obtain the condition on the \reducedsp\
flow
\[
...
= 0
\]
which yields the group phases flow
for the slice fixed by \slicep,
\beq
\dot{\gSpace}_a = \frac{\vel \cdot \Lg_a \slicep }
                       { (\Lg \sspRed) \cdot \Lg \slicep }
\,,
\ee{MFdtheta}
and the vector field for \reducedsp\ flow,
\beq
% \dot{\ssp} =
...
By construction, the motion
stays in the $(d\!-\!N)$-dimensional \slice.
    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File: infMF.xfig
\SFIG{infMF}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \PC{in \reffig{fig:infMF}:\\
        ...
        * Include a segment of the group-orbit ellipse, so $\sliceTan $ is
