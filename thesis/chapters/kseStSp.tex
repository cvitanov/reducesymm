...
We begin by computing invariants for the ``standard action'' \refneq{eq:SO2stndrd} of $\SOn{2}$ on $\Clx{n}\cong\Rls{2n}$ which we write here as
\beq
...
\beq
	\theta=
\eeq
which is a solution of \refeq{eq:SO2norm} and, when substituted back in \refneq{eq:SO2stand} results in invariants that do not depend on the sign of $b_1$.
