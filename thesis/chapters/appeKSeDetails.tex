...
are redundant from \refneq{eq:astar}, we can evaluate then nonlinear part in \refeq{expan}
...
consider a truncation to finite $N$ on its own right. Due to the hyperviscous
...
%  and we only have to compute $a_k$'s with $k\geq 1$. % Explain this in detail somewhere.
 ...
 numerical recipes basically sucks.}.	
