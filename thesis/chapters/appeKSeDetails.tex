...
%  and we only have to compute $a_k$'s with $k\geq 1$. % Explain this in detail somewhere.
 ...
 \bea
  \dot{b}_k & = & \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right)b_k  \continue
    & & - q_k \left(\sum_{m=1}^{k-1}c_m b_{k-m}+\sum_{m=k+1}^{N}c_m b_{m-k}
                    -\sum_{m=1}^{N-k}c_m b_{k+m} \right)  \continue
    & & - q_k \left(\sum_{m=1}^{k-1}b_m c_{k-m}-\sum_{m=k+1}^{N}b_m c_{m-k}
                    +\sum_{m=1}^{N-k}b_m c_{k+m} \right)
  \label{eq:tmp:b-Trunc}
 \eea
 \bea
   \dot{c}_k & = & \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right)c_k  \continue
    & & - q_k\left( \sum_{m=1}^{k-1}c_m c_{k-m}-\sum_{m=k+1}^{N}c_m c_{m-k}
                    -\sum_{m=1}^{N-k}c_m c_{k+m} \right)    \continue
    & & + q_k \left(\sum_{m=1}^{k-1}b_m b_{k-m}+\sum_{m=k+1}^{N}b_m b_{m-k}
 ...
 \etc and thus \refeq{eq:tmp:b-Trunc} and \refeq{eq:tmp:c-Trunc} simplify to
  \bea
  \dot{b}_k & = & \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right)b_k  \continue
    & & - q_k \left(\sum_{m=1}^{k-1}c_m b_{k-m}-2\sum_{m=1}^{N-k}c_m b_{k+m} \right)  \continue
    & & - q_k \left(\sum_{m=1}^{k-1}b_m c_{k-m}+2\sum_{m=1}^{N-k}b_m c_{k+m} \right)
  \label{eq:b-Trunc}
 \eea
 \bea
   \dot{c}_k & = & \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right)c_k  \continue
    & & - q_k\left( \sum_{m=1}^{k-1}c_m c_{k-m}-2\sum_{m=1}^{N-k}c_m c_{k+m} \right)  \continue
    & &  +q_k\left( \sum_{m=1}^{k-1}b_m b_{k-m}+2\sum_{m=1}^{N-k}b_m b_{k+m} \right)\,.
   \label{eq:c-Trunc}
 \eea
 ...
 \bea
    \frac{\partial \dot{c}_k}{\partial c_{j}}  =
        \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right) \delta_{kj}
            - q_k\frac{\partial}{\partial c_j}\left( \sum_{m=1}^{k-1}c_m c_{k-m}-2\sum_{m=1}^{N-k}c_m c_{k+m} \right) \,.
 \eea
 Concider the second term:
 \bea
    - q_k\frac{\partial}{\partial c_j}\left( \sum_{m=1}^{k-1}c_m c_{k-m}-2\sum_{m=1}^{N-k}c_m c_{k+m} \right) & = &
        - q_k \sum_{m=1}^{k-1} \left(\delta_{m,j} c_{k-m}+c_m \delta_{k-m,j} \right) \continue
                        & & + 2 q_k\sum_{m=1}^{N-k} \left(\delta_{m,j} c_{k+m}+c_m \delta_{k+m,j}\right)
 \eea
 ...
 \begin{itemize}
    \item $k\leq j$
        \bea
             -q_k\frac{\partial}{\partial c_j}\left( \sum_{m=1}^{k-1}c_m c_{k-m}-2\sum_{m=1}^{N-k}c_m c_{k+m} \right) & = &
                    -q_k( 0+0 ) + 2q_k (c_{k+j} + c_{j-k}) \continue
                & = &   2 q_k (c_{k+j}-c_{k-j})
        \eea
    \item $k > j$
        \bea
             -q_k\frac{\partial}{\partial c_j}\left( \sum_{m=1}^{k-1}c_m c_{k-m}-2\sum_{m=1}^{N-k}c_m c_{k+m} \right) & = &
                    -q_k(c_{k-j} + c_{k-j} ) + 2q_k (c_{k+j}  + 0 ) \continue
                & = &  2 q_k (c_{k+j}-c_{k-j})
        \eea
 \end{itemize}
 and thus
 \beq
    \frac{\partial \dot{c}_k}{\partial c_{j}} =  \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right) + 2 q_k (c_{k+j}-c_{k-j})
 \eeq

 Following the above procedure
 \beq
    \frac{\partial \dot{c}_k}{\partial b_{j}} =  2 q_k ( b_{k+j}+b_{k-j} )\,,
 \eeq
 \beq
    \frac{\partial \dot{b}_k}{\partial b_{j}} =  \left(q_k\right)^2\left(1- \left(k/\tildeL\right)^2 \right)\delta_{kj} - 2 q_k (c_{k+j} + c_{k-j}) \,,
 \eeq
 \beq
    \frac{\partial \dot{b}_k}{\partial c_{j}} = 2 q_k (b_{k+j}-b_{k-j}) \,.
 \eeq
