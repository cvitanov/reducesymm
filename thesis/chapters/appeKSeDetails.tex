...
\eeq
with $x_n = 2\pi\tildeL n/N$ and $a_{N-k} = a^\ast_k$ from the reality condition \refneq{eq:astar}
...
are redudant from \refneq{eq:astar}, we can evaluate then nonlinear part in \refeq{expan}
...
Another decision one has to take is how many modes to retain in \refneq{eq:ksPseudo}. One
...
space in \refeq{eq:ksPseudo} one has to worry about the effect of alliasing
...
results against direct evalution of the truncation of \refeq{expan}, see \refsect{sec:ksReal}.
...
To calculate stability of equilibrium $A(a_q)$, the matrix 
\beq
  A(a_q) = \left.\frac{\partial v}{\partial a}\right|_{a=a_q}
\label{eq:StabMat}
\eeq
has to be evaluated, either numerically or analytically. In \KS\
we can obtain the value of $A(a_q)$ efficiently for numerical
purposes by using the linearity of the
Fourier transform, in the same we did to obtain \ref{eq:ksPseudo},
see \refref{SCD07}. 
...
%  and we only have to compute $a_k$'s with $k\geq 1$. % Explain this in detail somewhere.
 To do so, we have to split \KS\ equation in real and imaginary
 parts \ES{or consider $a$ and $a^{*}$ as independent variables.}.
 Truncating the infinite tower of equations \refeq{expan} by setting $a_k=0$ for $k>N$, 
 using the identity \refneq{eq:astar} and splitting the
 ...
 \bea
   ...
   \label{eq:c-Trunc}
 \eea
 As discussed in \refsect{eq:ksPseudo} such expressions are not as efficient as \refeq{eq:ksPseudo},
 but we use them for comparison purposes, to detect possible issues in our pseudo-spectral integrator due to 
 aliasing or the FFT implementation\ES{For instance I was able to find using this that the FFT in
 numerical recipes basicaly sucks.}.	
