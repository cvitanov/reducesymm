...
and thus calculation of the discrete Fourier transform involves  $N-1$ independent, real variables. 
...
in $N/2-1$ real variables. The operation would correspond to multiplication of a $(N/2-1)\times(N/2-1)$
matrix by a $N/2-1$-dimensional vector, while calculating just the forward Fourier transform
seems to require the multiplication of a $(N-1)\times(N-1)$ matrix by an $N-1$-dimensional vector. 
...
Another decision one has to take is how many modes to retain in \refneq{eq:ksPseudo}. 
...
\subsection{KSe, real representation}
