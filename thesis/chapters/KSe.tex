...
\eeq
and reflections \refeq{KSparity}. Here we use subscript notation for shifts to differentiate
...
of $\On{2}$. Boundary conditions \refeq{eq:KSodd} restrict the system to \Fix{\Dn{1}}.
...
justifies truncation of the infinite tower of equations \refeq{expan} to finite
...
\begin{center}
\includegraphics[width=0.5\textwidth]{../rpo_ks/current/figs/ksBifDiag_pst.eps}
\end{center}
\caption{
...
but without explicit calculations. In this section we describe some of the elementary and well known
bifurcations in KS equation from the point of view of equivariant bifurcation theory.

 Since the origin (u(x,t)=0 in
...
attractor\cite{KNSks90}. At $L=2\pi$ the origin loses stability and due
...
of the unimodal branch, \cf\ also \refref{Krupa_bifurcations_1990}. Note that the k-modal branch
...
Condition \refneq{eq:O2ksDqFix} implies that in these states only the multiples of the $k$th
harmonic.

From here on we only consider bifurcations that play a role in the dynamics for our system size, $L=22$.
