...
Moreover, one only considers perturbations within \Fix{\On{2}},
...
system to \Fix{\Dn{1}}. To avoid technical difficulties
...
an equilibrium. From commutation relation \refeq{eq:LrzCommut} we conclude that $A(0)$
...
Therefore there is a unique solution branch of equilibria in \Fix{\Dn{1}}, \ie\ with symmetry $\Dn{1}$.\ES{
After secondary bifurcations it appears that the branch does not remain in \Dn{1}, check.}
...
\refeq{expan}. The unimodal equilibrium is stable at bifurcation.
...
and \refrefs{KNSks90,ksgreene88}.
...
At point $C$ in \reffig{fig:ksBifDiag} the $2$-cell state bifurcates to a type of
\eqv\ solution found by La Quey, Mahajan, Rutherford and Tang\rf{laquey74} and generalized by Greene and Kim who refer to them as GLMRT \eqva. In \refref{KNSks90} this branch of solutions is refered to as bi-tri branch as it is later on terminated at the trimodal branch. Bi-tri equilibria live in \Fix{\Dn{1}} and have components
in all Fourier modes\ES{Is this symmetry increasing bifurcation?}. At the point where the bi-tri branch
meets the trimodal branch a new branch is born that is still in \Fix{\Dn{1}}. This is seen as a continuation
of the bi-tri branch in \refref{KNSks90}. The equilibrium labeled \EQV{1} in \reffig{fig:ksBifDiag} is in
this branch.

Finally the \reqv\ that is labeled \REQV{\pm}{2} belongs to a branch bifurcating from the bi-tri branch in
a Hopf bifurcation. Once again the \reqva\ come in \Dn{1} related, counterotating pairs.
