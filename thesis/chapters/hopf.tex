...
ingredients to implementing this vision: (a) the geometry\ES{\ESedit{RG says more
topology than geometry.}} of
...
The simplest solution that might
exist in such a system is an \emph{equilibrium} point that is left
invariant by the flow, $f^t(x_o)=x_o$ for all times. When we examine
the neighborhood of the equilibrium we find that it can be decomposed
into a (local) \emph{stable subspace} along which points converge towards the
equilibrium and a (local) \emph{unstable subspace} along which points stray
away from the equilibrium under time evolution (for the time being
\emph{center subspaces} along which neither happens will be ignored).
Loosely speaking, the global continuation of the stable (unstable) subspace under 
backward (forward) time evolution is the \emph{stable (unstable) manifold}
of the equilibrium. Stable and unstable manifolds are \emph{flow-invariant}: 
the trajectory through any point on the manifold stays on it for all times.
In a typical nonlinear system studied in this thesis, stable and unstable manifolds are stretched away
from an equilibrium until nonlinearity causes them to fold sharply back.
As they are flow invariant they provide topological obstructions for any other
solution: as a solutions cannot cross an invariant manifold it is forced to follow its stretching
and folding. This provides a basic mechanism for \emph{recurrence}: trajectories
that are of relevance to the asymptotic dynamics (that belong to \emph{the non-wandering set})
return arbitrarily close to the initial point.}
\ESedit{This set of non-wandering orbits,
...
\beq
	x(t+T)=x(t)\,,
\eeq 
where $T$ the period. Periodic orbits provide a complete characterization
of the topology of a chaotic attractor in low dimensional systems\rf{GL-Gil07b},
and most importantly they can be used quantitatively to approximate the natural
measure and calculate ``observable'' quantities, such as
Lyapunov exponents and escape rates, within the framework of periodic
orbit theory\rf{DasBuch}, briefly summarized
here in \refappe{chap:POT}. The reason we refer to geometry rather
than topology here is that we are not only interested in elucidating
the topological mechanisms that result in recurrence but also in the
exact position of invariant objects, such as periodic orbits, in space
and this introduces a notion of distance.
}
...
details). \ESedit{A large set of periodic orbits, embedded into the
...
contributed corrections \ESedit{in a geometrical as well as quantitate sense
in the framework of periodic
orbit theory.}
This investigation was continued
for a ``more turbulent'' \KS\ system by Y.~Lan and
Cvitanovi\'c\rf{LanThesis,lanCvit07}. \ES{said elsewere?: In both studies
the dynamics where approximated to sufficient accuracy to
provide good initial guesses for periodic orbit searches by
