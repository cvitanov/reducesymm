...
For example, the vector field in Lorenz equations  \refeq{eq:lorenz} is equivariant under the group
...
As another example, the vector field in \CLe\ \refeq{eq:CLe} is equivariant under the group \SOn{2} acting on $\Rls{5}\cong \Clx{2}\times \Rls{}$
by
\beq
...
\index{Armbruster-Guckenheimer-Holmes flow!symmetry}Finally, the symmetry group of the \AGHe~\refeq{eq:AGH} is \On{2} acting by
\begin{eqnarray*}
...
\begin{example}
 The actions \refeq{eq:SO2act} of \SOn{2} and \refeq{eq:O2stndrd} of \On{2} are both faithful.
\end{example}
...
\subsubsection{Relative equilibria}
    } % end \PublicPrivate{
...
\begin{example}
Consider the action standart action of $\SOn{2}$ on \Rls{2}:
\beq
