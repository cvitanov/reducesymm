...
Here we present the method of moving coframes of Fels and Olver\cite{FelsOlver98,FelsOlver99}, also \cf~\refref{OlverInv} for
...
Cartan\ES{cite Cartan}. Here we follow the exposition in \cf~\refref{OlverInv} but we first motivate the method through an
example.
...
$SO(2)$, while the isotropy subgroup of $\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$ is the identity element. 

\begin{definition}
\label{def:free}
\index{free}
A group $\Gamma$ acts \emph{freely} on $V$ if all isotropy subgroups are trivial: \stab{x}=\{e\} for all $x\in V$. 
$\Gamma$ acts \emph{locally freely} if all isotropy subgroups are discrete subgroups of $\Gamma$.
\end{definition}

Thus $\Sigma$ does not act freely on $\Rls{5}$ but acts freely on $\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$. If we don't 
restrict $\theta$ in $[0,2\pi)$ then the group $\Rls{}$ acts locally freely on $\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$,
since the isotropy subgroup is the discrete subgroup $2\pi\mathbb{Z}$ of integer multiples of $2\pi$.

\begin{definition}
\label{def:faithfull}
\index{faithfull}
A group $\Gamma$ acts \emph{faithfully} (or \emph{effectively}) on $V$ if it has trivial global isotropy subgroup.
\end{definition}
...
A group $\Gamma$ acts \emph{semi-regularly} on $V$ if all its orbits have the same dimension. If in addition for each point $x\in V$
there exists an arbitrarily small neigborhood $U$ such that each orbit of $\Gamma$ intersects $U$ in a pathwise connected subset.
\end{definition}
...
Thus $\Sigma=\SOn{2}$ acts regurarly on $\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$ but not on $\Rls{5}$. For groups acting regularly we can define a cross-section for the group orbits. 
...
the literature or try it to fully justify this.}. On a 1GHz Pentium III processor the fundamental equivariants for $n=16$, the 
fundamental invariants for the cross section $b_1=0$ were computed in approximatelly 130s. Most importantly the time was mostly 
spend in simplification of expressions. If one only wants to project equivariant dynamics on the orbit space then the moving frame method can be used to perform the projection without explicit knowledge of the fundamental invariants. 
...
\end{array}
\]
\caption[Fundametal invariants for SO(2), n=6.]{Tabulation of fundamental invariants for the standard action of \SOn{2} on \Rls{6}}
\end{table}
