...
group \Ztwo) or as a reflection about the origin in a plane perpendicular to the $z$-axis (hence the group \Dn{1}.)
...
\begin{proposition}
 ...
 \end{enumerate}
\end{proposition}
The proof can be found in \refref{golubII}. We note a useful relation from the proof:
...
a disjoint union of strata which are in an $1-1$ correspondence to the group orbit types. Notice that in general
...
Now we examine how the vertex $A$ of the triangle transforms
under the axiom of \Dn{3}. Under $\Drot$ and $\Drot^2$ it is
...
Thus $\Sigma=\SOn{2}$ acts regularly on $V^*$ but not on $\Rls{5}$. For groups acting regularly we can define a cross-section for the group orbits.
...
a high-dimensional space. For example the
