...
\[
	\Rot{\pi}(x,y,z) = (-x,-y,z)\,.
\]
Note that this transformation can be considered either as
...
\ES{Is this related to us being able to construct a local {\slice}?}.

\PublicPrivate{}{
\subsection{Principal fibre bundles}

Assume a Lie group $\Gamma$ acts\ES{Any restriction on how it acts? In definition in Isham the term G-space is used, which I do not understand since he doesn't define it.} on a topological space $\tSp$. We call the bundle $(\tSp,\prj,\bSp)$ a \emph{$\Gamma$-bundle} if it is isomorphic to $(\tSp,\rho,\tSp/\Gamma)$. The fibres over the points of the base space $\tSp/\Gamma$ are the group orbits of points in $\tSp$.
...
$\dim \Gamma - \dim \stab{x}$ passing\ES{not passing if disconnected, fix this.} through $x$.
...
For groups acting regularly we can define a {\slice} for the group orbits.
    \PublicPrivate{}{
...
\begin{proposition}%[\rf{OlverInv}]
 ...
 Let $\Gamma$ act regularly on a $n$-dimensional manifold
 $\Manif$ with $r$-dimensional orbits. Define a (local)
 \emph{{\slice}} to be an $(n-r)$-dimensional submanifold $K$
 of $\Manif$ such that $K$ intersects each orbit
 transversally and at most once. If a Lie group $\Gamma$ acts
 regularly on a manifold $\Manif$, then one can construct a
 local {\slice} passing through any point $x\in \Manif$.
\end{proposition}

\ES{Is this {\slice} related to a cross-section in a $\Gamma$-bundle?
