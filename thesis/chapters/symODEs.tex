...
\Dn{m}, $m>0$, of \On{2} is generated by the reflection $\Refl$ and a rotation $\Rot{2\pi/m}$. 
The subgroup \Cn{q}, $q>0$ of \SOn{2} is generated by a rotation $\Rot{2\pi/q}$.
If $q$ divides $m$ then we have the following containment relations $\Cn{q}\subset\Cn{m}\subset\Dn{m}$ 
and $\Dn{q}\subset\Dn{m}$. Note that $\Cn{1}\cong\Idg$.
...
\refneq{eq:O2CqFix} cannot be satisfied and all $z_k$'s have to be equal to zero. This implies that the \fixedsp\ of $D_q$ and $C_q$ for $q>n$ is the origin and as a result those subgroups are not in the isotropy lattice.

For reasons that will become apparent in the study of \KSe\ we owe some special attention to the $n$-dimensional \fixedsp\ of $\Dn{1}(\kappa)$ given by the condition $Im(z_k)=0$ for $k=1,\ldots,n$.  $\Dn{1}(\kappa)$ is conjugate to the one-parameter family of subgroups $\Dn{1}(\kappa \Rot{\theta})$
for $\theta\in[0,2\pi)$ which implies that the \fixedsp\ of $\Dn{1}(\kappa)$ comes in copies related by
the action of $\Rot{\theta}$.
...
\caption[O(2) lattices]{
	...
	defined by \refeq{eq:O2irep}. In both figures dashed lines inticate
	containement only when $q$ divides $m$, while the relations $\Cn{q}\subset\Dn{q}$
	and $\Dn{q}\subset\Dn{m}$ when $q$ divides $m$ are not drawn. For the isotropy lattice $m,q\le n$.
	}}
\label{fig:O2lattice}
    \vspace*{-5pt}
\end{figure}
...
For example $\Sigma=\SOn{2}$ acts regularly on $\Manif^*$ but not on $\Rls{5}$. 

The group orbits of an effective and regural or semiregular action of a Lie group $\Gamma$ on a manifold \Manif\ form a foliation of \Manif\ES{Is this related to us being able to construct a local cross-section?}. For groups acting regularly we can define a cross-section for the group orbits.
