...
of a group $\Gamma$ on a manifold $X$ under certain conditions. 
...
$\SOn{2}$, while the isotropy subgroup of $X^*\equiv\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$ is the identity element.

\begin{definition}
\label{def:free}
\index{free}
A group $\Gamma$ acts freely on $X$ if all isotropy subgroups are trivial: \stab{x}=\{e\} for all $x\in X$.
$\Gamma$ acts locally freely if all isotropy subgroups are discrete subgroups of $\Gamma$.
\end{definition}

Thus $\Sigma$ does not act freely on $\Rls{5}$ but acts freely on $X^*$. If we don't
restrict $\theta$ in $[0,2\pi)$ then the group $\Rls{}$ acts locally freely on $X^*$
since the isotropy subgroup is the discrete subgroup $2\pi\mathbb{Z}$ of integer multiples of $2\pi$.

\begin{definition}
\label{def:faithfull}
\index{faithful}
A group $\Gamma$ acts faithfully (or effectively) on $X$ if it has trivial global isotropy subgroup.
\end{definition}
...
A group $\Gamma$ acts semi-regularly on $X$ if all its orbits have the same dimension. If in addition for each point $x\in X$
...
Thus $\Sigma=\SOn{2}$ acts regularly on $X^*$ but not on $\Rls{5}$. For groups acting regularly we can define a cross-section for the group orbits.

\begin{definition}
\label{def:cross-section}
\index{cross-section}
Let $\Gamma$ act regularly on a $n$-dimensional manifold $X$ with $r$-dimensional orbits. Define a (local) \emph{cross-section}
to be an $(n-r)$-dimensional submanifold $K$ of $X$ such that $K$ intersects each orbit transversally and at most once.
\end{definition}

\begin{proposition}[\cite{OlverInv}]
 If a Lie group $\Gamma$ acts regularly on a manifold $X$, then one can construct a local cross-section
 passing through any point $x\in X$.
\end{proposition}
...
thus $\Gamma$-orbits have the same dimension, say $r$, as $\Gamma$. The condition of free
action can be relaxed \cite{OlverInv}. Choose a coordinate cross-section $K=\{x_1=c_1,\ldots,x_r=c_r\}$.
