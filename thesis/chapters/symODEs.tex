...
\begin{definition}
\label{def:stab}
\index{isotropy subgroup}\index{stabilizer} Define the isotropy subgroup or stabilizer of $x\in\Rls{n}$ as
\beq
...
The isotropy subgroups are: $\stab{\{0\}}=\Dn{3}$, $\stab{I_1^*}=\Zn{2}(\Refl)\simeq\stab{I_2^*}\simeq\stab{I_3^*}$, $\stab{\Rls{2}\backslash\{\cup I_i\}}=\Idg$, 
...
There are three strata in correspondance with the orbit types (and with isotropy subgroups): the origin (type \Dn{3}),
$\{\cup I_i^*\}$ (type $\Zn{2}(\Refl)$), and the principal stratum $\Rls{2}\backslash\{\cup I_i\}$ (type $\Idg$).

If we now consider a two dimensional system of \ode s equivariant under the action \refneq{eq:D3onR2} of \Dn{3} we
can conlude immediatelly that the \fixedsp s are flow invariant by \refPro{pro:gfInv}. Thus the origin has to be
a \fixedpnt of the flow. Moreover the primary stratum $\Rls{2}\backslash\{\cup I_i\}$ is partitioned by the symmetry
...
of a group $\Gamma$ on a space $V$ under certain conditions. It draws inspiration from the method of moving frames of
Cartan.
    \ES{cite Cartan}
...
of \CLe. Choose coordinates $x_1,x_2,y_1,y_2,z$ on $\Rls{5}$, related tp the complex coordinates
of \refeq{eq:RotCLe} by $x=x_1+i x_2$, $y=y_1+i y_2$. Notice that the \fixedsp\
...
$\SOn{2}$, while the isotropy subgroup of $V^*\equiv\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$ is the identity element.
...
Thus $\Sigma$ does not act freely on $\Rls{5}$ but acts freely on $V^*$. If we don't
restrict $\theta$ in $[0,2\pi)$ then the group $\Rls{}$ acts locally freely on $V^*$
...
Thus $\Sigma=\SOn{2}$ acts regurarly on $V^*$ but not on $\Rls{5}$. For groups acting regularly we can define a cross-section for the group orbits.
...
We can now construct a moving frame for the action \refneq{eq:SO2act} of $\SOn{2}$ as follows. We write out explicitly the
...
\end{split}
\eeq
Then make $\overline{x_1}$ equal to the constant in the choice of cross-section, \ie~set $\overline{x_1}=0$. Thus, we can solve
...
$\theta = \tan^{-1}\frac{x_2}{x_1}$ our results are multiplied by $sgn(x_2)$.} We notice that $\overline{x}_2,\overline{y}_1,
...
$n=6$ calculated with the method of moving frames are listed in \reftab{tab:SO2n6}. Notice another advantage of the method: no syzygies are present.
