...
of a group $\Gamma$ on a space $V$ under certain conditions. It draws from the method of moving frames of
Cartan.
    \ES{cite Cartan}
...
$\SOn{2}$, while the isotropy subgroup of $V^*\equiv\Rls{5}\backslash\{x_1=x_2=y_1=y_2=0\}$ is the identity element\ES{Is $V^*$ a manifold? Is its isotropy subgroup defineds?}.
...
to be an $(m-r)$-dimensional\ES{check: $n-r$-dimensional?} submanifold $K$ of $V$ such that $K$ intersects each orbit transversally and at most once.
\end{definition}
...
in much more general situations as follows. Assume $\Gamma$ acts (locally\ES{Check that locally freely is enough.}) freely on X and
