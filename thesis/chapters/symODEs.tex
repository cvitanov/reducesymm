...
and the usual \Le\ are recovered. From equivariance, any subspace on the $\SOn{2}$-orbit of the real
subspace is invariant as well, for example the imaginary subspace $x_1=y_1=0$. We demonstrate the
situation for the standard \Le\ parameters in \reffig{fig:LorenzCoex}. A continuum of identical, disjoint 
Lorenz ``butterfly'' attractors exists. 

Yet, we have to notice that we cannot choose all initial conditions
in one of the flow invariant subspaces. Any point $(x_1,0,y_1 \cos \theta, -y_1 \sin\theta$ cannot
does not belong to those invariant subspaces.
