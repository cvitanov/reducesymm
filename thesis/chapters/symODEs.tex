...
If $q$ divides $m$ then \Cn{q} and \Dn{q}  are subrgoups of \Dn{m}.

The \fixedsp\ of \On{2} and \SOn{2} is the origin and thus \SOn{2} is not in the isotropy lattice. 
The \fixedsp\ of $\Dn{1}(\kappa)$ is given by the condition $Im(z_k)=0$ for $k=1,\ldots,n$. The \fixedsp\
of \Cn{q} is given by the condition that $z_k=0$ unless $q$ divides $k$, while for
the \fixedsp\ of \Dn{q} we add the condition that $Im(z_k)=0$ for $k=1,\ldots,n$. Note that $q$
can't divide any of the $k$'s for $q>n$ and thus the \fixedsp of $D_q$ and $C_q$ for $q>n$ is the
origin and thus those subgroups are not in the isotropy lattice.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}
    % \vspace*{-5pt}
\begin{center}
  (\textit{a})\includegraphics[height=0.2\textheight]{../figs/O2lattice}
~~~~(\textit{b})\includegraphics[height=0.2\textheight]{../figs/O2stablattice}
\end{center}
\caption[O(2) lattices]{
	...
	containement only when $q$ divides $m$, while the relation $\Dn{q}\subset\Dn{m}$
	when $q$ divides $m$ is not drawn. For the isotropy lattice $m,q\le n$.
	}}
\label{fig:O2lattice}
    \vspace*{-5pt}
\end{figure}
