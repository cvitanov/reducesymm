...
exactly once. The existense of a section is guaranteed since a cross-section as in \refPro{pro:crossExists} exists,
...
the cross-section $\mathcal{K}$. This provides a means of calculating stability of \reqva in
reduced \statesp. Assume that, as was the case for \CLe, the cross-section is orthogonal
to the group action for any $x$ on $\mathcal{K}$. We observe that for the point $x_o$ on \reqv\ \REQB{1} that lies on the cross-section $\mathcal{K}$ we can decompose $\vf(x)$ in \refeq{eq:difeq} in a part $\vf_\shortparallel$ parallel to the group action and a part $\vf_\perp$ on the cross-section
\beq
...
convenient to introduce the operator
 ...
 \eeq
that projects a vector to the cross-section. Then for the stability matrix $M_{ij}$ we have
 \bea
...
depend on the point on the cross-section and thus when we consider
...
\eeq
Since hyperbolicity of a \reqv\ is determined by the dynamics on the cross-section (see \rf{Krupa90})
we identify
\beq
...
\ESedit{
\subsection{Integration on the cross-section}
...
\eeq
solutions will stay on the cross-section $\mathcal{K}$ for any initial condition on $\mathcal{K}$
...
stay on the cross-section is not verified by this figure,
even though this could be attributed to numerical issues.
    ...
    cross-section integrations at the same point, and
    ...
    is wrong with the cross-section integration. }
...
In figure \reffig{fig:CLEerrTransv} we evaluate the accuracy of integration
of \refeq{eq:difeqTransv} by integrating initial conditions for several \rpo s 
both in \statesp\ and on the section and compute the distance 
$|\overline{\ssp}(t)-\overline{\ssp^\perp}(t)|$ in invariant variables \refneq{eq:invLaser2} 
(the norm is Euclidean).
The distance does not grow exponentially indicating that numerical solution
of \refeq{eq:difeqTransv} is accurate.
