...
\refFig{fig:CLE} of \CLe\ dynamics for $r_1=28,\, b=8/3,\, \sigma=10,\, a=1$, $e=0.01$, $r_2=0$ 
...
One approach to orbit space reduction is by using invariant polynomials as
has been done in \refref{GL-Gil07b}. The following set of invariant polynomials
has been used:
\beq
\begin{split}
...
One has to notice that those polynomials are not functionally independent. The syzygy is
\beq
...
the tearing mechanism is hidded since the dynamics is squeezed near the $z$-axis. 
...
surface of section $u_1=u_4$ for \CLe\ with $r_1=28,\, b=8/3,\, \sigma=10,\, a=1$, $e=0.01$, $r_2=0$,
...
we don't visit the lobe related by rotation by $\pi$, in reallity no such thing exists) we get a rapid
change in angle as the trajectory passes close to the origin. In the $y$-plane we don't necesserilly change
...
Since $x$ cannot vanish the problem is mostly aesthetical in the present case, but for \KSe it will be important to prevent
the denominator from vanishing. Since \SOn{2} acts irreducibly in the \CLe example (and also in \KSe) we cannot take more than one irreducible subspace into account when seting up the normalization equations. We can however restore democracy
...
surface of section $\overline{x}_2=\overline{y}_2$ for \CLe\ with $r_1=28,\, b=8/3,\, \sigma=10,\, a=1$, $e=0.01$, $r_2=0$,
