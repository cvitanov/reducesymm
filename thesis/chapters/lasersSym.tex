...
show that, for $e+r_2\neq 0$, a Hopf cycle \REQB{1} is created that also turns out to be an \SOn{2}-orbit,
...
have to reduce the surrogate \Poincare sections to those that are invariant (as a set) under 
...
Here we are naturally led to choose a section that passes through the \reqv \REQB{1} such the
section $\mathcal{P}_1$ defined by $\overline{x}_2=\overline{y}_2$ in the variables defined by
\refeq{eq:CLEinvLaserRepeat} or in original space $x_1^2+x_2^2=x_1 y_1 + x_2 y_2$. 
...
of intersection of \rpo ``01'' have been visualized as circles. 
...
exactly once. The existense of a section is guaranteed since a cross-section as defined in \refDef{def:cross-section} exists. Here we choose $x_2=0$ for $\mathcal{P}_2$. Geometrically
...
as is the case with truncations of PDE's. Globally the transformation is still non-linear through the
