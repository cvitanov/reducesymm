...
\section{Candidates for the role of Metric}
 ...
 \subsection{A Jacobian Matrix for a Loop}
  ...
  \eea
   ...
   \beq
   	\fder{\Js^s(x_o)}{s}=\dtds\,\mathbf{A}(x(s))\,\Js^s(x_o).
	\label{eq:difJ}
   \eeq
 ...
 \subsection{Properties of $\JL$}
  ...
  \beq
  	\fder{N_{ij}^s}{s}  =  \fder{\hat{J}_{ij}^s}{s}-\fder{H_{im}}{s}J^s_{mk}H^{-1}_{kj}
						-H_{im}\fder{J^s_{mk}}{s}H^{-1}_{kj} \,,
  \eeq
  or, using \refeq{eq:difJ} and \refeq{eq:difhatJ},
  \beq
	\fder{N_{ij}^s}{s}  = \hdtds\hat{A}_{im} \hat{J}^s_{mj}-\fp{H_{im}}{x_n}\fder{x_n}{s}J_{mk}H^{-1}_{kj}
  ...
  \beq	
	\fder{N_{ij}^s}{s} = \hdtds \left(\fp{H_{in}}{x_k}v_n + H_{in} A_{nk} \right)
				H^{-1}_{km}(x(s)) \hat{J}^s_{mj}
				-\left(\fp{H_{im}}{x_n}\fder{x_n}{s}
  ...
  \bea
	\fder{N_{ij}^s}{s} & = &  \left(\fp{H_{ik}}{x_n} \fder{x_n}{s} + \fder{t}{s} H_{in} A_{nk} \right)
				H^{-1}_{km}(x(s)) \hat{J}^s_{mj}
				-\left(\fp{H_{im}}{x_n}\fder{x_n}{s}
						+\fder{t}{s}H_{in}A_{nm}\right)H^{-1}_{mq}H_{ql}J^s_{lk}H^{-1}_{kj}(x_o) \continue
			& = & \left(\fp{H_{ik}}{x_n} \fder{x_n}{s} + \fder{t}{s} H_{in} A_{nk} \right) H^{-1}_{km} N_{mj} \,,
	\label{eq:dNds}
  \eea
...
\subsection{The variational method}
 ...
 \bea
	F^2	& = & \int_{t_i=0}^{T} d t \left(\fder{x}{t}-v(x)\right)^2 \continue
		& = & \int_{t_i=0}^{T} d t \left(\fder {s}{t}\right)^2\left(\tilde{v}(x)-v(x)\fder{t}{s}\right)^2\,,
	\label{eq:Fdiscr}
 \eea
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \begin{figure}[t!]
% 	(a)~\includegraphics[width=1.8in]{figs/ks21}%
% 	\hspace{0.2cm}%
% 	(b)~\includegraphics[width=1.8in]{figs/ks22}
% \caption{
%  My System in the
