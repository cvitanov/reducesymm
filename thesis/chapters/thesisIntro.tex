...
flow\rf{Visw07b}. A pretty recent application of \rpo s has
...
\PCedit{following the above cited imaginary paper}\ES{My purpose was to differentiate
the thesis from those imaginary papers, not sure I should say that.}, we emphasize that we develop methods in
a way that can be applicable in other PDEs. \ESedit{The choice of \KS\ system size
is such that the dynamics in physical space look nothing like ``fully developed turbulence''.
They are rather dominated by coherent structures, precisely the kind of dynamics in which
statistical assumptions\rf{frisch} fail and require a dynamical systems description in
the spirit of Hopf. In the absence of a better term we call our \KS\ dynamics ``turbulent''
without any claim that we provide a description of fluid turbulence. The purpose is to develop
ideas of relevance to other flows with coherent structures.}
% We will concentrate on a system size for which the dynamics are chaotic
% but we will not get into the spatiotemporally chaotic regime.
% Therefore when the term ``turbulence'' is
% used in this thesis for \KS\ equation it will be a synonym for chaos but will help
% to remind us that we deal with a spatially extended system.
	...
	{\bf ES}: No, I don't, but do we have a better, widely accepted word
		...
		I just drop the distinction alltogether.
       }
In \refchap{chap:KSe} we review \KS\ equation in detail and in
\refchap{chap:kseStSp} we study it's \statesp, through \eqva,
