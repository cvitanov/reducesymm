...
\ESedit{While in practice the application of the method to more complicated problems
turns out to be far from trivial,
the exercise leads one to critically re-examine several steps along the way that
the above quoted paper (profound as it might be)} only glossed over.
...
\ee{intro:ks}
\ESedit{
has been derived in a variety of contexts (see \refchap{chap:KSe}) including
the dynamics of reaction-diffusion systems and fluttering flame fronts
and has been studied extensively as on of the simplest systems with behavior
that shares common features with Navier-Stokes equations describing
incompressible fluids. By this bold statement we mean that
it shares some common structure, namely a term that pumps energy into the
system ($u_{xx}$) and a term that dissipates it ($u_{xxxx}$). Furthermore
it is also translationaly invariant under periodic boundary condition $u(x+L,t)=u(x,t)$ or
in an unbounded domain $x\in(-\infty,\infty)$, a feature also present in
many physical fluid flows.} In \refrefs{Christiansen97,lanCvit07,LanThesis} dynamics was restricted
...
\ESedit{As a result,
KS can have \emph{traveling wave} or \emph{\reqv} solutions,
\beq
 u(x,t)= u_o(x-c t)
\eeq 
where $c$ the constant velocity of the wave and $u_o(x)$ its profile. Furthermore, it can have
\emph{modulated traveling wave} or \emph{\rpo} solutions,
\beq
...
only generic \ESedit{compact} solutions, we are also faced with relative
...
structures}, \ESedit{that is localized, persistent structures.}. This is precisely the kind of dynamics in which
...
\ESedit{When a symmetry is present in a physical system taking it into account usually leads to simplification of the problem.} 
...
High dimensionality does not allow us to use a very powerful tool in symmetry reduction, \emph{Hilbert bases}. 
\ESedit{
These are bases of polynomials invariant under the group action in which one rewrites the dynamics, (see \refsect{sec:symRed}).
}
Unfortunately their determination appears at the present moment computationally prohibitive for 
phase-space dimension larger than ten\rf{ChossLaut00,gatermannHab}. Moreover, even if such a basis were available,
rewriting the equation of motion in a basis of invariant polynomials appears impractical for high-dimensional
flows, 
\ESedit{
eventhough this difficulty can be surpassed by mapping the solution to the invariant variables
}.
\ES{dropped: The issue of absence of  a global transformation demonstrates itself in different
...
\ESedit{
A different approach to symmetry reduction of PDEs has been presented by Rowley and Marsden\rf{rowley_reconstruction_2000}
formulated in the context of {Karhunen-Lo\`{e}ve} expansion but also applicable to direct numerical 
simulations\rf{rowley_reduction_2003}. The method allows one to integrate a PDE defined in the \emph{reduced space}
along with a \emph{reconstruction equation} that allows the dynamics in the original space to be recovered.
As noticed in \refref{rowley_reconstruction_2000} the reconstruction equation can fail for reasons related
to the structure of \statesp\ under the group action, see \refsect{sec:symRed} and one would
be forced to cover the reduced space with local coordinate charts. As the latter procedure is
not straightforward to implement and we don't want to
sacrifice the ability to move back and forth between the initial and the reduced \statesp we will need
to take a different path.
} 

\ESedit{
Our approach to symmetry reduction is centered around the method of moving frames of Cartan\rf{CartanMF}
that we present in the formulation of Fels and Olver\rf{FelsOlver98} in \refsect{sec:mf}. It allows 
the determination of (non-polynomial) invariants of the group action by a simple
and efficient algorithm that works well in \statesp\ dimension of order $100$.
Invariants generated for \CLe\ (see \refsect{sec:CLeMF}) and for \KS\ equation
(see \refsect{sec:CLeMF}) are singular in subsets of \statesp, again due to the
special structure of the \statesp\ under the group action. We modify these invariants so that there is 
no singularity in regions of dynamical interest and visualize dynamics of invariant objects in
reduced \statesp\ by mapping solutions computed in the original space
to this basis.
}

\ESedit{
Visualization in reduced space provides insight in its geometry and facilitates the choice 
of local Poincar\'e sections as the first step in
our attempt to describe the flow by discrete maps.}
...
see \refsect{sec:CLeMF}.
\ESedit{
This simplification allows one to perform the reduction procedure
for points of intersection of solutions with the Poincar\'e section very efficiently even
for very high dimensional spaces.
}

For \KS\ equation construction of discrete time maps in reduced \statesp\ is still a subject of ongoing
work, see \refchap{chap:tobedone}. Yet the reward of applying this procedure for visualization
...
to recurrence within the ``chaotic attractor'', see \refchap{chap:kseRedStSp}.}

\subsection{Updated version}

\ESedit{This thesis is part of an educational procedure that has come to an end but also
of an ongoing research project. Therefore errors and omissions are certainly present in this
version and the author will make every effort to correct them in an updated version available in
http://www.chaosbook.org/, under a link to theses, and it is recommended that the reader 
downloads the latest version or contacts the author for a corrected copy.
}
