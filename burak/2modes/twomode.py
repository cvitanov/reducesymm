...
def velocityvarRed(x, t, p=params):
    """
    Velocity function for the variational equation.
    Takes 4+4x4-D input x and parameter vector p as the input and gives
    4+4x4-D velocity output
    ...
    vel = np.zeros(4+4*4) #Generate the dummy vector
    xphi = np.append(x[0:4], 0)
    vel[0:4] = vhatvphi(xphi,t,p)[0:4] #First 4 elements are the regular velocity
    mvars = x[4:4+4*4].reshape(4,4) #Read the variations matrix
    mvelvar = np.dot(StabilityMatrixRed(x[0:4],p), mvars) #Velocity of variations in matrix form
    
    vel[4:4+4*4] = mvelvar.reshape(16) 
    
    return vel

def intvarRed(x0, t, p=params, abserror=1.0e-9, relerror=1.0e-9):
    """
    Integrate variational equation.
    Takes the initial condition, parameters and the time interval
    returns the result as a series in time.
    """
    xvarsol = odeint(velocityvarRed, x0, t, args=(p,), atol = abserror, rtol = relerror)
    return xvarsol

def JacobianRed(x, Ti):
    """
    Jacobian J^Ti(x)
    """ 
    xvar = np.append(x, np.identity(4).reshape(16))
...
def ftauRed(x, tau):
    ...
    xphi = np.append(x, 0)
    xsol = intslice(xphi, t, abserror=1.0e-9, relerror=1.0e-9)
    fxtau = xsol[1,:]
